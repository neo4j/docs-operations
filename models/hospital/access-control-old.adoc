...
For more information, see link:{neo4j-docs-base-uri}/cypher-manual/5/access-control/[Cypher Manual -> Access control].
...
[NOTE]
====
Privileges that were granted or denied earlier can be revoked using link:{neo4j-docs-base-uri}/cypher-manual/5/access-control/manage-privileges/#access-control-revoke-privileges[the `REVOKE` command].
====
...
Refer to the section link:{neo4j-docs-base-uri}/cypher-manual/5/access-control/built-in-roles/[Cypher Manual -> The `admin` role] for a complete list of privileges to consider.
====
