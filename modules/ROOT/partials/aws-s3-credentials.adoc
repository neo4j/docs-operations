...
region=us-east-1
----
. Configure the access to your AWS S3 bucket by either setting the `aws_access_key_id` and `aws_secret_access_key` in the `~/.aws/credentials` file or by using a bucket policy.
For example:

* Use `aws configure set aws_access_key_id aws_secret_access_key` command to set your IAM credetials from AWS and verify that the `~/.aws/credentials` is correct:
+
[source,shell]
----
cat ~/.aws/credentials
...
* Alternatively, you can use a resource-based policy to grant access permissions to your S3 bucket and the objects in it.
