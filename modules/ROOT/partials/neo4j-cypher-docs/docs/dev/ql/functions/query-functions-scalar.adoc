...
Function `size()`:: Only works for the three types: <<functions-size-of-string, strings>>, <<functions-size, lists>>, and <<functions-size-of-pattern-comprehension, pattern comprehension>>.
...
For examples on how to get a node and a relationship by ID, see <<get-node-rel-by-id>>.
...
|===
| +uuid+
| +"1edd958b-3677-4c40-9780-ec9598a94e22"+
1+d|Rows: 1
|===
