...
|&lt;prefix&gt;.page_cache.page_faults|The total number of page faults happened in the page cache. If this continues to rise over time, it could be an indication that more page cache is needed. Be aware that on Neo4j startup, all page cache warmup activity results in page faults and so it is not unexpected to see a significant page fault count immediately after startup. (counter)
