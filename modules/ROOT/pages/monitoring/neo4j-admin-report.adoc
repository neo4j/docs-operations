...
----
neo4j-admin server report [-h] [--expand-commands] [--list] [--verbose] [--ignore-disk-space-check[=true|false]]
                          [-a=<address>] [--additional-config=<file>] [--database=<database>] [-p=<password>]
...
The classifiers marked as _Requires authentication_ require you to provide authentication credentials to the running Neo4j instance.
...
By default, the address is derived from the instance configuration, but you can override it using the `--address` option.
...
|Address of the DBMS to connect to, including the scheme (e.g. bolt://localhost:7687 or
...
connection to a live database is selected. Can be specified as the NEO4J_PASSWORD
...
connection to a live database is selected. Can be specified as the NEO4J_USERNAME
environment variable.
|
...
The following command interactively prompts for the password for user `neo4j`, gathers information about the Neo4j instance using all classifiers, and saves it to a specified location:

[source, shell]
----
neo4j-admin server report --username=neo4j --to-path=./report all
----
====
...
[source, shell]
----
neo4j-admin server report --to-path=./report threads logs
----
====
