...
| Heap usage
| `<perfix>.dbms.vm.heap.used` and `<perfix>.dbms.vm.heap.total`
| If Neo4j consistently uses 100% of the heap, increase the initial and max heap size.
For more information, see xref:performance/memory-configuration.adoc[].

| Page cache
| `<perfix>.dbms.page_cache.hit_ratio`  and `<perfix>.dbms.page_cache.usage_ratio`
...
| Checkpoint time
| `<perfix>.database.<db>.check_point.duration`
...
| Leader
| `<perfix>.database.<db>.cluster.core.is_leader`
...
| `<perfix>.database.<db>.transaction.last_committed_tx_id`
