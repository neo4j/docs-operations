...
Neo4j's xref:performance/disks-ram-and-other-tips.adoc#performance-storage[performance storage best practices] recommend that customers place database store files and transaction logs on different drives.

An out of disk event may cause the database going offline and thus create the risk of database or log file corruption. 
To ensure system availability is not impacted by such an event, follow these instructions:

* Set up system monitoring tools to monitor available disk spaces on all drives used for databases, indexes, and transaction logs.
* Monitoring tools that permit actions to be taken when the alert threshold is reached should be set to issue the command `ALTER DATABASE name SET ACCESS READ ONLY`.
* The poll frequency and % free disk space alert should be configured such that there is adequate time to perform the `ALTER DATABASE` command during the database's peak load. 

[NOTE]
====
It is recommended that at least two full poll cycles at the database's peak load should be able to elapse without exceeding the % free disk space warning.
====
