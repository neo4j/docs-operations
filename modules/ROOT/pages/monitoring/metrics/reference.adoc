...
For lists of changed, deprecated, or removed metrics in the 2025-2026 series, refer to xref:changes-2025-2026.adoc[Changes in Neo4j 2025-2026 series], xref:deprecations.adoc[Current deprecations], and xref:breaking-changes.adoc[Breaking changes in Neo4j 2025.01].
...
For more information about how Cypher plans and caches queries, refer to the link:{neo4j-docs-base-uri}/cypher-manual/current/planning-and-tuning[Cypher Manual -> Query plans, tuning, and caching].
...
Neo4j caches 1000 plans by default.
Seeing sustained replanning events or large spikes could indicate an issue that needs to be investigated. (counter)

| <prefix>.cypher.replan_wait_time
| The total number of seconds waited between query replans. (counter)

| <prefix>.cypher_cache_ast_cache_flushes_total
| The total number of times the entire AST cache was cleared.
This can happen either explicitly via a procedure, if a database is stopped, or as a result of a schema change (for example, `CREATE INDEX`). (counter)

| <prefix>.cypher_cache_ast_compiled_total
| The total number of times Neo4j replaced a query's plan with a more optimized version that compiles expressions into executable code, after the query had been run repeatedly.
Not available in Community Edition. (counter)

| <prefix>.cypher_cache_ast_discards_total
| The total number of entries evicted from the AST cache. (counter)

| <prefix>.cypher_cache_ast_entries
| The current number of entries in the AST cache. (gauge)

| <prefix>.cypher_cache_ast_hits_total
| The total number of AST cache hits. (counter)

| <prefix>.cypher_cache_ast_misses_total
| The total number of AST cache misses. (counter)

| <prefix>.cypher_cache_ast_stale_entries_total
| The total number of times a query in the AST cache was replanned because its existing plan was considered stale (due to statistics divergence and/or elapsed time).
Related to `replan_events` above.
This is a counter of such events, not the current cache size. (counter)

| <prefix>.cypher_cache_executable_query_cache_flushes_total
| The total number of times the entire executable query cache was cleared.
This can happen either explicitly via a procedure, if a database is stopped, or as a result of a schema change (for example, `CREATE INDEX`). (counter)

| <prefix>.cypher_cache_executable_query_compiled_total
| The total number of times Neo4j replaced a query's plan with a more optimized version that compiles expressions into executable code, after the query had been run repeatedly.
Not available in Community Edition. (counter)

| <prefix>.cypher_cache_executable_query_discards_total
| The total number of entries evicted from the executable query cache. (counter)

| <prefix>.cypher_cache_executable_query_entries
| The current number of entries in the executable query cache. (gauge)

| <prefix>.cypher_cache_executable_query_hits_total
| The total number of executable query cache hits. (counter)

| <prefix>.cypher_cache_executable_query_misses_total
| The total number of executable query cache misses. (counter)

| <prefix>.cypher_cache_executable_query_stale_entries_total
| The total number of times a query in the executable query cache was replanned because its existing plan was considered stale (due to statistics divergence and/or elapsed time).
Related to `replan_events` above.
This is a counter of such events, not the current cache size. (counter)

| <prefix>.cypher_cache_execution_plan_cache_flushes_total
| The total number of times the entire execution plan cache was cleared.
This can happen either explicitly via a procedure, if a database is stopped, or as a result of a schema change (for example, `CREATE INDEX`). (counter)

| <prefix>.cypher_cache_execution_plan_compiled_total
| The total number of times Neo4j replaced a query's plan with a more optimized version that compiles expressions into executable code, after the query had been run repeatedly.
Not available in Community Edition. (counter)

| <prefix>.cypher_cache_execution_plan_discards_total
| The total number of entries evicted from the execution plan cache. (counter)

| <prefix>.cypher_cache_execution_plan_entries
| The current number of entries in the execution plan cache. (gauge)

| <prefix>.cypher_cache_execution_plan_hits_total
| The total number of execution plan cache hits. (counter)

| <prefix>.cypher_cache_execution_plan_misses_total
| The total number of execution plan cache misses. (counter)

| <prefix>.cypher_cache_execution_plan_stale_entries_total
| The total number of times a query in the execution plan cache was replanned because its existing plan was considered stale (due to statistics divergence and/or elapsed time).
Related to `replan_events` above.
This is a counter of such events, not the current cache size. (counter)

| <prefix>.cypher_cache_logical_plan_cache_flushes_total
| The total number of times the entire logical plan cache was cleared.
This can happen either explicitly via a procedure, if a database is stopped, or as a result of a schema change (for example, `CREATE INDEX`). (counter)

| <prefix>.cypher_cache_logical_plan_compiled_total
| The total number of times Neo4j replaced a query's plan with a more optimized version that compiles expressions into executable code, after the query had been run repeatedly.
Not available in Community Edition. (counter)

| <prefix>.cypher_cache_logical_plan_discards_total
| The total number of entries evicted from the logical plan cache. (counter)

| <prefix>.cypher_cache_logical_plan_entries
| The current number of entries in the logical plan cache. (gauge)

| <prefix>.cypher_cache_logical_plan_hits_total
| The total number of logical plan cache hits. (counter)

| <prefix>.cypher_cache_logical_plan_misses_total
| The total number of logical plan cache misses. (counter)

| <prefix>.cypher_cache_logical_plan_stale_entries_total
| The total number of times a query in the logical plan cache was replanned because its existing plan was considered stale (due to statistics divergence and/or elapsed time).
Related to `replan_events` above.
This is a counter of such events, not the current cache size. (counter)

| <prefix>.cypher_cache_preparser_cache_flushes_total
| The total number of times the entire preParser cache was cleared.
This can happen either explicitly via a procedure, if a database is stopped, or as a result of a schema change (for example, `CREATE INDEX`). (counter)

| <prefix>.cypher_cache_preparser_compiled_total
| The total number of times Neo4j replaced a query's plan with a more optimized version that compiles expressions into executable code, after the query had been run repeatedly.
Not available in Community Edition. (counter)

| <prefix>.cypher_cache_preparser_discards_total
| The total number of entries evicted from the preParser cache. (counter)

| <prefix>.cypher_cache_preparser_entries
| The current number of entries in the preParser cache. (gauge)

| <prefix>.cypher_cache_preparser_hits_total
| The total number of preParser cache hits. (counter)

| <prefix>.cypher_cache_preparser_misses_total
| The total number of preParser cache misses. (counter)

| <prefix>.cypher_cache_preparser_stale_entries_total
| The total number of times a query in the preParser cache was replanned because its existing plan was considered stale (due to statistics divergence and/or elapsed time).
Related to `replan_events` above.
This is a counter of such events, not the current cache size. (counter)
|===
...
|The total number of internally generated IDs for the different property names stored in the database. These IDs do not reflect changes in the actual data. The metric does not report values for databases in block format and always reports -1 instead. Informational, not an indication of any issue. (gauge)
...
|<prefix>.page_cache.pages_snapshot_loaded|label:new[Introduced in 2026.02] The total number of old page snapshot loads happened in the page cache. (counter)
|<prefix>.page_cache.async_submit|label:new[Introduced in 2026.02] The total number of page async io submit happened in the page cache. (counter)
|<prefix>.page_cache.async_complete|label:new[Introduced in 2026.02] The total number of page async io complete happened in the page cache. (counter)
|<prefix>.page_cache.async_failure|label:new[Introduced in 2026.02] The total number of page async io failure happened in the page cache. (counter)
|===
...
|<prefix>.transaction.rollbacks_deadlocked|label:new[Introduced in 2026.01] The total number of deadlocked rolled back transactions. (counter)
...
|<prefix>.cluster.raft.raft_log_entry_prefetch_buffer.async_put|Raft Log Entry Prefetch buffer async puts. Number of entries that could not be added at append time and were loaded into the buffer later by reading from the raft log on disk. (gauge)
|<prefix>.cluster.raft.raft_log_entry_prefetch_buffer.sync_put|Raft Log Entry Prefetch buffer sync puts. Number of entries placed in the buffer directly at append time, without a disk read. (gauge)
