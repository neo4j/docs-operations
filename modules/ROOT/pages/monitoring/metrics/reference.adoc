...
In particular, the `neo4j.database.mydb.transaction.started` metric has a default user-configured-prefix `neo4j.database` and is a metric for the `mydb` database.
...
|===
|Name |Description
|<prefix>neo4j.pool.<pool>.used_heap|Used or reserved heap memory in bytes. (gauge)
|<prefix>neo4j.pool.<pool>.used_native|Used or reserved native memory in bytes. (gauge)
|<prefix>neo4j.pool.<pool>.total_used|Sum total used heap and native memory in bytes. (gauge)
|<prefix>neo4j.pool.<pool>.total_size|Sum total size of capacity of the heap and/or native memory pool. (gauge)
|<prefix>neo4j.pool.<pool>.free|Available unused memory in the pool, in bytes. (gauge)
|===
