...
[WARNING]
====
This does not obfuscate potential literals or other sensitive information when logging errors.
...
| `false`
a|
label:new[Introduced in 2026.01]
If `true`, obfuscates all error information that can contain sensitive information before writing it to the log.
This applies to error messages if the query log has pattern layout ands the fields `failureReason` and `statusDescription` if the query log has xref:#_use_json_format_for_the_query_log[JSON layout].

It is recommended to set this setting to `true` in production.

[WARNING]
====
This does not obfuscate literals when logging Cypher queries.
