...
| label:new[Introduced in 2025.01] Logs exceptions that occur during the execution of the `neo4j-admin` commands in a separate log file. For example, _neo4j-admin-exception-trace-2025-01-31.19.31.25.log_.
|
|

| _dbname-yyyy-MM-dd.HH.mm.ss/import.log_ footnote:[See xref:import.adoc#import-logs[Import progress reporting] for details about the updated way of neo4j-admin import logging and reporting issues.]
...
label:changed[Changed in 2026.03] The import log name _neo4j-admin-import-yyyy-MM-dd.HH.mm.ss.log_  has been changed to _dbname-yyyy-MM-dd.HH.mm.ss/import.log_, as the `neo4j-admin database import` command creates a new directory from version 2026.03.
The path to the directory is changed from `server/logs` to `server/data/imports/`. +
label:changed[Changed in 2026.04] The path is changed back to `server/logs/`.
|
|
|===
...
[NOTE]
====
Starting from Neo4j 2025.01, the default configuration files are located in the _server.directories.configuration=conf_ directory.
...
For more information, see https://logging.apache.org/log4j/2.x/manual/appenders.html[Log4j official documentation on RollingRandomAccessFile Appender].
...
For details, see the https://logging.apache.org/log4j/2.x/manual/configuration.html#appenderrefs-elements-filters[Log4j official documentation].
...
| `false`
a|
If `true`, obfuscates all query literals before writing the query to the log.
...
[WARNING]
====
The setting does not obfuscate literals in parameters. +
...
The setting does not obfuscate any sensitive information when logging errors. +
To get that capability, set xref:configuration/configuration-settings.adoc#config_db.logs.query.obfuscate_errors[`db.logs.query.obfuscate_errors=true`].
====


//New in 2026.01
| label:new[Introduced in 2026.01] xref:configuration/configuration-settings.adoc#config_db.logs.query.obfuscate_errors[`db.logs.query.obfuscate_errors`]
| `false`
a|
...
[WARNING]
====
The setting does not obfuscate literals when logging Cypher queries.
To get that capability, set xref:configuration/configuration-settings.adoc#config_db.logs.query.obfuscate_literals[`db.logs.query.obfuscate_literals=true`].
====
...
The following query is run against the Movies dataset in Neo4j Browser using Neo4j 2026.05 Enterprise Edition:
...
The corresponding query log in _query.log_ is:

[source, plain format, role="noheader"]
----
2026-06-22 13:45:54.597+0000 INFO  id:34 - transaction id:6 - 195 ms: (planning: 185, waiting: 0) - 328 B - 68 page hits, 0 page faults - bolt-session  bolt    neo4j-javascript/6.0.1      client/127.0.0.1:53864  server/127.0.0.1:7687>  neo4j - neo4j - MATCH (n:Person {name:'Tom Hanks'})-[:ACTED_IN]->(n1:Movie)<-[:DIRECTED]-(n2:Person {name:"Tom Hanks"}) RETURN n1.title - {} - runtime=pipelined - {app: 'query', container_app: 'workspace', type: 'user-direct', vendor: 'neo4j'}
----
...
id:34::
The internal connection ID assigned by Neo4j to the client connection.
Every new Bolt connection (from Neo4j Browser, a driver, Bloom, etc.) gets a unique connection ID for the lifetime of the server process.

transaction id:6::
The internal transaction ID assigned to that specific operation.
...
In the example, this contributed 185 ms to the total execution time of 195 ms.
...
In the example, this contributed `0` to the total execution time of 195ms. +
...
In this case, the page cache was hit 68 times.
...
In this case, query results were returned entirely from the 68 page cache hits mentioned above, so there were 0 hits on the disk required.
...
`neo4j-javascript/6.0.1`::
The Driver version.

`client/127.0.0.1:53864`::
...
The parenthesis `{}`::
`{}` is for the query parameters.

`runtime=pipelined`::
Type of runtime.

{app: 'query', container_app: 'workspace', type: 'user-direct', vendor: 'neo4j'}::
Transaction metadata:
+
* `app: 'query'` - application name.
* `container_app: 'workspace'` - executing app was embedded in workspace (otherwise standalone).
* `type: 'user-direct'` - a query a user directly submitted to/through the application.
* `vendor: 'neo4j'` - application provider.
...
You can attach metadata to a transaction and have it printed in the query log using the built-in procedure xref:procedures/built-in-procedures.adoc#procedure_tx_setMetaData[`tx.setMetaData`].
...
You can also use the procedure xref:procedures/built-in-procedures.adoc#procedure_tx_getMetaData[`CALL tx.getMetaData()`] to show the metadata of the current transaction.
