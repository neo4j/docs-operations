...
| authenticatedUser footnote:[When SSO is enabled, the username is determined either by the xref:configuration/configuration-settings.adoc#config_dbms.security.oidc.-provider-.claims.username[`dbms.security.oidc.<provider>.claims.username`] setting or, if local user mapping is required (`dbms.security.require_local_user=true`), by the corresponding mapped local username .
