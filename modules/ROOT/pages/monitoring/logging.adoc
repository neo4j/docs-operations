...
//New in 5.26.21
| label:new[Introduced in 5.26.21] xref:configuration/configuration-settings.adoc#config_db.logs.query.obfuscate_errors[`db.logs.query.obfuscate_errors`]
| `false`
a|
...
The following sample query is run on the Movies database in Neo4j Browser using Neo4j 5.26.29 Enterprise Edition:
...
[source, plain format, role="noheader"]
----
2026-09-15 08:07:52.460+0000 INFO  transaction id:6 - 150 ms: (planning: 143, waiting: 0) - 328 B - 87 page hits, 0 page faults - bolt-session  bolt    neo4j-query-api/v2      clientlocal:E:c22df64b  serverlocal:461dc289-6dae-4cb2-87c4-f39beb06234f>   neo4j - neo4j - MATCH (n:Person {name:'Tom Hanks'})-[:ACTED_IN]->(n1:Movie)<-[:DIRECTED]-(n2:Person {name:"Tom Hanks"}) RETURN n1.title - {} - runtime=pipelined - {}
----
...
It is the total of planning time + waiting + any other processing time, e.g., taken to acquire execution threads.
...
In the example, this contributed 143ms to the total execution time of 150ms.
...
In the example, this contributed 0ms to the total execution time of 150ms. +
...
In this case, the page cache was hit 87 times.

`0 page faults`::
Page fault means that the query result data was not in the page cache and, therefore, had to be fetched from the file system.
In this case, query results were returned entirely from the 87 page cache hits mentioned above, so there were 0 hits on the disk required.
...
`neo4j-query-api/v2`::
Client/driver indentifier.
In this case it indicates that the request originated through Neo4j's Query API v2 client interface.

`clientlocal:E:c22df64b`::
The query client outbound `IP:port` used.

`serverlocal:461dc289-6dae-4cb2-87c4-f39beb06234f>`::
Server connection identifier — identifies the local Neo4j server handling the query.
...
`runtime=pipelined`::
Type of the Cypher runtime.

The last parenthesis `{}`::
`{}` is for transaction metadata.
...
Starting with Neo4j 5.24, for both OIDC and LDAP, if local user mapping is required (`dbms.security.require_local_user=true`), the log records the mapped local username instead of the external identity string.
