...
This applies to error messages when the query log uses pattern layout or <<#_json_format,JSON layout>> with the `failureReason` field.
...
To get that capability, set xref:configuration/configuration-settings.adoc#config_dbms.logs.query.obfuscate_literals[`dbms.logs.query.obfuscate_literals=true`].
====
