...
| _debug.log_
| Logs information that is useful when debugging problems with Neo4j. It is highly recommended to keep it enabled.
...
Appenders write to _debug.log_, _http.log_, _query.log_, and _security.log_.

<3> By default, Neo4j uses the `<RollingRandomAccessFile>` appender as it shows a performance increase between 20-200% as per the Log4j documentation.
However, all writes are buffered, which means that if the server crashes, the last log messages might be lost.
...
In version 4.0, this was configured with the parameter `dbms.logs.user.rotation.size`.
...
In version 4.0, this was configured with the parameter `dbms.logs.user.rotation.keep_number`.
...
== Possible logging customizations

The default logging configuration is a good starting point, but you might want to customize it to your needs.
The following sections describe the Log4j configuration elements and how to use them to customize the logging configuration.
