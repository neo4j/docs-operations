...
To estimate a vector index's filesystem cache requirements, you need to understand how Lucene stores the index.
...
Because quantization reduces search accuracy, especially with binary quantization, binary-quantized search results are by default rescored using the full-precision vector values from the `.vec` files.
...
Quantized indexes do not include the full-precision `.vec` files used to rescore candidates.
...
. Start by calculating the size of the full-precision vector values using the formula:
+
...
+
* for a full-precision, unquantized index: 3.072 GB + 0.128 GB = 3.2 GB
* for a scalar-quantized index: 768 MB + 128 MB = 896 MB
* for a binary-quantized index with rescoring: 96 MB + 128 MB = 224 MB
