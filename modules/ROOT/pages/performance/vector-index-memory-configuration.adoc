...
Lucene does not use Neo4j page cache memory; it relies on OS-managed memory as described in the xref:performance/memory-configuration.adoc[Memory configuration] section.
When you have a vector index, you must ensure that there is sufficient memory for Neo4j (JVM heap and Neo4j page cache) and for the OS-managed memory Lucene relies on.
If Lucene has insufficient OS-managed memory available, the OS will read data from disk and may start swapping, which will degrade vector search performance.
