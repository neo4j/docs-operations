...
For Lucene-backed vector indexes, the dimension precision is `4 bytes`.
...
The estimate depends on whether quantization is enabled (default `true`, see link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/semantic-indexes/vector-indexes/#config-vector.quantization.enabled[`vector.quantization.enabled`]).
