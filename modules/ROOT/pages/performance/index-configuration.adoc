...
For more information on the different index types, refer to link:{neo4j-docs-base-uri}/cypher-manual/{page-version}es-for-search-performance[Cypher Manual -> Indexes for search performance] and link:{neo4j-docs-base-uri}/cypher-manual/{page-version}es-for-full-text-search[Cypher Manual -> Indexes to support full-text search].
...
+
For details on indexes, see link:{neo4j-docs-base-uri}/cypher-manual/{page-version}es-for-search-performance[Cypher Manual -> Indexes for search performance].
...
+
For more information, see link:{neo4j-docs-base-uri}/cypher-manual/{page-version}es-for-search-performance[Cypher Manual -> Indexes for search performance] and link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/constraints[Cypher manual -> Constraints].
...
Indexes and constraints are best created through link:{neo4j-docs-base-uri}/cypher-manual/{page-version}es-for-search-performance[Cypher] but can still be created through the deprecated procedures described in the example below.
