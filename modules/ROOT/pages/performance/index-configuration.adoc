...
If a transaction reaches the key size limit for one or more of its changes, that transaction fails before <<commit, committing>> any changes.
...
For a complete list of Neo4j procedures, see xref:procedures/built-in-procedures.adoc#_index_management[Built-in procedures].
