...
+
Indexes are usually fully consistent, and the committing of a transaction returns once both the store and the indexes are updated.
...
By default, databases created before 4.3 get only a node label lookup index when used in Neo4j 4.3 or later.
...
When used in a Neo4j 4.3 or later, databases created before 4.3 automatically get a node label lookup index.
This index is created by converting the former label scan store and naming it `__org_neo4j_schema_index_label_scan_store_converted_to_token_index`.
