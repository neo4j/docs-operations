...
However, in a xref:clustering/introduction.adoc[Cluster] deployment, regardless of instance type, the maximum amount of memory each transaction is permitted to use is 2GB.
Additionally, if your configuration contains xref:reference/configuration-settings.adoc#config_dbms.clustering.enable[`dbms.clustering.enable=true`],
xref:reference/configuration-settings.adoc#config_dbms.mode[`dbms.mode=CORE`], or xref:reference/configuration-settings.adoc#config_dbms.mode[`dbms.mode=READ_REPLICA`], the largest value permitted for the xref:reference/configuration-settings.adoc#config_dbms.memory.transaction.max_size[`dbms.memory.transaction.max_size`] setting is also 2GB.
