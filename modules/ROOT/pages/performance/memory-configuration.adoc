...
However, in a xref:clustering/introduction.adoc[Causal Cluster] deployment, where an instance is configured as either a xref:clustering/introduction.adoc#causal-clustering-core-servers[Core] or a xref:clustering/introduction.adoc#causal-clustering-read-replicas[Read Replica], the maximum amount of memory each transaction is permitted to use is 2GB. 
Consequently, if your configuration contains `dbms.mode=CORE` or `dbms.mode=READ_REPLICA`, the largest value permitted for the `dbms.memory.transaction.max_size` setting is also 2GB.
