...
Marking a record as deleted requires writing a record update command to the xref:data-integrity.adoc#transaction-log-files[transaction log files], as when something is created or updated.
...
Transactions are eventually pruned out of the xref:data-integrity.adoc#transaction-log-files[transaction log], bringing the storage usage of the log back down to the expected level.
