...
To reclaim space after a large delete or a retention purge, checkpoint before and after the compaction, so it runs against a durable state and the reclaimed space is captured on disk immediately.
...
. Checkpoint once more, then check the store size a final time:
+
[source,cypher]
----
CALL db.checkpoint();
