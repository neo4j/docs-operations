...
[NOTE]
====
All of the above commands are executed as Cypher commands, and the database name is subject to the link:/docs/cypher-manual/4.4/syntax/naming[standard Cypher restrictions on valid identifiers].
...
For more information, see link:/docs/cypher-manual/4.4/aliases#alias-management-create-database-alias[Cypher Manual -> Creating database aliases]

For detailed information on Cypher administrative commands, see link:/docs/cypher-manual/4.4/access-control[Cypher Manual -> Access Control].
...
[NOTE]
====
Another way of preventing writes is to set the database access to read-only using the link:/docs/cypher-manual/4.4/databases#administration-databases-alter-database[ALTER DATABASE] command.
====
