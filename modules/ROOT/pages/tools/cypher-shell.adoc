...
Use `:param <Cypher Map>` to set parameters, `:param` to list current parameters and `:param clear` to clear all parameters.
...
For example, `{now: datetime()}` will set parameter `now` to the current date and time at the time of setting the parameter.
