...
Instead, you will need to add these afterwards (see link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/administrationes-for-full-text-search[Cypher Manual -> Indexes]).
====

If you wish to see in-depth examples of using the command `neo4j-admin import`, refer to the xref:tutorial/neo4j-admin-import.adoc[Tutorials -> Neo4j Admin import].
...
+
For horizontal tabulation (HT) use `\t` or the unicode character encoding `\9`.
+
