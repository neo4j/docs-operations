...
If you cannot afford a full downtime of your database, you can split the operation into several stages:

* _prepare_ stage
* _build_ stage
* _merge_ stage

The database must be stopped for the `prepare` and `merge` stages and put into read-only mode during the `build` stage (the database may be started though).
For more details, read <<import-tool-incremental-examples, Two ways to import data incrementally>>.
...
If the _merge_ stage fails, is aborted or crashes it may corrupt the database.

Therefore, it is highly recommended to back up your database before running the incremental import.
...
=== Two ways to import data incrementally / Two modes of the incremental import
