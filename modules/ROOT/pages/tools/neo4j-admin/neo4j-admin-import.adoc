...
The `neo4j-admin database import incremental` command imports a large amount of data from CSV files into an existing database in increments.
This is valuable when the data being imported is too large to stage locally, so it must be broken into smaller pieces and brought across one at a time, or for loading additional data into an immutable store.
...
* Add new properties in existing nodes or relationships.
* Update properties in nodes or relationships.
* Delete properties in nodes or relationships.
* Delete existing relationships.
* Delete existing nodes/relationships.
...
You can do an incremental import offline (on a stopped database) in a single command by setting the option `--stage` to `all` or in a semi-online mode by first setting the option `--stage` to `prepare` (offline), then `build` (read-only mode), and finally `merge` (offline).
...
. The `build` stage:
+
While the database must be stopped to run `--stage=prepare`, you can run `--stage=build` on a started database in read-only mode.
+
[source, shell, role=noplay]
----
