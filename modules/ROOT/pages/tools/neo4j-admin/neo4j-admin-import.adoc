...
|--quote=<char> footnote:ingnoredByParquet1[] footnote:[To escape quotation marks in the CSV data, you should double the configured character.]
|Character to treat a quotation mark for values in CSV data.

For example, quotes can be escaped as per link:{rfc-4180}[RFC 4180] by doubling them. Thus `""` would be interpreted as a literal `"`.
...
|--quote=<char>footnote:ingnoredByParquet2[] footnote:[To escape quotation marks in the CSV data, you should double the configured character.]
