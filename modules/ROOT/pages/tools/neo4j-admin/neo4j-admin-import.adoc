...
* If you cannot afford a full downtime of your database, you can run the import in three stages:
  ** _Preparing_ stage:
     During this stage, the import tool prepares the new graph data and indexes by caching them in memory.
  ...
  ** _Building_ stage:
     ...
     This is the longest stage and you can put the database in read-only mode to allow reads.
  ...
  ** _Merging_ stage:
     ...
     It also updates the affected indexes but upholds the affected uniqueness constraints and property existence constraints.
