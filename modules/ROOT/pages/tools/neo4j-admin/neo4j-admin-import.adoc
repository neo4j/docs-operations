...
ID::
  ...
  Note that the ID has to be unique across all nodes in the group; even for nodes with different labels.
  ...
  A node header can contain multiple `ID` columns, where the relationship data then references the composite value of all those columns. This also implies using `string` as `id-type`. Each `ID` column can specify to store its values as different node properties, however the composite value cannot be stored as a node property.
LABEL::
