...
|When `true`, non-array property values are converted to their equivalent Cypher types. For example, all integer values will be converted to 64-bit long integers. This takes up more space on disk but avoids Cypher converting the type during queries.
|true
...
|--normalize-types[=true\|false]
|When true, non-array property values will be converted to their equivalent Cypher types. For example, all integer values will be converted to 64-bit long integers. This takes up more space on disk, but avoids Cypher converting the type during queries.
...
By default, types (except arrays) will be converted to Cypher types [https://neo4j.com/docs/cypher-manual/current/values-and-types/property-structural-constructed/#_property_types].
...
Arrays are affected by the `--normalize-types` option. If you want a byte array to be stored as a Cypher long array you must explicitly declare the property as `long[]`.
