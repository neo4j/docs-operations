...
[NOTE]
.Using both an option with variable arity and a positional parameter
====
When using the option `--additional-config`, for example, `--additional-config neo4j.properties --nodes 0-nodes.csv mydatabase`, the `--nodes` option will behave as greedy and the next option, in this case, `mydatabase`,  will be pulled in via the nodes convertor.
+
...
Therefore, either put the positional parameters first, e.g `mydatabase --nodes 0-nodes.csv`, or put the positional parameters last, after `--`, and after the final value of the last multi-value option, e.g `nodes 0-nodes.csv -- mydatabase`.
====
