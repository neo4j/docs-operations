...
For Unix based systems Neo4j supports tab completion for the `neo4j` and `neo4j-admin` command line interfaces in terminals such as Bash and ZSH. For RPM and DEB packaged installations the neccesary files will automatically be installed in `bash-completion` and for tarball installations the files are located in the `bin/completion/` directory with detailed instuctions for manual installation.
