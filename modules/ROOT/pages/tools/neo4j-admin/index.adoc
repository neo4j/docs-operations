...
When `neo4j` and `neo4j-admin` finish as expected, they return exit code `0`.
...
The non-zero exit code can contain further information about the error, for an examples, see the `backup` command's xref:backup-restore/online-backup.adoc#backup-command-exit-codes[exit codes].

== Command-line completion

From v5.4 onwards, Neo4j supports command-line completion.
