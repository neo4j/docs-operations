...
The `neo4j-admin push-to-cloud` command uploads a local Neo4j database or a dump into a Neo4j Aura instance.
It loads the database or the database dump into your local installation of Neo4j {neo4j-version} and then uploads it into Neo4j Aura v4 or v5 depending on the source and destination compatibility.

.Compatibility between the different source (on-premise) and destination (cloud) versions of Neo4j:
[options="header" cols="50%, 50%"]
|===
| Neo4j version (source)   
| Aura version (destination)
