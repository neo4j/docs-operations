...
When starting the import, a new directory is created in the form of `server/data/imports/dbname-yyyy-MM-dd.HH.mm.ss/import.log`.
...
If the `--profile-results-path` option is provided, then this log is used instead for Java flight records.
...
The detailed progress of reporting is in `progress.json.log` and is a JSON doc per line for each snapshot.

Violations are reported as JSON entries, one per line, in `report.json.log`.
