...
See <<import-logs, Import progress reporting>> for more details.
...
You can use the `--schema` option to provide a file with Cypher commands to create indexes and constraints during the import process.
See <<indexes-constraints-import, Provide indexes and constraints during import>> for more information.

For other formats, you have to add indexes and constraints manually after the import process.
See link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/semantic-indexes/full-text-indexes/[Cypher Manual -> Indexes] for more information.
...
Starting with Neo4j 2026.08, the `jdk.incubator.vector` module is enabled by default.
See xref:changes-2025-2026.adoc#vector-api-changes[Changes in Neo4j 2025-2026] for details.
...
The xref:import.adoc#import-tool-examples[examples] for CSV can also be used with Parquet.
...
|--dry-run[=true\|false]footnote:[See <<import-full-dry-run>> for details.]
...
|--multiline-fields=true\|false\|<path>[,<path>] footnote:[The option's value depends on `--multiline-fields-format`. For details, see <<#_importing_data_that_spans_multiple_lines,Importing data that spans multiple lines>>.]
...
|--path-pattern-style=regex\|glob\|nonefootnote:[See <<import-tool-path-pattern-style>> for details.]
...
Assume that you have formatted your data as per xref:import.adoc#import-tool-header-format[CSV header format] so that you have it in six different files:
...
[[import-tool-path-pattern-style]]
