...
[NOTE]
====
The ability to specify indexes and constraints is not supported by `aligned` format.
====
...
Starting with 2026.05, the `--schema` option allows you specify a graph type, using a `ALTER CURRENT GRAPH TYPE` statement instead of individual constraints.
// TODO: (For the full importer only SET is supported)
...
You have to specify your constraints either as a single graph type statement, or as multiple individual constraint statements.

See link:{neo4j-docs-base-uri}/cypher-manual/current/schema/graph-types/[CypherManual -> `Graph types (Preview)`] for more information on graph types.

The import tool supports the following index types:
...
// TODO: Verify the official constraint names.
The import tool supports the following constraint types in addition to setting graph type:
...
The following constraint types are supported via graph types:
...
[NOTE]
====
Full import does not support dropping indexes and constraints. For graph type it supports SET operations.
====
...
This file uses ';' as a separator.
Note: only SET is supported by the full importer.
...
Note that you cannot drop indexes that are backing constraints.
You must drop the constraints themselves, which will also drop the underlying indexes.
...
// ----
// DROP INDEX PersonNameIndex IF EXISTS;
// CYPHER 25 ALTER CURRENT GRAPH TYPE DROP {
//     (p: person => {age :: INTEGER NOT NULL)
// };
// CYPHER 25 ALTER CURRENT GRAPH TYPE ADD {
//     CONSTRAINT FOR (p:Person) REQUIRE p.name IS UNIQUE
// };
// ----
// This file uses ';' as a separator.
// Note: Only ADD, DROP, ALTER is supported by the incremental importer.
//
// ======
// =====
