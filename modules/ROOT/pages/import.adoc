...
|--property-shard-count=<propertyShardCount>footnote:shardedDb1[See xref:scalability/sharded-property-databases/data-ingestion.adoc[Property Sharding -> Data ingestion] for details.]
...
|--property-shard-count=<propertyShardCount>footnote:shardedDb2[See xref:scalability/sharded-property-databases/data-ingestion.adoc[Property Sharding -> Data ingestion] for details.]
...
Each JSON entry includes a `message` field equivalent to the current output, along with additional fields that capture more detailed information.
