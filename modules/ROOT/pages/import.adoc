...
|Configuration file with additional configuration. See xref:neo4j-admin-neo4j-cli.adoc#_configuration[Neo4j Admin and Neo4j CLI -> Configuration] for details.
|
| {check-mark}
| {check-mark}
...
|--property-shard-count=<numSpdPropertyDataShards>
|label:new[Introduced in 2025.12]label:enterprise[Enterprise edition] label:spd[Sharded property databases] Number of shards of property data that will be created, each shard will be its own database. When this value is '0', it is the equivalent of running just a 'FULL' import. See xref:scalability/sharded-property-databases/data-ingestion.adoc[Property Sharding -> Data ingestion] for details.
|0
| {check-mark}
|
...
|label:new[Introduced in 2025.12]label:enterprise[Enterprise edition] label:spd[Sharded property databases] Target format can be either plain database directory/files structure or a backup artifact. Uses the `--temp-path` location to keep any intermediate state. See xref:scalability/sharded-property-databases/data-ingestion.adoc[Property Sharding -> Data ingestion] for details.
|database
| {check-mark}
|

|--target-location=<path>
|label:new[Introduced in 2025.12]label:enterprise[Enterprise edition] label:spd[Sharded property databases] Location for target backup artifact data. Used together with `--target-format=backup`. See xref:scalability/sharded-property-databases/data-ingestion.adoc[Property Sharding -> Data ingestion] for details.
|
| {check-mark}
|
...
|--property-shard-count=<numSpdPropertyDataShards>
|label:new[Introduced in 2025.12] label:enterprise[Enterprise edition] label:spd[Sharded property databases] (advanced) Number of shards of property data that will be created, each shard will be its own database. Typically this option doesn't need to be set at all, since the number of shards has already been decided during initial import. See xref:scalability/sharded-property-databases/data-ingestion.adoc[Property Sharding -> Data ingestion] for details.
|0
| {check-mark}
|
