...
For CSV data, this value must be different from the one specified in the `--delimiter` option.For Parquet data, this is only needed if the array is encoded as a string.
...
For CSV data, this value must be different from the one specified in the `--delimiter` option.For Parquet data, this is only needed if the vector is encoded as a string.
|;
| {check-mark}
| {check-mark}
...
Files containing node data must have an `ID` field, and can have a `LABEL` field, and properties.
