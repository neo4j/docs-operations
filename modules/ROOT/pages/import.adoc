...
Starting with 2026.03, the `neo4j-admin database import` command has a context directory used to capture information about the import process.

When starting the import, a new directory is created in the form of `server/data/imports/dbname-yyyy-MM-dd.HH.mm.ss`.

This contains the verbose logging, the detailed progress, the bad collector information (see the `--bad-tolerance` option), and profiling (Java flight recording) if enabled.
