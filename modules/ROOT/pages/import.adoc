...
|--quote=<char> footnote:[Escaping the double quote characters (`"`) in the CSV data by doubling them is supported only when the `--quote` option is set to `"` (the default value).]
|Character to treat as a quotation mark for values in CSV data.
