...
There are different methods of importing data into Neo4j depending on your dataset size, as well as whether you are using a Neo4j self-managed or a Neo4j Aura product.
Some of these methods are:

* link:https://neo4j.com/docs/data-importer/current/[Data Importer] -- a graphical tool for importing data into Neo4j.
It is ideal to get started quickly with testing and prototyping.
Currently, it is available in AuraDB Free and AuraDB Professional and as a standalone product.
* link:{neo4j-docs-base-uri}/cypher-manual/current/clauses/load-csv/[`LOAD CSV`] -- a Cypher command for loading CSV data into Neo4j.
It supports small- to medium-sized datasets (up to 10 million records) from local and remote files, including from cloud URIs, and data from relational databases.
* `neo4j-admin database import` command -- the most efficient way to import large CSV files into Neo4j, either with a full import or in a series of smaller batches (incremental).

This page describes only the `neo4j-admin database import` command.
For information on the other methods, see the links above.
...
=== When to use `neo4j-admin database import`

You should use `neo4j-admin database import` when:
