...
As of 2026.05, the `--schema` option supports adding a Graph type,
using the `ALTER CURRENT GRAPH TYPE` statement. See
link:{neo4j-docs-base-uri}/cypher-manual/current/schema/graph-types/[CypherManual -> `Graph types (Preview)`].
For full imports (`neo4j-admin database import full`), the operation SET is available (`ALTER CURRENT GRAPH TYPE SET {...}`).
For the incremental importer, the operations ADD and DROP are available (`ALTER CURRENT GRAPH TYPE ADD/DROP {...}`.
