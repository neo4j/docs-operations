...
=== Access Neo4j using a Kubernetes Ingress

The Neo4j Helm charts provide a helm chart that allows you to use a Kubernetes Ingress to access Neo4j running on port _:80_ or _:443_.
The helm chart is called _neo4j/neo4j-reverse-proxy_ and is available on the _neo4j_ helm repository from version 5.12.0.
...
reverseProxy:
  image: "neo4j/neo4j-reverse-proxy"
...
The standalone Neo4j has a Neo4j service with the name _standalone-admin_, and it has _:7474_ an _:7687_ opened.
To verify that, run `get all, pvc, pv, configmaps, secrets`.

. You need to have an Ingress controller for the Kubernetes Ingress to work.
You can use the following command to install the Nginx Ingress controller:
+
[source,shell]
----
...
. If your Neo4j is running on _:443_, then set the `tls.enabled` to `true` and create a secret with the following command:
.. Create Ingress certificates:
+
[source,shell]
----
...
----
.. Create Kubernetes secret using the Ingress certificates:
+
[source,shell]
----
...
----
reverseProxy:
    image: neo4j/neo4j-reverse-proxy:5.12.0
...
. If your Neo4j is running on _:80_, then set the `tls.enabled` to `false` and create the _ingress.yaml_ file with the following content:
+
[source, yaml]
----
reverseProxy:
            ...
            config:
                - secretName: ingress-cert
                  hosts:
                    - localhost
----
. Install the reverse proxy Helm chart using the following command:
+
[source,shell]
----
helm install rp neo4j/neo4j-reverse-proxy -f /path/to/your/ingress.yaml
----
. Open the Neo4j browser on _https://localhost:443_ or _http://localhost:80_ and log in with your credentials.

==== Access Neo4j using Cypher Shell

Alternatively, if you want to use Cypher Shell to access your data, you need to create a `configmap`, because Cypher Shell expects a TCP connection and Ingress is an HTTP connection.
