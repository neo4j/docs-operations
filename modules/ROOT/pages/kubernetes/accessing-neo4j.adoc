...
=== Access Neo4j on port `:80` or `:443`
...
For Neo4j clusters, the Neo4j headless service is used to route the traffic to the cluster instances.
...
The reverse proxy creates an HTTP server, which routes requests to either the Bolt reverse proxy or HTTP reverse proxy based on the request headers.
...
The following steps use the Nginx Ingress controller.
...
.. If you want to access Neo4j on port `:443`, create a Kubernetes secret containing the Ingress self-signed certificates and then create the _ingress.yaml_ file.
