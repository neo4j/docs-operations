...
=== Access Neo4j using Kubernetes Ingress

The Neo4j Helm charts provide a Helm chart that allows you to use a Kubernetes Ingress to access Neo4j on port `:80` or `:443`.
The Helm chart is called _neo4j/helm-charts-reverse-proxy_ and is available on the Neo4j Helm repository from version 5.12.0.
...
[source, shell]
--
helm show values neo4j/helm-charts-reverse-proxy
--
...
[source,shell]
----
helm install rp neo4j/helm-charts-reverse-proxy -f /path/to/your/ingress.yaml
----
