...
To avoid this, either use a separate container or `initContainer` or place the Neo4j Helm deployment in xref:kubernetes/maintenance.adoc#offline-maintenance[offline maintenance mode] to run `neo4j-admin database import`.
