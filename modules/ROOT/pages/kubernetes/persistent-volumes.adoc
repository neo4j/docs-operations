...
From version 4.4.21, you can also set a label on creation for the volumes with mode `dynamic`, `defaultStorageClass`, `selector`, and `volumeClaimTemplate`, which can be used to filter the PVs that are used for the volume mount.
...
A full example can be found in the https://github.com/neo4j/helm-charts/tree/4.4/[Neo4j GitHub repository]
