...
To make the data persistent, you can use a persistent volume.
You can also use a persistent volume claim (PVC) to provision a persistent volume dynamically.
+
For example, you can add the following to the _backup-values.yaml_ file to use a PVC for your _/data and _/backups_ mount:
+
[source, yaml, role='noheader']
----
volumes:
...
----
+
For more information, see xref:kubernetes/persistent-volumes.adoc#static-pv-config-helm[Provision a PVC for Neo4j Storage].
...
----
+
The _neo4j/neo4j-admin_ Helm chart installs a cronjob that launches a pod based on the job schedule. This pod performs a backup of the database and uploads it to the cloud provider bucket.
It also runs a cronjob that performs a consistency check of the backup file and uploads the report to the cloud provider bucket.
