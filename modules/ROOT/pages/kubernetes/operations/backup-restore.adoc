...
#The service account must already exist and have the necessary permissions to access and manage your S3 bucket. See https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies.html for more information.
...
helm install backup-name neo4j-admin -f /path/to/your/backup-values.yaml
----
. Monitor the pod logs using `kubectl logs pod/<neo4j-backup-pod-name>` to check the progress of aggregate backup.
