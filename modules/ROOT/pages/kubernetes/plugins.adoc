...
APOC core is shipped with Neo4j, but it is not installed in the Neo4j _plugins_ directory.  In this example we will merely point to the location of the core apoc library in the labs directory.
...
----
+
. Under `apoc_config`, configure the APOC (i.e apoc.conf) settings that you want, for example:
+
[source, yaml]
----
apoc_config:
