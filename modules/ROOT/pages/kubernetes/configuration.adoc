...
This example shows how to configure an SSL policy for intra-cluster communication using a self-signed certificate and the Neo4j Helm charts.
...
----
NAME                  TYPE                                  DATA   AGE
default-token-rkvwg   kubernetes.io/service-account-token   3      105m
...
----
. Configure `ssl` object in the _tls-value.yaml_ file using the created secret:
+
[source, yaml]
----
ssl:
# setting per "connector" matching neo4j config
  ...
  cluster:
    ...
    revokedCerts:
      sources: [ ]
----
. Deploy a Neo4j cluster using the Neo4j Helm chart with the updated _tls-value.yaml_ file:
+
[source, shell]
----
helm install core-1 neo4j/neo4j-cluster-core --set neo4j.acceptLicenseAgreement=yes --set neo4j.password=my-password --set neo4j.edition="enterprise" --set volumes.data.mode=defaultStorageClass -f ~/Documents/neo4j-ssl/tls-values.yaml
----
...
+
* Neo4j Browser:
.. Open a web browser and type _https://lb-EXTERNAL_IP:7473_ (in this example, https://20.62.191.227:7473/browser/).
