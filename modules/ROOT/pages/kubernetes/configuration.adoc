...
can be upgraded to a cluster by adding more members. Because of this, the `neo4j.name` parameter is now mandatory and the installation will fail if it is not specified. 
`neo4j.name` must be unique within a namespace.

[[configure-neo4j-cluster-size]]
== Set neo4j.minimumClusterSize parameter
Since Neo4j 5.0.0, there is no longer a differentiation between standalone cluster servers.
...
`neo4j.minimumClusterSize` is set to 1 by default. This means the server starts without waiting for other members.
When installing a cluster, `neo4j.minimumClusterSize` should be set to the number of desired members in the cluster.
If extra cluster servers are added in excess of `neo4j.minimumClusterSize`, the servers need to be enabled manually
using the `ENABLE SERVER`.
See xref:/clustering/servers.adoc[Managing servers in a cluster] for instructions on enabling new servers.
