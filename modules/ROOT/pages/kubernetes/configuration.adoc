...
.Example of configuring an SSL policy for intra-cluster communication
====
This example shows how to configure an SSL policy for intra-cluster communication using a self-signed certificate and the Neo4j Helm chart.
The `cluster` SSL policy is used for intra-cluster communication.
The `bolt` SSL policy is used for client communication.

. Create _cert.pem_ and _key.pem_ files for the self-signed certificate:
+
[source, shell]
----
...
. Convert _cert.pem_ into _public.crt_ to be used by the Neo4j deployment:
+
[source, shell]
----
openssl x509 -outform pem -in cert.pem -out public.crt
----

. Create private.key to be used by the Neo4j deployment:
+
[source, shell]
----
...
. Save the _public.crt_ and _private.key_ into a K8s secret:

.. The k8s secret requires _public.crt_ and _private.key_ to be converted into `base64`` format using one of the following commands:
+
.On MacOS, use the `-b0` option to disable line wrapping.
[source, shell]
----
cat public.crt| base64 -b0 
----
+
.On Linux, use the `-w0` option to disable line wrapping:
[source, shell]
----
cat public.crt | base64 -w0
----
.. Create a _self-signed-cert.yaml_ file with the following content:
+
[source, yaml]
----
apiVersion: v1
...
name: neo4j-ssc
data:
  public.crt: LS0tLS1CRUdJTiBDRVJUSUZJQ0FURS0tLS0tCk1JSUZ0VENDQTxxx #base64 format
  private.key: LS0tLS1CRUdJTiBSU0EgUFJJVkFURSBLRVktLS0tLQpNSUlKS0xxxxxxx # base64 format
----
.. Apply the self-signed-cert configuration:
+
[source, shell]
----
kubectl apply -f self-signed-cert.yaml -n neo4j
----

. Configure SSL in the _value.yaml_ file:
+
[source, yaml]
----
ssl:
...
. Connect to Neo4j using `neo4j+ssc` for the self-signed certificate:
+
[source, shell]
----
cypher-shell -a neo4j+ssc://server-1.neo4j.svc.cluster.local:7687 -u neo4j
----

. Run `CALL dbms.listConfig()` to verify that the configuration is deployed as expected. 
You should see `"dbms.ssl.policy.cluster.enabled" | "true"` in the list that is returned.
====
