...
#
  ...
  operations:
      enableServer: false
      image: "neo4j/helm-charts-operations:2026.07.1"
      ...
      ssl:
        ...
        # This should match the Neo4j server's dbms.ssl.policy.bolt.verify_hostname setting
        disableHostnameVerification: false
        # Set to true to skip all TLS certificate verification (not recommended for production)
...
volumes:
  ...
  # provide a volume to use for plugins
  # n.b. plugins will be mounted to /plugins and server.directories.plugins will be set to /plugins
  # any of the volume modes shown above for data can be used for plugins
  # plugins:
  #   disableSubPathExpr: false
  #   mode: "share" # share an existing volume (e.g. the data volume)
  #   share:
...
services:
  ...
  neo4j:
    ...
    selector:
      ...
      # Selector entries are added as additional label selectors on the load balancer service.
      # The "helm.neo4j.com/neo4j.loadbalancer": "include" selector matches pods whose
      # podSpec.loadbalancer value is set to "include" (the default).
      # Additional custom selectors can be specified here to further restrict which pods
      # are included in the load balancer.
...
#  customImage: eu.gcr.io/neo4j-helm/neo4j:v5
  ...
  # New optional separated image configuration. Use either this or customImage, not both.
  # If using these, the chart will manage the tag based on the Neo4j version and edition.
  registry: ""  # e.g., "my.private.registry.com"
  repository: ""  # default to "neo4j" if empty
  tag: ""  # if empty, uses chart's default tag (AppVersion[-enterprise])
...
#      demo: alpha
  ...
  # persistentVolumeClaimRetentionPolicy fields for Neo4j statefulset.
  # Requires a Kubernetes version that supports this StatefulSet field. Kubernetes docs mark it stable in v1.32.
  # Both fields accept "Retain" (default) or "Delete".
  # Example for ephemeral/CI environments:
...
# PodDisruptionBudget for this Neo4j instance.
# NOTE: Because each cluster member is a separate Helm release with its own single-replica StatefulSet,
# this PDB only protects the single pod of THIS release.
# For cluster-wide disruption protection during worker node rolling upgrades (e.g., Kubermatic, GKE node auto-upgrades),
# create a separate PDB that spans all cluster members using the shared label "helm.neo4j.com/neo4j.name".
podDisruptionBudget:
  enabled: false
  matchLabels: {}
#    "demo": "neo4j"
...
For Kubernetes deployments on IPv6 or dual-stack clusters, set IPv6 addresses in the Helm chart _values.yaml_ file under the `config` key.
...
config:
  ...
  server.bolt.advertised_address: "[2001:db8::1]:7687"
  server.http.listen_address: "[::]:7474"
----

[NOTE]
====
The Kubernetes cluster must be configured for IPv6 single-stack or dual-stack networking before deploying Neo4j.
Refer to the https://kubernetes.io/docs/concepts/services-networking/dual-stack/[Kubernetes dual-stack documentation] for cluster-level prerequisites.
====
