...
For more information, see xref:kubernetes/helm-charts-setup/index.adoc[Configure the Neo4j Helm Chart repository].
...
----
+
. Pass the _neo4j-values.yaml_ file during installation. The `neo4j.name` parameter is mandatory and can be supplied
either in `neo4j-values.yaml` or by using the `--set` argument.
+
[source, shell]
----
...
All Neo4j cluster members are linked together by the value of the parameter `neo4j.name`.
Starting from Neo4j 5.0.0, standalone servers and cluster servers have no distinction. This means a standalone server
can be upgraded to a cluster by adding more members. Because of this, the `neo4j.name` parameter is now mandatory and the installation will fail if it is not specified. `neo4j.name` must be a unique
within a namespace.

[[configure-neo4j-cluster-size]]
== Set neo4j.minimumClusterSize parameter
Since Neo4j 5.0.0, there is no longer a differentiation between standalone, core and read replica servers.
By default, servers in a cluster can host primary or secondary databases, see the xref:clustering/introduction.adoc[clustering documentation] for more details.
`neo4j.minimumClusterSize` is set to 1 by default. This means the server will start without waiting for other members.
When installing a cluster, `neo4j.minimumClusterSize` should be set to the number of desired members in the cluster.
If extra cluster servers are added in excess `neo4j.minimumClusterSize`, the servers will need to be enabled manually
using the `ENABLE SERVER`, see xref:/clustering/servers.adoc[Managing servers in a cluster] for instructions on enabling new servers.
...
For full details of how to configure memory usage in Neo4j, see xref:memory-configuration.adoc[Performance - Memory Configuration].
