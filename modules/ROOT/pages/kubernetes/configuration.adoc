...
Instead, you can configure the helm deployment to use APOC core by upgrading the deployment with these additional setting in the _values.yaml_ file:

. Configure APOC core, by loading and unresticting the functions and procedures you need (for more details see link:{neo4j-docs-base-uri}/apoc/{page-version}/installation/#restricted[APOC installation guide]) , for example:
+
[source, yaml]
----
config:
