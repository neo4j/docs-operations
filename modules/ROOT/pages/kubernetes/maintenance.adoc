...
When using `helm install` or `helm template` with `--dry-run`, you must disable `nodeSelector` by either adding the following to the _values.yaml_ file:
...
or passing it as `--set disableLookups=true`, when running `helm template` or `helm install` with `--dry-run` flag:
