...
[TIP]
====
For information about the command syntax, options, and usage, see xref:backup-restore/offline-backup/index.adoc[Back up an offline database].
====
...
[TIP]
====
For information about the command syntax, options, and usage, see xref:backup-restore/restore-dump/index.adoc[Restore a database dump].
====
+
. <<put-online-mode>>.
. Verify that Neo4j is working by refreshing Neo4j Browser.
...
For more information, see xref:kubernetes/quickstart-cluster/access-inside-k8s/index.adoc[Access the Neo4j cluster from inside Kubernetes]
and xref:kubernetes/quickstart-cluster/access-outside-k8s/index.adoc[Access the Neo4j cluster from outside Kubernetes].
====
...
For information about the command syntax, options, and usage, see xref:backup-restore/restore-backup/index.adoc[Restore a database backup].
...
For more information, see xref:backup-restore/restore-backup/index.adoc#restore-backup-example[Restore a database backup -> Example].
====

[NOTE]
====
To restore the `system` database, follow the steps described in xref:kubernetes/maintenance/index.adoc#kubernetes-neo4j-dump-load[Dump and load databases (offline)].
====
...
== Migrate Neo4j from the Labs Helm chart to the Neo4j Helm chart (offline)

To migrate your Neo4j deployment from the Labs Helm chart to the Neo4j Helm chart, back up your standalone instance or cluster created with the Labs Helm chart and restore it in a standalone instance or a cluster created using the Neo4j Helm chart.
...
See xref:kubernetes/configuration/index.adoc#configure-resources[Configure Resource Allocation] for more details.
====
...
For more information, see  xref:kubernetes/quickstart-cluster/create-value-file/index.adoc[Create Helm deployment values files] and xref:kubernetes/quickstart-cluster/install-servers/index.adoc[Install Neo4j cluster servers].
+
