...
This is possible because the second query uses a link:{neo4j-docs-base-uri}/cypher-manual/current/patterns/variable-length-patterns/#quantified-relationships[quantified relationship] (`-[]\->+`), which enables batching relationships into sets and then fetching their properties from the different shards, and this is not possible when using the old link:{neo4j-docs-base-uri}/cypher-manual/current/patterns/reference/#variable-length-relationships[variable-length relationship syntax] (`-[*1..]\->`).
