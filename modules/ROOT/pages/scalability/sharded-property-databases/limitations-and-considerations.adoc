...
xref:procedures/built-in-procedures.adoc#procedure_db_index_fulltext_awaitEventuallyConsistentIndexRefresh[`db.index.fulltext.awaitEventuallyConsistentIndexRefresh()`] is not supported in sharded property databases versions 2025.12 and 2026.01.

[CAUTION]
====
It is strongly recommended not to use xref:procedures/built-in-procedures.adoc#procedure_dbms_setConfigValue[`dbms.setConfigValue()`] on sharded property databases, as sharded property databases run in a <<cluster, clustered environment>>, which means the procedure must be run against each cluster member and is not propagated to other members.
...
* PBAC for `MATCH` and `TRAVERSE` is not supported.
* PBAC for `READ` is not supported in versions prior to 2026.07.
See xref:scalability/sharded-property-databases/property-based-access-control.adoc[Property-based access control] for details.
