...
A more performant version of the same query would be:
...
This is possible because the second query uses a link:{neo4j-docs-base-uri}/cypher-manual/current/patterns/variable-length-patterns/#quantified-relationships[quantified relationship] (`-[]->+`), which enables batching relationships into sets and then fetching their properties from the different shards, and this is not possible when using the old link:{neo4j-docs-base-uri}/current/patterns/reference/#variable-length-relationships[variable-length relationship syntax] (`-[*1..]->`).

[NOTE]
Property filtering on queries matching for link:{neo4j-docs-base-uri}/current/patterns/shortest-paths[shortest paths] (with or without quantified path patterns) may not yet be fully optimized for performance.
