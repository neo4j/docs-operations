...
A standalone server can be used for testing purposes, in which case, you can skip step 2.
...
You can create a new sharded property database from an existing database by seeding it with data from one or more URIs.
...
For more information on how seed from a URI works, see see the xref:database-administration/standard-databases/seed-from-uri.adoc[Create a database from a URI].

The following examples show how to create a shared database with seeding from one or multiple URIs.

==== Example 1: Create a sharded database by seeding data from one URI

When seeding from a single URI, the system needs metadata to allocate the data to the correct databases.
This requires seed data to be in backup form, and the system looks for `foo-sharded-g000.backup`. i.e s3://bucket/folder/foo-sharded-g000.backup, s3://bucket/folder/foo-sharded-p001.backup, etc.
...
==== Example 2: Create a sharded database by seeding data from one URI with a different name
...
This is similar to Example 3, but you instead of dumps you use backups that are spread across locations.
You specify different URIs for each shard.
