...
If the process is running on the same server as another Neo4j DBMS process, the Neo4j DBMS process must be stopped.
...
It is also possible to perform the import on a cluster that has no access to any cloud.
