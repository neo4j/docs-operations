...
Your seed data must be in backup form, and the system looks for `foo-sharded-g000.backup`. i.e s3://bucket/folder/foo-sharded-g000.backup, s3://bucket/folder/foo-sharded-p001.backup etc.
...
You would use this method for two reasons, the first is if your seed data is spread across multiple locations, and the second is if your seed data is in dump form not backup form.
