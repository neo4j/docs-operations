...
If you want to deploy graph data science (GDS) on a sharded property database, you can deploy a secondary with the graph shard and GDS installed to run the GDS operations.
For details, see xref:scalability/sharded-property-databases/deploy-sharded-database-gds.adoc[Deploying GDS on a sharded property database].
...
Internal <<bookmark, bookmarks>> are used to ensure that the cluster can read its own writes.
