...
. Wait until the last <<commit, committed>> transaction ID is the same for all allocations of the database and then verify that the new database is online by running the following command:
+
[source, cypher]
----
SHOW DATABASES;
...
SHOW DATABASES;
----
. After the new database is online, you can drop the database `foo-sharded` if it is no longer needed:
+
[source, cypher]
----
DROP DATABASE foo-sharded;
