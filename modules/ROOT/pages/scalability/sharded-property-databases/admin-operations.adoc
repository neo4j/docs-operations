...
If any property shard replica falls behind the transaction log range available on the graph shard, the system puts the sharded property database into read-only mode to prevent the graph shard from pruning its transaction log entries and the server from running out of disk space.
The state persists until the issue with the lagging property shards is resolved.
