...
If any property shard replica falls behind the transaction log range available on the graph shard, the system puts the graph shard database into read-only mode to prevent it from pruning the transaction log and the server from running out of disk space, and persists this state until the issue with the lagging property shards is resolved.

.Workaround in case of a system failover in 2025.12
[IMPORTANT]
====
...
If a property shard replica becomes severed, you can recover using one of the following options:

* If only a subset of replicas of a property shard are severed, they can be recovered by quarantining the replica and then using `dbms.unquarantineDatabase` with the `replaceStateReplaceStore` option:
+
...
When a sharded property database becomes unrecoverable, it must be dropped and recreated from a backup.
