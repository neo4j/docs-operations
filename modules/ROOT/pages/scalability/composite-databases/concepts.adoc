...
* Can be deployed in standalone and <<cluster, cluster deployments>>.
...
Composite databases can be used only in a typical client/server mode when users connect to a Neo4j DBMS from their <<client-application, client application>> or tool via Bolt or HTTP protocol.
