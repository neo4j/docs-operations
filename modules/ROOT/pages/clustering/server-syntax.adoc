...
| Description
a| Shows all servers visible to the cluster.
...
[NOTE]
====
The `REALLOCATE DATABASES` command is not a pure server command.
It is considered a database allocation or cluster topology administration command.
This command still falls under server management operations because it distributes databases across servers in the cluster.

In Neo4j cluster, database placements are managed across servers.
Therefore, the `REALLOCATE DATABASES` command is better classified as a cluster-level command, not a server-level command.
====

[options="header", cols="<15s,<85"]
|===
2+| `REALLOCATE DATABASES`
...
[options="header", cols="<15s,<85"]
|===
2+| `DEALLOCATE DATABASES FROM SERVERS`
...
| Description
a| Removes all standard databases from the given servers.
