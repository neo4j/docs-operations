...
== Server management commands syntax
...
To run the command, the `SHOW SERVERS` or `SERVER MANAGEMENT` privilege is required. 
For details, see xref:authentication-authorization/dbms-administration/dbms-server-management-privileges.adoc[`SERVER MANAGEMENT` privileges].
...
Before removing a server from a cluster, you have to detach all databases placed on this server.
...
`DROP SERVER` removes a server not hosting any databases from the cluster.
