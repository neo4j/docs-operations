...
In case of a disaster affecting a server hosting the `system` database, but where the majority of servers hosting the database are still available, it is possible to recover the `system` database by adding a new server to the cluster and copying the `system` database from one of the available servers.
This method prevents downtime for the other databases in the cluster.
If this is the case, skip ahead to step 4.
====
+
...
.. Return to step 1.
+
. Restore the `system` database on a new server
+
[NOTE]
====
This method is only applicable if a majority of the servers hosting the `system` database are still available.
If not, go back to step 3.
====

.. Add a new server to the cluster (see xref:clustering/servers.adoc#cluster-add-server[Add a server to the cluster]).
