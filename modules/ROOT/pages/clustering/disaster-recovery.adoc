...
Note that even if a disaster scenario only affects one database, it is good practice to ensure that the `system` database is available and the cluster topology is satisfied before recovering the database.
