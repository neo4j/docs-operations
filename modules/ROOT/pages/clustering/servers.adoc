...
For a description of all the server options (e.g., server tags) that can be altered via the `ALTER SERVER` command, see the xref:link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/administration/access-control/server-management/#server-management-alter-server[Cypher Manual -> Modifying servers].
