:description: This section describes how to manage servers in a Neo4j cluster.
:page-role: enterprise-edition

[[manage-servers]]
= Managing servers in a cluster

In a <<cluster, clustered environment>>, server management is completely separate from database management.
...
A server can exist in six different states within the <<database-management-system, DBMS>>:
...
Starting from 2025.12, database name patterns (wildcards) are supported, and the minimum length is reduced from `3` to `1`.
...
For details, see <<server-enabled-state, Enabled state>>.
+
...
`allowedDatabases` and `deniedDatabases` do not affect <<composite-database, composite databases>>, they are always available everywhere.
Note that starting from 2025.12, database name patterns (wildcards) are supported, and the minimum length is reduced from `3` to `1`.
...
* You explicitly rebalance the databases across the cluster, possibly reallocating some databases to the newly added server(s).
For more information, see xref:clustering/databases.adoc#reallocate-databases[Reallocate databases].
