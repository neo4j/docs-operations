...
Note that the input for server tags  is a comma-separated list which cannot have duplicates.
Also note that the encoding of .conf files is Latin1.
As such, for server tags which need a larger character set (e.g Chinese or Arabic), our recommendation is to use cypher to alter server tags.
