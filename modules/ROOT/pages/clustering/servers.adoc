...
Starting with Neo4j 5.8.0 and above, _.conf_ files can use *UTF-8* by adding setting the environment variable `NEO4J_CONFIG_FILE_CHARSET=utf8`.
...
For a description of all the server options (e.g., server tags) that can be altered via the `ALTER SERVER` command, see the link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/access-control/server-management/#server-management-alter-server[Cypher Manual -> Modifying servers].
