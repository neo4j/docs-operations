...
To set a server to not host any databases, use `DEALLOCATE DATABASES FROM SERVER 'name'`, in preparation for removing the server from the cluster (see xref:clustering/servers.adoc#deallocating-state[Deallocating state] for more information).
