...
In case of failure (e.g. a partial failure of a cluster due to the loss of an instance, but not of the majority), you can transform a Read Replica instance into a Core instance to avoid availability downtime.
...
This action would cause in the override of the raft state, thus preventing the replica from successfully joining the targeted cluster.

After performing that change, you need to unbind the Replica instance and update the discovery configurations amongst cluster members.
Here are the instructions: 

. Ensure that the converted _read_replica_ currently belongs to the same cluster it should be re-introduced back to as core. 
...
At this point, the previous _read_replica_ (now core) instance may be introduced into the running cluster. 
To keep this setting, the following configuration updates are advised:

- On the previous _read_replica_ (now core) instance, set `causal_clustering.discovery_advertised_address` and `causal_clustering.discovery_listen_address` as appropriate.
...
This should replace addresses of any removed core(s) with the discovery addresses of the previous _read_replica_ (now core) instance.
+
[NOTE]
====
In cases where `causal_clustering.discovery_type` is other than `LIST`, make sure to update the corresponding address resolution addresses records -- e.g. DNS A records for discovery types DNS and SRV, and any Kubernetes service address alternation to reflect the inclusion of the _read_replica_ discovery address.
====
