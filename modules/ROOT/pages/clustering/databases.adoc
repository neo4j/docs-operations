...
This limitation is due to Raft replication not being present when only a single server is running, i.e. the server is running in _standalone_ or _single mode_.
...
The mechanism is pluggable, allowing new sources of seeds to be supported (see link:{docs-base-uri}/java-reference/current/extending-neo4j/project-setup/#extending-neo4j-plugin-seed-provider[Java Reference -> Implement custom seed providers] for more information).
