...
The `system` database contains metadata on the <<database-management-system, DBMS>> and security configuration.
...
* You can use <<restore-database-using-recreate-procedure, the `dbms.recreateDatabase()` procedure>>.
* You can also use <<restore-database-using-copy-command, the `neo4j-admin database copy` command>>.
...
Starting with Neo4j 2026.04, you can also restore a database from a server URI using the `server://` scheme.
