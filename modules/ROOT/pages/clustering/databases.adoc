...
----
Alternatively, starting from 5.26, you can use link:{neo4j-docs-base-uri}/cypher-manual/5/syntax/parameters[parameters] to provide the number of primaries and secondaries:
...
[NOTE]
====
The aforementioned procedures do not put a server in a xref:clustering/servers.adoc#deallocating-state[_deallocating_] state.
They only move database allocations away from the server.

If you need to transition a server to the _deallocating_ state, use the Cypher command xref:clustering/servers.adoc#_deallocating_databases_from_a_server[`DEALLOCATE DATABASES FROM SERVERS`].
====
...
* You can <<restore-database-using-uri-approach, seed the cluster from a URI>>, where all servers to host the database are seeded with an identical seed from an external source specified by that URI.
* You can use <<cluster-designated-seeder, a _designated seeder_>>, where a designated server is used to create a backed-up database on other servers in the cluster.
* label:new[Introduced in 5.24] You can use <<restore-database-using-recreate-procedure, the `dbms.recreateDatabase()` procedure>>.


[[restore-database-using-uri-approach]]
=== Restore a database from a URI

Before restoring a database from a backup, ensure you have its backup and drop the database on all servers.

The seed can be a full backup, a differential backup (see xref:database-administration/standard-databases/seed-from-uri.adoc#cloud-seed-provider[Create database from a URI -> `CloudSeedProvider`], starting from Neo4j 5.26), or a dump from an existing database.
...
You have the opportunity to create a topology that will differ from the original one.
...
. On one of the cluster's servers, run the command:
+
[source, cypher]
----
CREATE DATABASE foo
...
----
+
Number of primaries and secondaries must respect the number of `ENABLED` servers and their mode constraints.
...
The new database can be allocated to any `ENABLED` server in the cluster, subject to its mode constraints.
...
The number of newly defined `primaries` and `secondaries` must respect the number of `ENABLED` servers in a cluster and their mode constraints.
