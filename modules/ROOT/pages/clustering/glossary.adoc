...
As long as it uses bookmarks, the cluster guarantees that a client application can read at least its own writes.
...
[[glossary-leader]]
| Leader
| Each database has a designated leader within the cluster and it can only be located on a primary server.
The leader receives all write transactions from clients and replicates writes synchronously to followers and asynchronously to secondary copies of the database.
