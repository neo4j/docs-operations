:description: The page describes recommended and non-recommended patterns of deploying Neo4j cluster(s) across multiple cloud regions / data centers.
...
In this guide, the terms _cloud region_ and _data center_ are used interchangeably to represent distinct, isolated geographical deployment locations.

Whether you deploy Neo4j on your own physical hardware or in the cloud, the architectural rules for latency, quorum, and replication remain identical.
====
...
== Deploying multiple independent clusters
...
The replica database is always **read-only**.
It can be promoted to become write-available if required.

For detailed instructions, see xref:clustering/replicating-databases-across-clusters.adoc[].
...
. By running `SHOW DATABASES`, verify that the former replica database is online on the desired number of servers and has type `standard`.

For detailed information, see xref:clustering/replicating-databases-across-clusters.adoc#dr-scenario[Replicating databases across clusters -> Disaster recovery scenario].


[[single-cluster-across-multiple-dc]]
== Stretching a single cluster across multiple data centers
...
Taking into account cluster architecture and topology, you decide where database primaries and secondaries are located, balancing performance and fault tolerance.
...
5+^| Multiple independent clusters
...
| * More database management required.
* The `system` database cannot be replicated, therefore permissions and RBAC have to be configured separately on each cluster.
| 

5+^| A single cluster with database copies distributed across multiple data centers
