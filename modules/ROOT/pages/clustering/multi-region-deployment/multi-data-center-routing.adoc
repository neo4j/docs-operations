...
When deploying a cluster it is often desirable to take advantage of locality to reduce latency and improve performance.
...
.Tag the current server with `us` and `us-east`
[source, properties]
----
initial.server.tags=us,us-east
----

.Tag the current server with `london`
[source, properties]
----
initial.server.tags=london
----
.Tag the current server with `eu`
[source, properties]
----
...
Depending on the deployment and the available number of database allocations in the cluster, different strategies make sense for whether or not the reading workload should be routed to the primary allocations.

The following boolean configuration settings allow the routing of read workload to primary databases:

* xref:configuration/configuration-settings.adoc#config_dbms.routing.reads_on_primaries_enabled[`dbms.routing.reads_on_primaries_enabled`] defaults to `true`.

* xref:configuration/configuration-settings.adoc#config_dbms.routing.reads_on_writers_enabled[`dbms.routing.reads_on_writers_enabled`] defaults to `false`.


[[mdc-policy-definitions]]
=== Policy definitions

The configuration of load balancing policies is transparent to client applications and expressed via DSL.
...
There are several filters available for specifying rules of routing policies.
...
* label:new[Introduced in 2025.12] `roles()`
** The filter applies to database copies.
** Possible values are `writer`, `follower`, and `secondary`.
`writer` is a primary leader.
It synchronously replicates writes to other primaries.
The `writer` is elected automatically from amongst database’s primaries and may change over time.
`followers` are other primaries.
...
There are different ways to configure the load balancing for the cluster so that client applications can direct their workload to the most appropriate cluster members, such as those nearby.
...
The server policies plugin is selected by specifying the following setting:
...
[[builtin-routing-policies]]
== Built-in routing policies

In 2025.12, the following built-in routing policies are introduced:

* `builtin_default` +
// `roles(follower,secondary);roles(writer);halt()`
Ensures read operations prefer database members that are not processing write workloads (non-writer primaries and secondaries), reducing resource competition between reads and writes.

* `builtin_all` +
// `all()`
This policy does not separate read and write workloads.

* `builtin_prefer_secondaries` +
// `roles(secondary);roles(follower);roles(writer);halt()`
Read operations are routed exclusively to secondaries, ensuring isolation of read workloads from write workloads.

* `builtin_add_writer`
// `roles(writer,secondary)->notEmptyRoles(secondary);roles(writer,follower);halt()`

// * `builtin_prefer_writer`
// `roles(writer,secondary);roles(follower);halt()`

As user-defined policies, the built-in policies are configured when deploying a cluster or adding a server to an existing one.
The default policy is determined according to configuration settings specified in the _neo4j.conf_ file.
See <<mdc-primaries-for-reading, configuration settings>> and <<mdc-filters,filters>> for details.

The built-in routing policies are derived from the configuration settings:

* xref:configuration/configuration-settings.adoc#config_dbms.routing.reads_on_primaries_enabled[`dbms.routing.reads_on_primaries_enabled`], which defaults to `true`.
Controls whether non-writer primaries (`followers`) are included as read endpoints.
Otherwise, secondaries are used.

* xref:configuration/configuration-settings.adoc#config_dbms.routing.reads_on_writers_enabled[`dbms.routing.reads_on_writers_enabled`], which defaults to `false`.
Controls whether the writer is included as a read endpoint.
Otherwise, non-writer primaries (`followers`) and secondaries are used.

// internal.dbms.routing.reads_on_writer_only_enabled - default false
...
The DSL syntax consists of a set of rules which are considered in order.
Each rule in turn consists of a set of <<mdc-filters, filters>> which limit the considered database allocations, starting with the complete set.
The first rule to produce a non-empty result is the final result.
If there are any database allocations still left after the last filter then these allocations are returned to the driver.
If no rule is able to produce a usable result then the driver is signalled a failure.
////

The `builtin_default` routing policy::

The `builtin_default` routing table prefers non-writer primaries and secondaries as read endpoints.
Falls back to the writer if no other endpoints are available.
+
// DSL: roles(follower,secondary);roles(writer);halt()
+
You do not need to configure _neo4j.conf_ file as the settings remain default:
+
[source, properties]
----
...
The `builtin_all` routing policy::

The `builtin_all` routing table includes the writer, non-writer primaries, and secondaries as read endpoints.
+
// DSL: all()
+
Configure _neo4j.conf_ file:
+
[source, properties]
----
dbms.routing.reads_on_primaries_enabled=true
...
The `builtin_add_writer` routing policy::

The `builtin_add_writer` routing table includes writer and secondaries as read endpoints.
If no endpoints are available, the policy falls back to tagged secondaries, before finally back to the writer and non-writer primaries. 
+
// DSL: roles(writer,secondary)->notEmptyRoles(secondary);roles(writer,follower);halt()
+
Configure _neo4j.conf_ file: 
+
[source, properties]
...
The `builtin_prefer_secondaries` routing policy::

The `builtin_prefer_secondaries` policy prefers secondaries, then non-writer primaries, and finally the writer.
+
// DSL: roles(secondary);roles(follower);roles(writer);halt()
+
Configure _neo4j.conf_ file: 
+
[source, properties]
...
_Catch-up strategy plugins_ are sets of rules that define how secondaries contact upstream database members in the cluster in order to synchronize transaction logs.
...
These provide coarse-grained algorithms for selecting an upstream database member:

[options="header",width="100%",cols="1,1"]
|===
| Plugin name                                          | Resulting behavior
| `connect-to-random-primary-server`                   | Connect to any *primary database allocation* selecting at random from those currently available.
| `typically-connect-to-random-secondary`              | Connect to any available *secondary allocation*, but around 10% of the time connect to any random primary allocation.
| `connect-randomly-to-server-tags`                    | Connect at random to any available *secondary allocation* tagged with any of the server tags specified in the comma-separated list `server.cluster.catchup.connect_randomly_to_server_tags`.
| `leader-only`                                        | Connect only to the current Raft leader of the *primary databases*.
| `connect-randomly-within-server-tags`                | Connect at random to any available *secondary database allocations* with any of the server tags which this server has.
|===

Pre-defined strategies are used by configuring the xref:configuration/configuration-settings.adoc#config_server.cluster.catchup.upstream_strategy[`server.cluster.catchup.upstream_strategy`] setting.
...
The catch-up strategy is selected by asking each of the strategies in list-order whether they can provide an upstream database member from which transactions can be pulled.

.Define an upstream database member selection strategy
====
...
With this configuration the secondary first tries to connect to any other server with tag(s) specified in `server.cluster.catchup.connect_randomly_to_server_tags`.
Should it fail to find any live servers with those tags, then it connects to a random secondary allocation.
...
To ensure that downstream database members can still access live data in the event of upstream failures, the last resort of any database allocation is always to contact a random primary.
