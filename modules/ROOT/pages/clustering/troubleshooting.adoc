...
[NOTE]
====
This setting is internal in Neo4j 5.26 LTS.
Use it only to mitigate this issue.
====

[[clustering-troubleshooting-raft-sender-channel-acquisition-calver]]
=== Behavior from Neo4j 2025.02

Starting in Neo4j 2025.02, the setting is available as xref:configuration/configuration-settings.adoc#config_dbms.cluster.raft.async_channel_acquisition_enabled[`dbms.cluster.raft.async_channel_acquisition_enabled`] and defaults to `true`.
