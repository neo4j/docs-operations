...
:page-role: enterprise-edition deprecated-5.26
...
[NOTE]
====
The HTTP API is replaced by the Query API, which is enabled by default in all Neo4j deployments starting with Neo4j 5.26.
...
| `raftCommandsPerSecond` label:deprecated[Deprecated in Neo4j 5.4]  | number   | yes    | `124`                       | An estimate of the average Raft state machine throughput over a sampling window configurable via `clustering.status_throughput_window` setting.
