...
====
<<<<<<< HEAD
Deviating values for `currentPrimariesCount` and `requestedPrimariesCount`, and for `currentSecondariesCount` and `requestedSecondariesCount` is not necessarily a cause for concern.
Often, these values can vary while the cluster is allocating, reallocating, and/or deallocating databases.
=======
...
>>>>>>> 292cfd1... added admonition about different values for current and requested counts (#1015)
====