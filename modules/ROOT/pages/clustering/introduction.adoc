...
Servers are not primary or secondary, though they have a xref:configuration/configuration-settings.adoc#config_initial.server.mode_constraint[`modeConstraint`] referred to as `PRIMARY`, `SECONDARY`, or `NONE`.
...
A _primary_ is a copy of a database that serves write operations and can be the writer for that database.
