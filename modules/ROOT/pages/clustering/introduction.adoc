...
That means they can host standard databases only in primary mode or only in secondary mode (see servers 1 or 3 on the _Figure 1_).
If the `modeConstraint` is set to `NONE`, a server can host databases in either mode.
In other words, a server can host primaries for some databases and secondaries for other databases (see server 6 on the _Figure 1_).
