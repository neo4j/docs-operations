...
Server tags are defined on each server using the  `xref:reference/configuration-settings.adoc#config_server.server.tags[initial.server.tags]` parameter in _neo4j.conf_. 
Each server in a cluster can be tagged with to zero or more server tags.
Server tags can be altered at runtime via the `ALTER SERVER` command, consult the Cypher operations manual for further information.
