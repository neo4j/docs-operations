...
If a <<database-management-system, Neo4j DBMS>> cluster contains multiple databases, each one of those databases operates within a logically separate Raft group, and therefore each has an individual leader.
...
The Drivers do not perform any routing when the <<bolt-protocol, `bolt://`>> URI scheme is used, instead connecting directly to the specified host.
