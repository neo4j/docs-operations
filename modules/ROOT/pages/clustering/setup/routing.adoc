...
Typically this would be to make sure that write operations are sent to the server that can write for your specific database, and that read operations are sent to other servers.
...
A routing table provides the writers, readers and routers for a specific database.
There will usually be one writer, though there may be none if the database is read only or unhealthy.
With the default configuration all other servers that host the database will be considered readers, i.e. the writer will not be in the list of readers.
This is to let it focus on the write load and not have to manage two kinds of interaction.
Typically all servers that host the database will be given as 'routers', which are servers that can be contacted to get a new routing table for that database.

Our standard drivers will get a routing table the first time they attempt to connect to a database, and will then get a fresh one after the configured time-to-live, or if it seems the routing table has become out of date.
For example, if the routing table says `server-3` is the writer for the database, but write requests get rejected with 'not able to write', the driver may decide to get a new routing table, because the writer could be a different server.
...
=== Routing table API
If you want lower level detail about getting routing tables, you can read the link:{neo4j-docs-base-uri}/bolt/current/driver-api/#_driver_routing_table[Bolt driver API].
