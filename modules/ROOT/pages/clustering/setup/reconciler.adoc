...
Executing an operation and getting back a successful response means that the request is safely <<commit, committed>> to the `system` database, and will be processed by each member of the cluster at some point, assuming the server is healthy enough.
