...
[CAUTION]
====
Securing client-to-server communication is not covered in this chapter (e.g. <<bolt-protocol, Bolt>>, HTTPS, Backup).
====
...
Encryption is one aspect of security, the other cornerstones are authentication and integrity.
...
However, even if you change the default and encrypt it, without mutual TLS, any network-reachable client can replicate the database via the cluster port.
Therefore, configure a cluster to encrypt its intra-cluster communication by setting `dbms.ssl.policy.cluster.enabled=true` and `client_auth=REQUIRE`.
See xref:security/ssl-framework.adoc#ssl-configuration[SSL framework -> Configuration] for details.
