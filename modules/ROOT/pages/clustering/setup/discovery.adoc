...
The behavior of the initial discovery is determined by the parameters `xref:reference/configuration-settings.adoc#config_dbms.cluster.discovery.resolver_type[dbms.cluster.discovery.resolver_type]` and `xref:reference/configuration-settings.adoc#config_dbms.cluster.discovery.endpoints[dbms.cluster.discovery.endpoints]`, and is described in the following sections.
