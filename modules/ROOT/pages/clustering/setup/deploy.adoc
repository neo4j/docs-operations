...
This is described in detail in xref:clustering/setup/discovery.adoc[Cluster server discovery].
| xref:configuration/dynamic-settings.adoc#config_dbms.cluster.num_primaries[`dbms.cluster.num_primaries`]
| The number of initial servers in primary mode.
If not specified, it defaults to one server in primary mode.
| xref:configuration/dynamic-settings.adoc#config_dbms.cluster.num_secondaries[`dbms.cluster.num_secondaries`]
...
If `TOPOLOGY` is not specified, the database is created according to `dbms.cluster.num_primaries` specified in `neo4j.conf`.
Also, if `dbms.cluster.num_secondaries` is specified to any other number than 0, the second line of the command would read `TOPOLOGY 3 PRIMARIES 0 SECONDARIES`.
Thus the number specified with `TOPOLOGY` overrides both `dbms.cluster.num_primaries` and `dbms.cluster.num_secondaries` (if applicable) provided that the specified numbers do not exceed the number of available servers.
====
...
Note that this operation is possible even without specifying `dbms.cluster.num_secondaries` in the initial configuration.
Anything specified in the `TOPOLOGY` part of the Cypher command overrides the `dbms.cluster.num_secondaries` setting.
====
