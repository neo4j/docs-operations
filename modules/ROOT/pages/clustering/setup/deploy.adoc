...
See also xref:configuration/configuration-settings.adoc#cluster-settings[cluster settings reference] for more detailed descriptions.
...
| Minimum number of machines initially required to form a clustered <<database-management-system, DBMS>>.
...
If not specified, it defaults to one copy in primary role.
...
If not specified, it defaults to zero copies in secondary role.
...
Neo4j clusters can be deployed over IPv6 networks.
IPv6 addresses must be enclosed in square brackets in all cluster address settings.
