...
====
If you can afford sending unencrypted metrics within the internal network (e.g. server.metrics.prometheus.endpoint=10.0.0.123:2004), all servers within the same netmask will be able to access it. +
If you specify anything more permissible, e.g. server.metrics.prometheus.endpoint=0.0.0.0:2004, you should have a firewall rule to prevent any unauthorized access. Data in transit will still not be encrypted, so it should never go other any insecure networks.
|Default value
m|+++localhost:2004+++
|===
