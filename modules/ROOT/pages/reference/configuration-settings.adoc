...
|===
|Description
a|Enables or disables tracking of how much time a query spends actively executing on the CPU. Calling `SHOW TRANSACTIONS` will display the time, but not in the query log. If you want the CPU time to be logged in the query log, use `db.track_query_cpu_time=true` and `db.logs.query.time_logging_enabled=true`.
|Valid values
