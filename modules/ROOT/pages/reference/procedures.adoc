...
`vectorDimension` must be between `1` and `2048` _inclusively_.
...
| Description
a|
List available node labels within the database.
However, results from the procedure exclude labels created, for example through the operation <<procedure_db_createlabel, `CALL db.createLabel`>>, that have not been attached to pre-existing nodes.
The procedure outputs are also filtered according to the caller's RBAC (role-based access control) rules, and the procedure returns empty results for users not authorized to view those labels.
| Signature
...
| Description
a|
List available realtionship types within the database.
However, results from the procedure exclude relationship types created, for example through the operation <<procedure_db_createrelationshiptype, `CALL db.createRelationshipType`>>, that have not been attached to pre-existing relationships.
The procedure outputs are also filtered according to the caller's RBAC (role-based access control) rules, and the procedure returns empty results for users not authorized to view those relationship types.
| Signature
