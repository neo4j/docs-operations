...
| label:yes[]
| label:admin-only[] +
This procedure is deprecated in 5.6 and will be removed.
It is replaced by `dbms.cluster.secondaryReplicationDisable()`.
...
| label:yes[]
| label:admin-only[] +
Replaces `dbms.cluster.readReplicaToggle()` in 5.6.
...
====
This procedure is deprecated in 5.6 and will be removed.
It is replaced by xref:reference/procedures.adoc#procedure_dbms_cluster_secondaryreplicationdisable[`dbms.cluster.secondaryReplicationDisable()`]
====
