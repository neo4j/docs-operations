...
.dbms.cluster.cordonServer() label:admin-only[]
[cols="<15s,<85"]
|===
| Description
...
.dbms.cluster.uncordonServer() label:admin-only[]
|===
| Description
a| Removes the cordon on a server, returning it to 'enabled'.
| Signature
m|dbms.cluster.uncordonServer
| Mode
m|WRITE
|===
...
| Signature
m|dbms.setDefaultAllocationNumbers
| Mode
a|WRITE
|===
...
| Signature
m|dbms.showTopologyGraphConfig()
| Mode
m|READ
|===
