...
| xref:reference/procedures.adoc#procedure_cdc_current[`cdc.cu()`]
| label:no[]
| label:yes[]
...
.db.index.fulltextlableAnalyzers()
[cols="<15s,<85"]
|===
| Description
a|
...
An example of the `options` map: `{skip: 30, limit: 10, analyzer: 'whitespace'}`
| Signature
m|db.index.fulltext.queryRelati (relationship :: RELATIONSHIP, score :: FLOAT)
| Mode
m|READ
// | Default roles
// m|reader, editor, publisher, architect, admin
