...
* xref:reference/procedures.adoc#change-data-capture[]
* xref:reference/procedures.adoc#cluster-and-server-management[]
* xref:reference/procedures.adoc#configuration-and-security[]
* xref:reference/procedures.adoc#connection-and-lock-management[]
* xref:reference/procedures.adoc#database-and-transaction-utilities[]
* xref:reference/procedures.adoc#genai-and-vectors[]
* xref:reference/procedures.adoc#index-management[]
* xref:reference/procedures.adoc#job-scheduling-and-upgrade[]
* xref:reference/procedures.adoc#schema-and-metadata[]
* xref:reference/procedures.adoc#statistics[]

This page also lists current xref:reference/procedures.adoc#deprectated-procedures[deprecated procedures] and the xref:reference/procedures.adoc#removed-procedures[procedures removed in Neo4j 5], along with their replacements.

The available procedures on an instance depends on several factors:

* Neo4j Enterprise Edition provides a larger set of procedures than Neo4j Community Edition.
* Neo4j's link:{neo4j-docs-base-uri}/apoc/{page-version}/[APOC Core library] and link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/genai-integrations/[GenAI plugin] are installed by default on link:{neo4j-docs-base-uri}/aura/[Aura] instances, but have to be installed separately on on-prem instances.
...
| *Description* 3+| Query changes that happened from the provided change identifier.
...
.Details
|===
| *Syntax* 3+| `db.cdc.query([ from selectors ]) :: (id, txId, seq, metadata, event)`
| *Description* 3+| Query changes that happened from the provided change identifier.
.3+| *Input arguments* | *Name* | *Type* | *Description*
| `from` | `STRING` | The change identifier to query changes from.
| `selectors` | `LIST<MAP>` | An optional list of selectors to filter out changes.
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.checkConnectivity([ port-name server ]) :: (serverId, name, address, mode-constraint, port-name, port-address, result)`
| *Description* 3+| Check the connectivity of this instance to other cluster members. Not all ports are relevant to all members. Valid values for 'port-name' are: [CLUSTER, INTRA_BOLT, RAFT]
.3+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.cordonServer(server)`
| *Description* 3+| Mark a server in the topology as not suitable for new allocations. It will not force current allocations off the server. This is useful when deallocating databases when you have multiple unavailable servers.
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.deallocateDatabaseFromServer(server, database [, dryrun ]) :: (database, fromServerName, fromServerId, toServerName, toServerId, mode)`
| *Description* 3+| Deallocate a specific user database from a specific server.
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.deallocateDatabaseFromServers(servers, database [, dryrun ]) :: (database, fromServerName, fromServerId, toServerName, toServerId, mode)`
| *Description* 3+| Deallocate a specific user database from a list of servers.
.4+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.deallocateNumberOfDatabases(server, number [, dryrun ]) :: (database, fromServerName, fromServerId, toServerName, toServerId, mode)`
| *Description* 3+| Deallocate a number of user databases from a specific server.
.4+| *Input arguments* | *Name* | *Type* | *Description*
| `server` | `STRING` | The id of the server to deallocate from.
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.protocols() :: (orientation, remoteAddress, applicationProtocol, applicationProtocolVersion, modifierProtocols)`
| *Description* 3+| Overview of installed protocols
.6+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.readReplicaToggle(databaseName, pause) :: (state)`
| *Description* 3+| The toggle can pause or resume read replica (deprecated in favor of dbms.cluster.secondaryReplicationDisable)
.3+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.reallocateDatabase(database [, dryrun ]) :: (database, fromServerName, fromServerId, toServerName, toServerId, mode)`
| *Description* 3+| Reallocate a specific database.
.3+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.reallocateNumberOfDatabases(number [, dryrun ]) :: (database, fromServerName, fromServerId, toServerName, toServerId, mode)`
| *Description* 3+| Reallocate a specified number of user databases.
.3+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.routing.getRoutingTable(context [, database ]) :: (ttl, servers)`
| *Description* 3+| Returns the advertised bolt capable endpoints for a given database, divided by each endpoint's capabilities. For example, an endpoint may serve read queries, write queries, and/or future `getRoutingTable` requests.
.3+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.secondaryReplicationDisable(databaseName, pause) :: (state)`
| *Description* 3+| The toggle can pause or resume the secondary replication process.
.3+| *Input arguments* | *Name* | *Type* | *Description*
| `databaseName` | `STRING` | The name of the database to toggle the secondary replication process for.
| `pause` | `BOOLEAN` | Whether or not to enable/disable the secondary replication process.
.2+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.setAutomaticallyEnableFreeServers(autoEnable)`
| *Description* 3+| With this method you can set whether free servers are automatically enabled.
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.showParallelDiscoveryState() :: (mode, stateComparison, v1ServerCount, v2ServerCount)`
| *Description* 3+| Compare the states of Discovery service V1 and Discovery service V2.
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.switchDiscoveryServiceVersion(mode)`
| *Description* 3+| Allows you to select which discovery service should be started.
Possible values are:
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.uncordonServer(server)`
| *Description* 3+| Remove the cordon on a server, returning it to 'enabled'.
.2+| *Input arguments* | *Name* | *Type* | *Description*
| `server` | `STRING` | The name or id of the server to be uncordoned.
| *Mode* 3+| WRITE
|===

Replaced by the xref:clustering/server-syntax.adoc#server-management-syntax[`ENABLE SERVER`] command.
...
.Details
|===
| *Syntax* 3+| `dbms.routing.getRoutingTable(context [, database ]) :: (ttl, servers)`
| *Description* 3+| Returns the advertised bolt capable endpoints for a given database, divided by each endpoint's capabilities. For example, an endpoint may serve read queries, write queries, and/or future `getRoutingTable` requests.
.3+| *Input arguments* | *Name* | *Type* | *Description*
| `context` | `MAP` | Routing context, for example, routing policies.
| `database` | `STRING` | The database to get a routing table for.
...
For more information, see xref:configuration/index.adoc[] and xref:security/index.adoc[].
...
.Details
|===
| *Syntax* 3+| `dbms.checkConfigValue(setting, value) :: (valid, message)`
| *Description* 3+| Check if a potential config setting value is valid.
.3+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.components() :: (name, versions, edition)`
| *Description* 3+| List DBMS components and their versions.
...
.Details
|===
| *Syntax* 3+| `dbms.info() :: (id, name, creationDate)`
| *Description* 3+| Provides information regarding the DBMS.
.4+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.listCapabilities() :: (name, description, value)`
| *Description* 3+| List capabilities.
.4+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.listConfig([ searchString ]) :: (name, description, value, dynamic, defaultValue, startupValue, explicitlySet, validValues)`
| *Description* 3+| List the currently active configuration settings of Neo4j.
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.security.clearAuthCache()`
| *Description* 3+| Clears authentication and authorization cache.
| *Mode* 3+| DBMS
|===
...
.Details
|===
| *Syntax* 3+| `dbms.setDatabaseAllocator(allocator)`
| *Description* 3+| With this method you can set the allocator that is responsible for selecting servers for hosting databases.
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.setDefaultAllocationNumbers(primaries, secondaries)`
| *Description* 3+| With this method you can set the default number of primaries and secondaries.
.3+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.setDefaultDatabase(databaseName) :: (result)`
| *Description* 3+| Change the default database to the provided value. The database must exist and the old default database must be stopped.
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.showCurrentUser() :: (username, roles, flags)`
| *Description* 3+| Show the current user.
.4+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.showTopologyGraphConfig() :: (allocator, defaultPrimariesCount, defaultSecondariesCount, defaultDatabase, autoEnableFreeServers)`
| *Description* 3+| With this method the configuration of the Topology Graph can be displayed.
.6+| *Return arguments* | *Name* | *Type* | *Description*
| `allocator` | `STRING` | The name of the allocator.
...
For more information, see xref:monitoring/connection-management.adoc[] and xref:database-internals/concurrent-data-access.adoc[].
...
.Details
|===
| *Syntax* 3+| `dbms.killConnection(id) :: (connectionId, username, message)`
| *Description* 3+| Kill network connection with the given connection id.
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.killConnections(ids) :: (connectionId, username, message)`
| *Description* 3+| Kill all network connections with the given connection ids.
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.listActiveLocks(queryId) :: (mode, resourceType, resourceId)`
| *Description* 3+| List the active lock requests granted for the transaction executing the query with the given query id.
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.listConnections() :: (connectionId, connectTime, connector, username, userAgent, serverAddress, clientAddress)`
| *Description* 3+| List all accepted network connections at this instance that are visible to the user.
.8+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.listPools() :: (pool, databaseName, heapMemoryUsed, heapMemoryUsedBytes, nativeMemoryUsed, nativeMemoryUsedBytes, freeMemory, freeMemoryBytes, totalPoolMemory, totalPoolMemoryBytes)`
| *Description* 3+| List all memory pools, including sub pools, currently registered at this instance that are visible to the user.
...
.Details
|===
| *Syntax* 3+| `dbms.quarantineDatabase(databaseName, setStatus [, reason ]) :: (databaseName, quarantined, result)`
| *Description* 3+| Place a database into quarantine or remove it from it.
.4+| *Input arguments* | *Name* | *Type* | *Description*
...
[[database-and-transaction-utilities]]
== Database and transaction utilities

For more information, see xref:database-administration/index.adoc[] and xref:database-internals/transaction-management.adoc[].
...
.Details
|===
| *Syntax* 3+| `db.clearQueryCaches() :: (value)`
| *Description* 3+| Clears all query caches.
.2+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.info() :: (id, name, creationDate)`
| *Description* 3+| Provides information regarding the database.
.4+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.listLocks() :: (mode, resourceType, resourceId, transactionId)`
| *Description* 3+| List all locks at this database.
.5+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.ping() :: (success)`
| *Description* 3+| This procedure can be used by client side tooling to test whether they are correctly connected to a database. The procedure is available in all databases and always returns true. A faulty connection can be detected by not being able to call this procedure.
.2+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.prepareForReplanning([ timeOutSeconds ])`
| *Description* 3+| Triggers an index resample and waits for it to complete, and after that clears query caches. After this procedure has finished queries will be planned using the latest database statistics.
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.queryJmx(query) :: (name, description, attributes)`
| *Description* 3+| Query JMX management data by domain and name. For instance, use `*:*` to find all JMX beans.
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `tx.getMetaData() :: (metadata)`
| *Description* 3+| Provides attached transaction metadata.
.2+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `tx.setMetaData(data)`
| *Description* 3+| Attaches a map of data to the transaction. The data will be printed when listing queries, and inserted into the query log.
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.create.setNodeVectorProperty(node, key, vector)`
| *Description* 3+| Set a vector property on a given node in a more space efficient representation than Cypher's `SET`.
.4+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.create.setRelationshipVectorProperty(relationship, key, vector)`
| *Description* 3+| Set a vector property on a given relationship in a more space efficient representation than Cypher's `SET`.
.4+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.create.setVectorProperty(node, key, vector) :: (node)`
| *Description* 3+| Set a vector property on a given node in a more space efficient representation than Cypher's SET.
.4+| *Input arguments* | *Name* | *Type* | *Description*
| `node` | `NODE` | The node on which the new property will be stored.
| `key` | `STRING` | The name of the new property.
...
Replaced by xref:reference/procedures.adoc#procedure_db_create_setnodevectorproperty[`db.create.setNodeVectorProperty()`] and xref:reference/procedures.adoc#procedure_db_create_setrelationshipvectorproperty[`db.create.setRelationshipVectorProperty()`].
...
.Details
|===
| *Syntax* 3+| `db.index.vector.createNodeIndex(indexName, label, propertyKey, vectorDimension, vectorSimilarityFunction)`
| *Description* 3+| Create a named node vector index for the specified label and property with the given vector dimensionality using either the EUCLIDEAN or COSINE similarity function.
...
.Details
|===
| *Syntax* 3+| `db.index.vector.queryRelationships(indexName, numberOfNearestNeighbours, query) :: (relationship, score)`
| *Description* 3+| Query the given relationship vector index.
...
.Details
|===
| *Syntax* 3+| `genai.vector.encodeBatch(resources, provider, configuration) :: (index, resource, vector)`
| *Description* 3+|  Generate several vector embeddings with a single API request.
.4+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `genai.vector.listEncodingProviders() :: (name, requiredConfigType, otionalConfigType, defaultConfig)`
| *Description* 3+| List the available GenAI providers.
.5+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.index.fulltext.awaitEventuallyConsistentIndexRefresh()`
| *Description* 3+| Wait for the updates from recently committed transactions to be applied to any eventually-consistent full-text indexes.
| *Mode* 3+| READ
|===
...
.Details
|===
| *Syntax* 3+| `db.index.fulltext.listAvailableAnalyzers() :: (analyzer, description, stopwords)`
| *Description* 3+| List the available analyzers that the full-text indexes can be configured with.
.4+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.index.fulltext.queryNodes(indexName, queryString [, options ]) :: (node, score)`
| *Description* 3+| Query the given full-text index. Returns the matching nodes and their Lucene query score, ordered by score.
...
.Details
|===
| *Syntax* 3+| `db.index.fulltext.queryRelationships(indexName, queryString [, options ]) :: (relationship, score)`
| *Description* 3+| Query the given full-text index. Returns the matching relationships and their Lucene query score, ordered by score.
Valid _key: value_ pairs for the `options` map are:
...
.Details
|===
| *Syntax* 3+| `db.resampleIndex(indexName)`
| *Description* 3+| Schedule resampling of an index (for example: CALL db.resampleIndex("MyIndex")).
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.resampleOutdatedIndexes()`
| *Description* 3+| Schedule resampling of all outdated indexes.
| *Mode* 3+| READ
|===
...
[[job-scheduling-and-upgrade]]
== Job scheduling and upgrade

For more information, see xref:monitoring/background-jobs.adoc[]
https://neo4j.com/docs/operations-manual/current/monitoring/background-jobs/
...
.Details
|===
| *Syntax* 3+| `dbms.scheduler.failedJobs() :: (jobId, group, database, submitter, description, type, submitted, executionStart, failureTime, failureDescription)`
| *Description* 3+| List failed job runs. There is a limit for amount of historical data.
.11+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.scheduler.groups() :: (group, threads)`
| *Description* 3+| List the job groups that are active in the database internal job scheduler.
.3+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.scheduler.jobs() :: (jobId, group, submitted, database, submitter, description, type, scheduledAt, period, state, currentStateDescription)`
| *Description* 3+| List all jobs that are active in the database internal job scheduler.
...
.Details
|===
| *Syntax* 3+| `dbms.upgrade() :: (status, upgradeResult)`
| *Description* 3+| Upgrade the system database schema if it is not the current schema.
.3+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `dbms.upgradeStatus() :: (status, description, resolution)`
| *Description* 3+| Report the current status of the system database sub-graph schema.
.4+| *Return arguments* | *Name* | *Type* | *Description*
| `status` | `STRING` | The upgrade status of the system database.
...
.Details
|===
| *Syntax* 3+| `db.schema.nodeTypeProperties() :: (nodeType, nodeLabels, propertyName, propertyTypes, mandatory)`
| *Description* 3+| Show the derived property schema of the nodes in tabular form.
.6+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.schema.relTypeProperties() :: (relType, propertyName, propertyTypes, mandatory)`
| *Description* 3+| Show the derived property schema of the relationships in tabular form.
.5+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.createLabel(newLabel)`
| *Description* 3+| Create a label
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.createProperty(newProperty)`
| *Description* 3+| Create a Property
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.createRelationshipType(newRelationshipType)`
| *Description* 3+| Create a RelationshipType
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.labels() :: (label)`
| *Description* 3+| List all labels attached to nodes within a database according to the user's access rights. The procedure returns empty results if the user is not authorized to view those labels.
.2+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.propertyKeys() :: (propertyKey)`
| *Description* 3+| List all property keys in the database.
.2+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.relationshipTypes() :: (relationshipType)`
| *Description* 3+| List all types attached to relationships within a database according to the user's access rights. The procedure returns empty results if the user is not authorized to view those relationship types.
.2+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.stats.clear(section) :: (section, success, message)`
| *Description* 3+| Clear collected data of a given data section. Valid sections are 'QUERIES'
.2+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.stats.collect(section [, config ]) :: (section, success, message)`
| *Description* 3+| Start data collection of a given data section. Valid sections are 'QUERIES'
.3+| *Input arguments* | *Name* | *Type* | *Description*
| `section` | `STRING` | Specify 'QUERIES'.
...
.Details
|===
| *Syntax* 3+| `db.stats.retrieve(section [, config ]) :: (section, data)`
| *Description* 3+| Retrieve statistical data about the current database. Valid sections are 'GRAPH COUNTS', 'TOKENS', 'QUERIES', 'META'
.3+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.stats.retrieveAllAnonymized(graphToken [, config ]) :: (section, data)`
| *Description* 3+| Retrieve all available statistical data about the current database, in an anonymized form.
.3+| *Input arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.stats.status() :: (section, status, data)`
| *Description* 3+| Retrieve the status of all available collector daemons, for this database.
.4+| *Return arguments* | *Name* | *Type* | *Description*
...
.Details
|===
| *Syntax* 3+| `db.stats.stop(section) :: (section, success, message)`
| *Description* 3+| Stop data collection of a given data section. Valid sections are 'QUERIES'
.2+| *Input arguments* | *Name* | *Type* | *Description*
| `section` | `STRING` | Specify 'QUERIES'.
.4+| *Return arguments* | *Name* | *Type* | *Description*
...
The procedures deprecated in Neo4j 5 will be removed in Neo4j 6.
