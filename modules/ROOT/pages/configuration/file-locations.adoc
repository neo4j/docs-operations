...
The _bin_ directory contains the Neo4j running script and built-in tools, such as xref:cypher-shell.adoc[Cypher Shell] and xref:neo4j-admin-neo4j-tools.adoc[].
...
Starting with Neo4j 2026.03, Fleet Manager is built-in and enabled by default, therefore, the plugin is no longer supported.
* link:{neo4j-docs-base-uri}/graph-data-science/current/[Graph Data Science Library]
* link:{neo4j-docs-base-uri}/genai/plugin/current/[GenAI plugin]
