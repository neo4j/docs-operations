...
[NOTE]
====
If  _tmp_ is set to `noexec`, we recommend defining in `conf/neo4j.conf` the following: `dbms.jvm.additional=-Djava.io.tmpdir=/home/neo4j`.
Additionally, replace `/home/neo4j` with a path that has `exec` permissions.

For `/bin/cypher-shell`, set this via environment variable: `export JAVA_OPTS=-Djava.io.tmpdir=/home/neo4j` and also replace `/home/neo4j` with a path that has `exec` permissions.
====
