...
* The _neo4j-admin.conf_ file, a configuration file located in the same directory as the _neo4j.conf_ file, must, at least, be readable by its owner or by the user-group to which the owner belongs.
...
* The _neo4j.conf_ and _neo4j-admin.conf_ files must, at least, be readable by the user that the Neo4j process runs as.
