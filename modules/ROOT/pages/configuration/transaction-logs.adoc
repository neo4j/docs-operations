...
| <<config_db.tx_log.rotation.retention_policy,`db.tx_log.rotation.retention_policy`>>
| `2 days`
a|
...
You can control the number of transaction logs that Neo4j keeps using the parameter <<config_db.tx_log.rotation.retention_policy,`db.tx_log.rotation.retention_policy`>>.
