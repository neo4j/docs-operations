...
|Valid values
a|A comma-separated list where each element is a string, and in which entries must be either `nativge` or a valid OIDC authorization provider. Entries must exist in dbms.security.authorization_providers.
|Default value
m|
|===
