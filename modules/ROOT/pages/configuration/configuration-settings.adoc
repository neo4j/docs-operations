...
|Valid values
a|db.cluster.raft.leader_transfer.priority_tag, a string identifying a Server Tag
|Default value
m|++++++
|===
...
|Valid values
a|server.cluster.catchup.connect_randomly_to_server_tags, a ',' separated list with elements of type 'a string identifying a Server Tag'.
|Default value
m|++++++
|===
...
|Valid values
a|server.bolt.advertised_address, a socket address in the format `hostname:port`, `hostname`, or `:port` with accessible address. If missing, port or hostname, it is acquired from `server.default_advertised_address`.
|Default value
m|+++:7687+++
|===
...
|Valid values
a|db.logs.query.annotation_data_as_json_enabled, a boolean
|Default value
m|+++false+++
|===
...
|Valid values
a|dbms.security.logs.ldap.groups_at_debug_level_enabled, a boolean
|Default value
m|+++false+++
|===
