...
|===
|Description
a|A comma-separated list of OIDC provider names that will be used for attribute-based access control (ABAC). For more information, see xref:authentication-authorization/attribute-based-access-control.adoc[Attribute-based access control].
|Valid values
a|A comma-separated list where each element is a string.
|Default value
m|
