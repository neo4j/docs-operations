...
|===
|Description
a|Whether to require the use of an encryption level to secure communications with this connector.
|Valid values
...
|===
|Description
a|The address to which the routing connector binds.
|Valid values
a|A socket address in the format of `hostname:port`, `hostname`, or `:port`. If missing, it is acquired from server.default_listen_address.
|Default value
m|+++:7688+++
...
Error is raised when no connection can be acquired within the configured time.
...
If this option is too low, acquiring a connection will require an additional network call, which will cause a performance hit.
If this is set high, live connections might no longer be used, leading to errors.
...
|===
|Description
a|Sets the level for the driver internal logging.
|Valid values
a|One of [DEBUG, INFO, WARN, ERROR, NONE].
|Default value
m|+++INFO+++
