...
|===
|Description
a|The number of threads that are applying to the queued index updates for eventually consistent full-text indexes.
|Valid values
a|An integer that is minimum 1.
|Default value
m|+++1+++
...
|===
|Description
a|How often an eventually consistent full-text index is refreshed (readers are guaranteed to see changes). If set to `0`, refresh is done by the threads applying eventually consistent full-text index updates.
|Valid values
a|A duration (Valid units are: ns, μs, ms, s, m, h and d; default unit is s).
|Default value
m|+++0s+++
