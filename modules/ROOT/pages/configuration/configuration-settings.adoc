...
|===
|Description
a|Vary the order of the entries in routing tables each time one is produced. This means that clients are less likely to all contact the same server if alternatives are available.
|Valid values
a|a boolean
|Default value
m|+++true+++
