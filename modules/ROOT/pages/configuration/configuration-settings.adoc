...
In a <<cluster, clustered environment>>, `CALL dbms.setConfigValue` affects only the server it is run against, and it is not propagated to other members.
...
Neo4j provides support for <<bolt-protocol, Bolt>>, HTTP, and HTTPS protocols via connectors.
...
|===
|Description
a|Configure <<client-application, client applications>> such as Browser and Bloom to send Product Analytics data.
|Valid values
a|A boolean.
|Default value
m|+++true+++
