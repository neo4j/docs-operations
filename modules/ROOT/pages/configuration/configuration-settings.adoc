...
[[config_db.cluster.backup.pull_interval]]
=== `db.cluster.backup.pull_interval`

.db.cluster.backup.pull_interval
[frame="topbot", stripes=odd, grid="cols", cols="<1s,<4"]
|===
|Description
a|The interval at which a secondary server fetches updates for a specific database from a backup folder.
|Valid values
a|A duration (Valid units are: ns, μs, ms, s, m, h and d; default unit is s).
|Default value
m|+++1m+++
...
For details on how to use `db.cluster.backup.pull_interval` with cross-cluster database replication (GA in Neo4j 2026.08), see xref:clustering/replicating-databases-across-clusters.adoc#create-replica-via-remote-backup-pulling[Clustering -> Creating a replica database with remote backup pulling].
...
For details on how to use `db.cluster.catchup.pull_interval` with cross-cluster database replication (GA in Neo4j 2026.08), see xref:clustering/replicating-databases-across-clusters.adoc#create-replica-via-the-network[Clustering -> Creating a replica database via the network].
...
The `dbms.cluster.endpoints` is renamed in 2025.01 from `dbms.cluster.discovery.v2.endpoints`.
For details, see xref:breaking-changes.adoc[].

Typically, all cluster members, including the current server, are specified in this list.
For example, `dbms.cluster.discovery.resolver_type=LIST` combined with `server01.example.com:6000,server02.example.com:6000,server03.example.com:6000` attempt to reach Neo4j servers listening on _server01.example.com_, _server02.example.com_ and _server03.example.com_; all on port `6000`.
...
This setting specifies the strategy that the server uses to determine the addresses for other servers in the cluster to contact for _bootstrapping_.
...
`dbms.cluster.minimum_initial_system_primaries_count` defines the minimum number of servers initially required to form the cluster.
Reaching this threshold allows members to discover each other, but it does not guarantee that the DBMS is online or that the `system` databases are in the `STARTED` state.
For details, see xref:clustering/setup/deploy.adoc[].
...
The initial number of database copies in primary role.
For details on primary databases in a Neo4j cluster, see xref:clustering/introduction.adoc#clustering-primaries[Introduction: Neo4j clustering architecture].
...
The initial number of database copies in secondary role.
For details on secondary databases in a Neo4j cluster, see xref:clustering/introduction.adoc#clustering-secondaries[Introduction: Neo4j clustering architecture].
...
For example, a server configured with `initial.server.mode_constraint=SECONDARY` hosts databases only in `secondary` role.
For details, see xref:clustering/servers.adoc#server-enabled-state[How to enable a server].

Once enabled, a server's mode constraint can only be changed with xref:clustering/servers.adoc#alter-server-options[`ALTER SERVER 'name' SET OPTIONS {modeConstraint:'PRIMARY'}`].
...
This address/port setting specifies the location where the server advertises its availability for discovery service communication as well as transaction-shipping catch-up requests.

For example,`server.cluster.advertised_address=192.168.33.20:6001` listens for cluster communication on the network interface bound to `192.168.33.20` on port `6001`.
...
The address/port setting that specifies which network interface and port the Neo4j server binds to for cluster communication.
This setting must be set in coordination with the address this server advertises it listens at in the setting `server.cluster.advertised_address`.

For example, `server.cluster.listen_address=0.0.0.0:6001` listens for cluster communication on any network interface at port `6001`.
For details, see xref:configuration/ports.adoc#_cluster[Configuration -> Ports].
...
For example, `server.cluster.raft.advertised_address=192.168.33.20:7000` listens for cluster communication in the network interface bound to `192.168.33.20` on port `7000`.
For information about cluster ports, see xref:configuration/ports.adoc#_cluster[Ports].
...
The address/port setting that specifies which network interface and port the Neo4j server binds to for cluster communication.
This setting must be set in coordination with the address this server advertises it listens at in the setting `server.cluster.raft.advertised_address`.

For example, `server.cluster.raft.listen_address=0.0.0.0:7000` listens for cluster communication on any network interface at port `7000`.
...
This setting controls which mode a given server hosts the `system` database in: `PRIMARY` or `SECONDARY`.

For example, `server.cluster.system_database_mode=SECONDARY` means that this server holds only a secondary copy of the `system` database.

If you have a large cluster, then increasing the number of `system` primaries up to seven can improve the resiliency of your DBMS.
However, if enabling more than 10 servers, it is recommended to start making the later ones secondaries.
For details, see xref:clustering/databases.adoc#cluster-system-db[The `system` database in a cluster].
...
When configuring the HTTPS or xref:/performance/bolt-thread-pool-configuration.adoc[Bolt], see also  <<security-settings>> and xref:security/ssl-framework.adoc[SSL framework] for details on how to work with SSL certificates.
...
[[config_dbms.bolt.local_connector.object_transport_enabled]]
...
|===
|Description
a|Enables the Query API to use local objects instead of Packstream for Bolt Server communication.
Removing this conversion step improves performance.
|Valid values
a|A boolean.
|Default value
m|+++false+++
...
[NOTE]
====
Starting with Neo4j 2026.03, Fleet Manager is built-in and enabled by default, so there is no need to download and install the plugin separately.
====
...
|Valid values
a|A comma-separated list where each element is a string, which entries must be either `native` or a valid OIDC authorization provider and entries must exist in `dbms.security.authorization_providers`.
|Default value
m|
|===
...
[IMPORTANT]
====
Always use `pkce` auth flow.
It adds an additional layer of security to the authorization code flow, which prevents certain types of attacks, such as authorization code interception attacks.

`implicit` flow has been removed from OAuth 2.1 because it exposes the access or ID token in the browser history and the redirect URL, in addition to other security vulnerabilities.
Starting with Neo4j 2026.06, its support is deprecated and will be removed in a future release.
====
...
----
  ...
  response_type: code if auth_flow is pkce.
...
[role=label--new-2026.09]
[[config_server.split_archive.part_size]]
=== `server.split_archive.part_size`

.server.split_archive.part_size
[frame="topbot", stripes=odd, grid="cols", cols="<1s,<4"]
|===
|Description
a|The size of individual files when creating a split archive with multiple files (dump or backup). The size can be specified in bytes or with a unit suffix (e.g. `5G`, `100g`, `1TiB`). The minimum split size is `1GiB`. The default value is `0` and then archives will not be split into multiple files regardless of the total size.
|Valid values
a|A byte size (valid multipliers are `B`, `KiB`, `KB`, `K`, `kB`, `kb`, `k`, `MiB`, `MB`, `M`, `mB`, `mb`, `m`, `GiB`, `GB`, `G`, `gB`, `gb`, `g`, `TiB`, `TB`, `PiB`, `PB`, `EiB`, `EB`) that is minimum `1.00GiB` or is `0B`.
|Default value
m|+++0B+++
|===

For more information on splitting backup or dump archives, see:

* xref:backup-restore/online-backup.adoc#split-backup-archive[Back up an online database -> Split a backup archive into multiple files].
* xref:backup-restore/aggregate.adoc#split-consolidated-backup-archive[Aggregate a database backup chain -> Split a consolidated backup archive into multiple files].
* xref:backup-restore/offline-backup.adoc#split-dump-archive[Back up an offline database ->Split a dump archive into multiple files].
* xref:backup-restore/copy-database.adoc#split-backup-archive[Copy a database store -> Split a backup archive into multiple files].
