...
|===
|Description
a|Allow label inference during cardinality estimation. During cardinality estimation, if the planner can logically deduce that a node has a label that was not explicitly expressed in the query, the plannerwill use this information during cardinality estimation. +
...
Not inferring more labels yields better results for nodes with several dependent labels.
|Valid values
