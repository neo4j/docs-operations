...
For example, `100M size` will the limit logical log space on the disk to 100MB per database, and `200K txs` will limit the number of transactions kept to 200 000 per database.
Valid optional space restriction is a logical log space restriction like `100M`. For example, `7 days 100M`.
|Default value
m|+++2 days+++
|===
