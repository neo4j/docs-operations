...
* `PERIODIC` label:default[]- it runs a checkpoint as per the interval specified by `<<config_db.checkpoint.interval.tx,db.checkpoint.interval.tx>>` and `<<config_db.checkpoint.interval.time,db.checkpoint.interval.time>>`.
...
* `CONTINUOUS` label:enterprise-edition[Enterprise Edition] -- it ignores `<<config_db.checkpoint.interval.tx,db.checkpoint.interval.tx>>` and `<<config_db.checkpoint.interval.time,db.checkpoint.interval.time>>` settings and runs the checkpoint process all the time.

* `VOLUME` label:enterprise-edition[Enterprise Edition] -- it makes the best effort to checkpoint often enough so that the database does not get too far behind on deleting old transaction logs as specified in the `<<config_db.tx_log.rotation.retention_policy,db.tx_log.rotation.retention_policy>>` setting.

* `VOLUMETRIC` -- it makes the best effort to checkpoint often enough so that the database does not get too far behind on deleting old transaction logs as specified in the `<<config_db.tx_log.rotation.retention_policy,db.tx_log.rotation.retention_policy>>` setting.
|Valid values
...
Longer checkpoint intervals typically mean that recovery will take longer to complete in case of a crash.
On the other hand, a longer checkpoint interval can also reduce the I/O load that the database places on the system, as each checkpoint implies a flushing and forcing of all store files.
|Valid values
a|a duration (Valid units are: `ns`, `μs`, `ms`, `s`, `m`, `h` and `d`)
|Default value
m|15m
|===
...
Longer checkpoint intervals typically mean that recovery will take longer to complete in case of a crash.
On the other hand, a longer checkpoint interval can also reduce the I/O load that the database places on the system, as each checkpoint implies a flushing and forcing of all store files.
The default `100000` means a checkpoint on every 100000 transactions.
|Valid values
a|an integer which is minimum `1`
|Default value
m|100000
|===
...
On the other hand, a longer checkpoint interval can also reduce the I/O load that the database places on the system, as each checkpoint implies a flushing and forcing of all store files.
|Valid values
a|a byte size (valid multipliers are `B`, `KiB`, `KB`, `K`, `kB`, `kb`, `k`, `MiB`, `MB`, `M`, `mB`, `mb`, `m`, `GiB`, `GB`, `G`, `gB`, `gb`, `g`, `TiB`, `TB`, `PiB`, `PB`, `EiB`, `EB`). The minimum is `1.00KiB`.
|Default value
m|250.00MiB
|===
...
An IO is in this case a 8 KiB (mostly sequential) write.
...
Removing the setting, or commenting it out, sets the default value of `600`.
|Valid values
a|an integer
|Default value
m|600
...
|Valid values
a|a ',' separated list with elements of type 'a socket address in the format 'hostname:port', 'hostname' or ':port''.
|Default value
m|
|===
...
|Valid values
a|one of [DNS, LIST, SRV, K8S] which may require different settings depending on the discovery type: `DNS requires [dbms.cluster.discovery.endpoints], LIST requires [], SRV requires [dbms.cluster.discovery.endpoints], K8S requires [dbms.kubernetes.label_selector, dbms.kubernetes.service_port_name]`
|Default value
m|LIST
|===
...
|Valid values
a|an integer which is minimum `2`
|Default value
m|3
|===
...
|Valid values
a|an integer which is in the range `4096` to `10485760`
|Default value
m|32768
|===
...
|Valid values
a|a ',' separated list with elements of type 'a string'.
|Default value
m|
|===
...
|Valid values
a|a byte size (valid multipliers are `B`, `KiB`, `KB`, `K`, `kB`, `kb`, `k`, `MiB`, `MB`, `M`, `mB`, `mb`, `m`, `GiB`, `GB`, `G`, `gB`, `gb`, `g`, `TiB`, `TB`, `PiB`, `PB`, `EiB`, `EB`) which is minimum `1.00KiB`
|Default value
m|250.00MiB
|===
...
|Valid values
a|an integer which is minimum `1` and is maximum `11`.
...
|Valid values
a|an integer which is minimum `0` and is maximum `20`.
The same value applies to runtime max number.
|Default value
m|0
|===
...
|Valid values
a|a ',' separated set with elements of type 'a string'.
|Default value
m|
|===
...
|Valid values
a|a ',' separated list with elements of type 'a string identifying a Server Tag'.
|Default value
m|
|===
...
|Valid values
a|a socket address in the format 'hostname:port', 'hostname' or ':port' which accessible address. If missing port or hostname it is acquired from server.default_advertised_address
|Default value
m|:6000
|===
...
|Valid values
a|a socket address in the format 'hostname:port', 'hostname' or ':port'. If missing port or hostname it is acquired from server.default_listen_address
|Default value
m|:6000
|===
...
|Valid values
a|a duration (Valid units are: `ns`, `μs`, `ms`, `s`, `m`, `h` and `d`; default unit is `s`) which is minimum `1ms`
|Default value
m|1m
|===
...
|Valid values
a|a duration (Valid units are: `ns`, `μs`, `ms`, `s`, `m`, `h` and `d`; default unit is `s`) which is minimum `0s`
|Default value
m|1m
|===
...
|Valid values
a|a ',' separated set with elements of type 'one of [TRANSACTIONAL_ENDPOINTS, UNMANAGED_EXTENSIONS, BROWSER, ENTERPRISE_MANAGEMENT_ENDPOINTS]'.
|Default value
...
|Valid values
a|a socket address in the format 'hostname:port', 'hostname' or ':port' which has no specified port and accessible address
|Default value
m|localhost
|===
...
|Valid values
a|a socket address in the format 'hostname:port', 'hostname' or ':port' which has no specified port
|Default value
m|localhost
|===
...
|Valid values
a|a string which specified load balancer plugin exist.
|Default value
m|server_policies
|===
...
|Valid values
a|a duration (Valid units are: `ns`, `μs`, `ms`, `s`, `m`, `h` and `d`; default unit is `s`) which is minimum `1s`
|Default value
m|5m
|===
...
|Valid values
a|a double which is in the range `0.0` to `1.0`
|Default value
m|0.75
|===
...
|Valid values
a|an integer which is in the range `1` to `50000000`
|Default value
m|10000
|===
...
|Valid values
a|an integer which is in the range `1048576` to `2147483647`
|Default value
m|8388608
|===
...
|Valid values
a|an integer which is minimum `0`
|Default value
m|5
|===
...
|Valid values
a|a byte size (valid multipliers are `B`, `KiB`, `KB`, `K`, `kB`, `kb`, `k`, `MiB`, `MB`, `M`, `mB`, `mb`, `m`, `GiB`, `GB`, `G`, `gB`, `gb`, `g`, `TiB`, `TB`, `PiB`, `PB`, `EiB`, `EB`) which is minimum `1.00MiB` or is `0B`
|Default value
m|0B
|===
...
|Valid values
a|a byte size (valid multipliers are `B`, `KiB`, `KB`, `K`, `kB`, `kb`, `k`, `MiB`, `MB`, `M`, `mB`, `mb`, `m`, `GiB`, `GB`, `G`, `gB`, `gb`, `g`, `TiB`, `TB`, `PiB`, `PB`, `EiB`, `EB`) which is minimum `10.00MiB` or is `0B`
|Default value
m|0B
|===
...
|Valid values
a|an integer which is minimum `16`
|Default value
m|128
|===
...
|Valid values
a|a byte size (valid multipliers are `B`, `KiB`, `KB`, `K`, `kB`, `kb`, `k`, `MiB`, `MB`, `M`, `mB`, `mb`, `m`, `GiB`, `GB`, `G`, `gB`, `gb`, `g`, `TiB`, `TB`, `PiB`, `PB`, `EiB`, `EB`) which is minimum `4.00KiB` and is power of 2
|Default value
m|512.00KiB
|===
...
|Valid values
a|a byte size (valid multipliers are `B`, `KiB`, `KB`, `K`, `kB`, `kb`, `k`, `MiB`, `MB`, `M`, `mB`, `mb`, `m`, `GiB`, `GB`, `G`, `gB`, `gb`, `g`, `TiB`, `TB`, `PiB`, `PB`, `EiB`, `EB`) which is minimum `0B`
|Default value
m|2.00GiB
|===
...
|Valid values
a|an integer which is in the range `1` to `512`
|Default value
m|128
|===
...
|Valid values
a|an integer which is in the range `0` to `255`
|Default value
m|4
|===
...
|Valid values
a|a byte size (valid multipliers are `B`, `KiB`, `KB`, `K`, `kB`, `kb`, `k`, `MiB`, `MB`, `M`, `mB`, `mb`, `m`, `GiB`, `GB`, `G`, `gB`, `gb`, `g`, `TiB`, `TB`, `PiB`, `PB`, `EiB`, `EB`) which is in the range `0B` to `8388608.00TiB`
|Default value
m|10.00MiB
|===
...
|Valid values
a|a ',' separated list with elements of type 'A simple globbing pattern that can use `*` and `?`.'.
|Default value
...
|Valid values
a|a socket address in the format `hostname:port`, `hostname`, or `:port`. If missing, port and hostname are acquired from `server.default_listen_address`.
|Default value
m|localhost:2004
|===
...
|Valid values
a|a socket address in the format `hostname:port`, `hostname` or `:port`
|Default value
...
label:version-number[Neo4j v5.3]
...
|Valid values
a|a string which matches the pattern `[A-Za-z0-9-]*` (has to be a valid LDAP attribute name, only containing letters [A-Za-z], digits [0-9] and hyphens [-].)
|Default value
m|samaccountname
|===
...
|Valid values
a|a string which Must be a string containing '{0}' to understand where to insert the runtime authentication principal.
|Default value
...
|Valid values
a|a ',' separated list with elements of type 'a string'. which Can not be empty
|Default value
m|memberOf
|===
...
----
|Valid values
a|a string which must be semicolon separated list of key-value pairs or empty
|===
...
|Valid values
a|a ',' separated set with elements of type 'A valid database name containing only alphabetic characters, numbers, dots and dashes with a length between 3 and 63 characters, starting with an alphabetic character but not with the name 'system''.
|Default value
m|
|===
...
|Valid values
a|an integer, which is in the range `1` to `44738`
|Default value
m|Number of available processors, or 500 for machines that have more than 500 processors.
|===
...
|Valid values
a|a ',' separated list with elements of type '<classname>=<mount point> string'.
|Default value
m|
|===
...
|Valid values
a|an integer which is in the range `1` to `100`
|Default value
m|5
|===
...
|Valid values
a|a byte size (valid multipliers are `B`, `KiB`, `KB`, `K`, `kB`, `kb`, `k`, `MiB`, `MB`, `M`, `mB`, `mb`, `m`, `GiB`, `GB`, `G`, `gB`, `gb`, `g`, `TiB`, `TB`, `PiB`, `PB`, `EiB`, `EB`) which is minimum `128.00KiB`
|Default value
m|256.00MiB
|===
