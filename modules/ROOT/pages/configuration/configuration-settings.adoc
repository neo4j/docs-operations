...
|Valid values
a|a socket address in the format `hostname:port`, `hostname`, or `:port`. If missing, they are acquired from server.default_advertised_address.
|Default value
m|+++:6000+++
|===
...
|Valid values
a|a socket address in the format `hostname:port`, `hostname` or `:port`. If missing, they are acquired from server.default_listen_address
|Default value
m|+++:6000+++
|===
...
|Valid values
a|a `,` separated list with elements of type String. which Can not be empty
|Default value
m|+++memberOf+++
|===
