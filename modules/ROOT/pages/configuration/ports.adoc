...
The page lists the Neo4j-specific ports.
They are in addition to those necessary for ordinary network operation.

All ports apply equally to IPv4 and IPv6.
To listen on all IPv6 interfaces, use `[::]` instead of `0.0.0.0` as the listen address, for example `[::]:7687`, and `[::1]:7687` for the loopback interface.
...
All servers of Neo4j Enterprise open these ports, whether or not they are currently in a multi-process configuration.

In production environments, external access to the <<cluster>> transaction port `6000` should be blocked by a firewall unless clustering is configured with SSL encryption (`dbms.ssl.policy.cluster.enabled=true` with `client_auth=REQUIRE`).
This port allows unauthenticated database replication.
...
* xref:configuration/configuration-settings.adoc#cluster-settings[Settings reference]
