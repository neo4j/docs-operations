...
. Go to _<NEO4J_HOME>/products_ and copy or move the chosen plugins (_.jar files_) into the _<NEO4J_HOME>/plugins_ directory.
See the table in xref:configuration/file-locations/[File locations] for more information.

. *Optional*: Some plugins, such as Bloom, require licensing and configuring to become available to the user within Neo4j.
