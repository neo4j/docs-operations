...
The default language version for the whole <<database-management-system, DBMS>> can be configured in the _neo4j.conf_ file using the setting xref:configuration/configuration-settings.adoc#config_db.query.default_language[db.query.default_language]. +
...
The default value of `db.query.default_language` is `CYPHER_5`.
This means that unless `db.query.default_language=CYPHER_25` is explicitly set in _neo4j.conf_, newly created databases will default to `CYPHER_5`.
+
