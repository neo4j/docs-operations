...
* For versions prior to 2026.01, Cypher 5 is the default version for all newly created databases, however, if you want to take advantage of the new features in Cypher 25, you can set the default version to Cypher 25.
* For versions 2026.02 and later, Cypher 25 is the default version for all newly created databases, but you can still set the default version to Cypher 5 if you delete or comment out the xref:configuration/configuration-settings.adoc#config_db.query.default_language[db.query.default_language=CYPHER 25] in the _neo4j.conf_ file.
...
The default language version for the whole DBMS can be configured in the _neo4j.conf_ file using the setting xref:configuration/configuration-settings.adoc#config_db.query.default_language[db.query.default_language]. +
