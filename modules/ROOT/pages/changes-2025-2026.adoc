...
The log that the `neo4j-admin database import` command creates to store information about the import process, is moved from _server/data/imports_ to _server/logs/_.

Neo4j 2026.03::

The import log name _neo4j-admin-import-yyyy-MM-dd.HH.mm.ss.log_ and the path to it are updated to _server/data/imports/dbname-yyyy-MM-dd.HH.mm.ss/import.log_. +
