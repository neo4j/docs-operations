...
Neo4j 2025.12 label:cypher-25[]::

* The `dbms.setDefaultAllocationNumbers()` procedure now accepts an additional input parameter: `propertyShardReplicas`.
* The `dbms.showTopologyGraphConfig()` procedure now includes `propertyShardReplicas` in its returned results.
