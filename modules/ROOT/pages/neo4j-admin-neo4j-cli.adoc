...
Both `neo4j-admin` and `neo4j` commands support the <<_help_command, `--help`>> option, which prints the command's usage and options, and the <<_version_command, `--version`>> option, which prints the version of the command.
...
Starting from Neo4j 2025.04, when evaluating a license, the command prints information about the days left until the license expires.
If the license has expired, the command prints a warning with the number of days since the license expired.
