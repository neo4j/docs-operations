...
`SHOW TRANSACTIONS` can be combined with `TERMINATE TRANSACTIONS` to identify and terminate transactions in the same statement (no other `SHOW` command can be combined with either of the transaction commands).
...
* In Neo4j 5.x, you cannot combine transaction commands with other Cypher clauses.
====
