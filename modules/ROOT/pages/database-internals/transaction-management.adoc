...
Note that the timeout value can be overridden to a value smaller than that configured by `db.transaction.timeout`.

label:new[Introduced in Neo4j 5.3]
Starting from Neo4j 5.3 and onwards, you can set the transaction timeout to any value, even larger than configured by `db.transaction.timeout`.
