...
No resource cleanup is required for a transaction that is explicitly <<commit, committed>> or rolled back, and the transaction closure is an empty operation.
...
For all available settings, see xref:/configuration/configuration-settings.adoc#transaction-settings[Transaction settings].
