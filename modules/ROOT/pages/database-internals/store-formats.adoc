...
High Limit:: label:enterprise-edition[] label:deprecated[Deprecated in 5.23]
* Based on standard but allows more nodes and relationships.
* Performs slightly worse than standard and requires more disk space.
* Stores graph data in linked list-like structures on disk.
...
Standard and High Limit formats are deprecated starting from Neo4j 5.23. +
...
Questions to consider when choosing a store format::
...
If you are on Standard or High Limit, consider migrating to Block.
+
. Can I have zero downtime migration?
No, changing the store format requires the database to be offline.
For more information on how to change the store format of an existing database, see <<changing-the-store-format-of-existing-databases>>.
+
. How long does it take to migrate to Block?
+
Migrating to the block format is an IO-intensive offline operation.
The duration of the migration process depends on several factors:
+
* Database Size: Larger databases will take longer to migrate.
