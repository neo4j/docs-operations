...
`block` format provides a better performance and scalability than the deprecated formats. 
They will remain part of the product throughout the v5 and vNext server lifecycles. 
The last version of Neo4j to include these formats will be vNext.LTS which is planned for release in November 2026. LTS versions of Neo4j are supported for 3 years following their release. 
...
See https://neo4j.com/docs/operations-manual/current/database-administration/standard-databases/listing-databases/#_show_detailed_information_for_a_particular_database

. *When will support for `standard` and `high_limit` formats end?* +
The last version of Neo4j to include these formats will be an LTS release planned for release in November 2026. 
This LTS release will be supported for three years, so November 2029.
...
[NOTE]
====
Changing the store format can be a time-consuming operation and will also require any indexes to the re-populated.
