...
`standard` and `high_limit` record formats are deprecated starting from Neo4j 5.23.
For more information, see <<record-format-deprecation, Record format deprecation>>.
...
* *Performance:* Faster queries, uses advanced data structures and inlining techniques for better data locality.
...
Block format means a few pages need to be loaded to serve a query, which means fewer page faults and less IO.
...
Under optimal conditions, migrating 100GB of data to `block` format typically takes around 10 minutes.
====
