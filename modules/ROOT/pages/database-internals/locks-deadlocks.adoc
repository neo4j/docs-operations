...
*_A node is considered dense if it at any point has had 50 or more relationships (i.e. it will still be considered dense even if it comes to have less than 50 relationships at any point in the future)._
...
For an example of how deadlocks can be handled in procedures, server extensions, or when using Neo4j embedded, see https://neo4j.com/docs/java-reference/current/transaction-management/[Transaction management in the Neo4j Java Reference].
