...
Checkpointing is the process of flushing all pending updates from the page cache to the store files.
...
Consequently, pruning is triggered whenever checkpointing occurs, with or without a specific verification of their existence.
...
This policy checks every 15 minutes whether there are changes pending flushing and if so, it performs a checkpoint and subsequently triggers a log prune.
The periodic policy is specified by the `db.checkpoint.interval.tx` and `db.checkpoint.interval.time` settings and the checkpointing is triggered when either of them is reached.
...
It ignores the `db.checkpoint.interval.time` and `db.checkpoint.interval.tx` settings.
