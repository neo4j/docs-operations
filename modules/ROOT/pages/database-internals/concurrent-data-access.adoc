...
Even if it later has fewer than 50 relationships, it is still considered as dense.
+
A node is considered sparse if it has never had more than 50 relationships.

`block` format::
A node is considered dense for a particular relationship type.
For example, a node can be dense for type A and sparse for type B.
A node is considered dense for a specific relationship type when it has, at one point, surpassed a certain internal size threshold, which typically happens at approximately 50 relationships of that type, but depends on the number and size of properties connected to these relationships.
...
This process may acquire additional exclusive locks in a sorted manner if necessary, to ensure data consistency.
