...
For example, running the following two queries in xref:cypher-shell.adoc[Cypher-shell] at the same time will result in a deadlock because they are attempting to modify the same node properties concurrently:
