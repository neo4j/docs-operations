...
This is because all queries read the value of `n.prop` within their own transaction, and cannot see the incremented value from any other transaction that has not yet been <<commit, committed>>.
...
This is true even for indexes that back link:{neo4j-docs-base-uri}/cypher-manual/current/schema/constraints/create-constraints/#create-property-uniqueness-constraints[property uniqueness constraints].
