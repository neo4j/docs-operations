...
The default value is `2 days 2G`, which means Neo4j keeps logical logs that contain any transaction <<commit, committed>> within 2 days from the current time and within the allocated log space (2G) and prunes the ones that are older or larger.
