...
The inspect command shows the metadata stored in the header of backup files.
...
=== Showing the metadata of the backup files

The following command shows the backup files' names along with their respective metadata:
...
=== Showing the latest backups

To show only the most recent backups performed for each database, use the `--latest-backup` option.
