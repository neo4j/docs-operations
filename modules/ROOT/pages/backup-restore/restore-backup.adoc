...
* label:new[Introduced in 5.24] Recreate a database using the xref:database-administration/standard-databases/recreate-database.adoc[`dbms.recreateDatabase()` procedure].
* label:new[Introduced in 5.20] You can also load a **full** database backup artifact using the xref:backup-restore/restore-dump.adoc[`neo4j-admin database load`] command.
...
* label:new[Introduced in 5.24] Recreate a database using the `dbms.recreateDatabase()` procedure.
...
Starting with Neo4j 5.20, you can load a **full** database backup artifact using the xref:backup-restore/restore-dump.adoc[`neo4j-admin database load`] command.
This functionality is available in the Community Edition.
...
For details on how to restore a database backup located in cloud storage in a Neo4j Kubernetes deployment, see xref:kubernetes/operations/backup-restore.adoc#kubernetes-neo4j-restore[Restore a single database].
...
If you need to restore the users and roles metadata, you have to do it manually by running the Cypher script from the _<NEO4J_HOME>_ directory, _/data/scripts/databasename/restore_metadata.cypher_, which the `neo4j-admin database restore` command outputs, using xref:tools/cypher-shell.adoc[]:
