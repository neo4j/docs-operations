...
* You have a backup (created with the `neo4j-admin database backup` command) of your existing database `foo` in an AWS S3 bucket.
* Or you use the copy command with the new `--target-format=backup --target-location=S3 bucket`.
