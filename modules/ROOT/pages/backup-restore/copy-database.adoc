...
|--property-shard-count=<propertyShardCount>footnote:shardedDb[See xref:scalability/sharded-property-databases/resharding-sharded-databases.adoc#splitting-existing-db-into-shards[Property Sharding -> Splitting an existing database into shards] for details.]
...
For examples of how to use `neo4j-admin database copy` to create a sharded property database, see xref:scalability/sharded-property-databases/resharding-sharded-databases.adoc#resharding-a-databases[Resharding a database].
