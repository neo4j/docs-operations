...
neo4j-admin database copy [-h] [--copy-schema] [--expand-commands] [--force] [--verbose] [--compact-node-store
                          [=true|false]] [--compress
                          [=true|false]] [--additional-config=<file>] [--from-pagecache=<size>]
                          [--property-shard-count=<propertyShardCount>] [--source-format=<format>] [--source-location=<path>]
                          [--split-archive-part-size=<splitsize>] [--target-format=<format>]
...
In this case, the database is copied to a temporary location (by default the current working directory or the path as defined by `--temp-path`), before being moved to replace the original.
...
|--compress[=true\|false]footnote:shardedDb[For using this option with sharded property databases, see xref:scalability/sharded-property-databases/data-ingestion.adoc[Property Sharding -> Data import].]
|label:new[Introduced in 2026.04] Request backup artifact to be compressed. Compression can yield a backup artefact many times smaller, but the exact reduction depends upon many factors, including the database format and the kind of data stored. If disabled, the size of the produced artifact will be approximately equal to the size of the backed-up database. The speed of the copy operation is affected by compression, but which is faster depends upon the relative performance of CPU and storage. If copy speed is important, consider evaluating both options - with compression enabled and disabled.
|false
...
|--property-shard-count=<propertyShardCount>footnote:shardedDb[]
|label:new[Introduced in 2025.12] label:infinigraph[] Number of shards of property data that will be created, each shard will be its own database.
|0
...
|label:new[Introduced in 2026.03] Source format can be either a plain database directory/files structure (`database`) or a backup artifact (`backup`). Uses the `--temp-path` location to keep any intermediate state.
|database

|--source-location=<path>
|label:new[Introduced in 2026.03] Location for source backup artifact data. Used together with `--source-format=backup`.
|
...
|--target-format=<format>footnote:shardedDb[]
|label:new[Introduced in 2025.12] Target format can be either a plain database directory/files structure (`database`) or a backup artifact (`backup`). Uses the `--temp-path` location to keep any intermediate state.
|database

|--target-location=<path>footnote:shardedDb[]
|label:new[Introduced in 2025.12] Location for target backup artifact data. Used together with `--target-format=backup`.
|
...
The following examples show how to use `neo4j-admin database copy` to copy a database, filter out data while copying a database, and further compact an existing database.
For examples of how to use `neo4j-admin database copy` to create a sharded property database, see xref:scalability/sharded-property-databases/resharding-databases.adoc[Resharding databases].
...
[[split-backup-archive]]
=== Split a backup archive into multiple files

Starting with Neo4j 2026.09, you can split the backup archive created by `neo4j-admin database copy` into multiple files using the `--split-archive-part-size` option.

Run the following command to split the backup into parts of up to 500GB:

[source, shell]
----
neo4j-admin database copy mydatabase mydatabase-copy \
    --to-path=/path/to/backup \
    --temp-path=/path/to/preferred/staging/area \
    --target-format=backup \
    --split-archive-part-size=500G
----

Note that you can split the backup archive only be if `--target-format` is set to `backup`.

The `--split-archive-part-size` option defines the size of each split part.
...
When `server.split_archive.part_size` is configured, `neo4j-admin database copy` uses that value by default.
