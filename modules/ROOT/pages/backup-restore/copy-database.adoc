...
|--property-shard-count=<numSpdPropertyDataShards>footnote:spd[See xref:scalability/sharded-property-databases/data-ingestion.adoc#splitting-existing-db-into-shards[Property Sharding -> Splitting an existing database into shards] for details.]
|label:new[Introduced in 2025.12]label:enterprise[Enterprise edition] Number of shards of property data that will be created, each shard will be its own database.
|0
