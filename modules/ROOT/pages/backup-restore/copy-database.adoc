...
You can use `neo4j-admin database copy` to filter out data when copying a database, for example, by removing nodes, labels, properties, and relationships.
...
To use this command, you must specify the `<toDatabase>` and `<fromDatabase>` values.
If you do not need an actual copy of the database, you can keep the values the same.
