...
`<toDatabase>` -- Name of the target database. If this is the same as `<fromDatabase>` the database will be copied to a temporary location before being moved to replace the original.
...
====
You can use `neo4j-admin database copy` to compact an existing database.
If you do not need an actual copy of the database, you can keep the values of `<toDatabase>` and `<fromDatabase>` the same.
The command will replace the original database with the newly created copy.

However, this resulting database will still be completely new, meaning you must recreate the indexes with the generated script, and if the database belongs to a cluster, reseed the cluster with that database.
...
. Copy the `neo4j` database in place using the command:
+
[source, shell]
----
