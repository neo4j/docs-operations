...
[NOTE]
====
For versions before Neo4j 5.20, any additional _schema_ (indexes and constraints) defined are not included and must be recreated manually by running the Cypher statements output by the command.

From Neo4j 5.20 onwards, you can use the `--copy-schema` option to automatically copy the schema as well.
For the full description of the `--copy-schema` option, see <<copy-database-command-options, Options>>.
====
