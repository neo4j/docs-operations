...
It dumps only a database content into a single-file archive, called _<database>.dump_, and stores it in the xref:configuration/file-locations.adoc#data[_<NEO4J_HOME>/data_] directory.
Note that the `neo4j-admin database dump` command **does not** support backing up database metadata.
