...
Even when `--to-path` points directly to a cloud storage bucket (for example, an AWS S3, Google Cloud Storage, or Azure bucket), the backup process still requires local disk space.
The backup client first downloads the database store files and transaction logs to a temporary local directory and only then streams them as a backup artifact to the cloud destination.
Consequently, the temporary location must have at least as much free space as the size of the database being backed up.
...
Therefore, it is strongly recommended to provide a `--temp-path` option, pointing to a location with sufficient free disk space, when executing a backup-related command, especially if the folder provided in the `--path` option points to a cloud storage bucket.
====
