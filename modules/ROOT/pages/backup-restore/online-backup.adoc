...
The following are examples of how to perform a full and a differential backup of a single database, and how to back up multiple databases.
The target directory _/mnt/backups/neo4j_ must exist before calling the command and the database(s) must be online.

.Use `neo4j-admin database backup` to perform a full backup of a single database.
====
[source,shell]
----
bin/neo4j-admin database backup --type=full --to-path=/tmp/backups/neo4j neo4j
----
====

.Use `neo4j-admin database backup` to perform a differential backup of a single database.
====
[source,shell]
----
bin/neo4j-admin database backup --type=diff --to-path=/tmp/backups/neo4j neo4j
----
The produced backup artifact contains transaction logs that can be replayed and applied to stores contained in full backup artifacts when restoring a backup chain.
====

.Use `neo4j-admin database backup` to back up multiple databases.
====
To back up several databases that match database pattern you can use name globbing.
