...
When `--to-path` points to a cloud storage bucket, the backup process still requires a temporary local directory to store the database store files and transaction logs before creating and streaming the backup artifact to the cloud destination.
...
[NOTE]
====
To avoid taking a backup from a database member that is lagging behind, you can look at the transaction IDs by exposing Neo4j metrics or via Neo4j Browser.
...
=== Back up a database to cloud storage
...
The store files and transaction logs are copied to a local temporary directory (configured with `--temp-path`) before being streamed to the cloud destination.
Therefore, the temporary location must have free space at least equal to the size of the database being backed up.
