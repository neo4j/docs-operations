...
`--from-path` must be a directory containing an archive(s).
An archive can be a database dump created with the dump command, or a full backup artifact created by the backup command in Neo4j Enterprise.
If neither `--from-path` nor `--from-stdin` is supplied, the `server.directories.dumps.root` setting will be searched for the archive.
...
The `--info` argument does not load the database. Instead, it prints information about the archive, such as file count, byte count, and format of the load file.
