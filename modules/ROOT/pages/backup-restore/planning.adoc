...
Starting with Neo4j 2026.09, you can split a backup archive into multiple files using the `--split-archive-part-size` option.
The option is available with the following commands:
...
To configure a default split size, you can also use the xref:configuration/configuration-settings.adoc#config_server.split_archive.part_size[`server.split_archive.part_size` setting].
...
If `server.split_archive.part_size` is configured, `neo4j-admin` commands use that value by default.
