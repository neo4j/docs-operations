...
In 2026.09, Neo4j introduces splitting a backup into multiple files, allowing large backups to be stored and transferred as a set of files rather than as a single file.
...
A single backup operation can produce multiple files, with the complete set representing the backup.

The feature is controlled by the `--split-archive-part-size` option, which is available for the following commands:
