...
[TIP]
====
Starting with Neo4j 2026.02, full and differential backups can be scheduled on independent cadences.
...
[IMPORTANT]
====
Verify the target directory permissions before running backup or dump command.
The backup directory must never be world-readable or world-executable.
Even if individual backup files inside are restricted, a world-executable directory can still expose sensitive information.

For details and intructions, refer to xref:configuration/file-locations.adoc#_security_considerations[Security considerations].
====
+
[TIP]
====
...
It uses xref:neo4j-admin-neo4j-tools.adoc[Neo4j Admin tool] commands that can be executed on a Neo4j DBMS, whether it is running or offline.
...
Starting with 2025.09, you can use the `--remote-address-resolution` option to let the DBMS select which servers to use as backup sources.
...
[[splitting-backup-archive]]
== Splitting a backup archive

By default, a backup is created as a single file.
This becomes a limitation for very large databases because cloud storage providers, such as Amazon S3, impose maximum object-size limits.
As Neo4j supports increasingly large data stores, including databases of 10 TB and larger, a complete backup may exceed the maximum size supported by the target storage system.

Splitting a backup into multiple files removes this limitation.
Starting with Neo4j 2026.09, a single backup operation can divide the backup archive into smaller parts, with the complete set representing the backup.
The `--split-archive-part-size` option controls the maximum size of each part and is available with the following commands:

* Community Edition
** xref:backup-restore/offline-backup.adoc#split-dump-archive[`neo4j-admin database dump`].
* Enterprise Edition
** xref:import/full-import.adoc[`neo4j-admin import full`].
** xref:import/incremental-import.adoc[`neo4j-admin import incremental`].
** xref:backup-restore/online-backup.adoc#split-backup-archive[`neo4j-admin database backup`].
** xref:backup-restore/aggregate.adoc#split-consolidated-backup-archive[`neo4j-admin backup aggregate`].
** xref:backup-restore/copy-database.adoc#split-backup-archive[`neo4j-admin database copy`].

The `--split-archive-part-size` option defines the maximum size of each split part.

If you regularly run `neo4j-admin` commands and want to use the same split size without specifying `--split-archive-part-size` for every command, you can configure the xref:configuration/configuration-settings.adoc#config_server.split_archive.part_size[`server.split_archive.part_size`] setting.
When this setting is configured, `neo4j-admin` commands use its value by default, unless the `--split-archive-part-size` option is specified explicitly.
