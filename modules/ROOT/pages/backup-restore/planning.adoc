...
It uses <<neo4j-admin, Neo4j Admin tool>> commands, which can be run from a live, as well as from an offline Neo4j DBMS.
All `neo4j-admin` commands must be invoked as the `neo4j` user to ensure the appropriate file permissions.

* `neo4j-admin database backup/restore` (Enterprise only) -– used for performing online backup (<<full-backup, full>> and <<differential-backup, differential>>) and restore operations.
...
For more information, refer to the <<backup-server-configuration, security configurations guide>>.
====
+
[TIP]
====
...
See <<show-databases-monitoring-listing-single, Listing a single database>> for more information.
...
[NOTE]
====
The Neo4j Admin commands `backup`, `restore`, `dump`, `load`, `copy`, and `check-consistency` are not supported for use on xref:composite-databases/introduction/index.adoc#composite-databases-concepts[composite databases].
...
* The <<file-locations, _neo4j.conf_>> file.
...
The locations of these are described in <<ssl-framework>>.
