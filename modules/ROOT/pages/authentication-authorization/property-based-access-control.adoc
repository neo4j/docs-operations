...
or in a more concise form, for example, for granting a property-based privilege to a role, you can use the following syntax:
...
The following forms are available in Cypher 25:
...
label:new[Introduced in 2026.10]::
...
See <<negation-and-deny-with-abac-native-user-tags>> for details.
...
Unlike a rule that checks the property against a fixed list of literals, the list of values is dynamically resolved per user from their tags, so a single privilege can grant different access to different users based on how they are tagged.
...
[NOTE]
====
The `abac.native.user_tags()` function is the only ABAC function permitted on the right-hand side of `IN` in a property rule.
Other ABAC accessor functions, such as `abac.oidc.user_attribute()`, are not allowed in property rules.
====

=== Configuring the `native` provider for ABAC

To use `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-security-native-user-tags[abac.native.user_tags()]` in a property rule, you must configure the `native` provider as an attribute-based access control (ABAC) authorization provider in both xref:configuration/configuration-settings.adoc#config_dbms.security.abac.authorization_providers[`dbms.security.abac.authorization_providers`] and xref:configuration/configuration-settings.adoc#config_dbms.security.authorization_providers[`dbms.security.authorization_providers`].
For details on configuring ABAC providers, see xref:authentication-authorization/attribute-based-access-control.adoc[Attribute-based access control].
...
=== Resolving `abac.native.user_tags()` for externally authenticated users
...
[[negation-and-deny-with-abac-native-user-tags]]
=== Negation and DENY with `abac.native.user_tags()`
...
=== Example of granting access based on user tags
