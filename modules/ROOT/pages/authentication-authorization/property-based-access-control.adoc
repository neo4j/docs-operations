...
[CAUTION]
====
These `DENY` rules fail open for nodes without a `classification` property.
...
The following example shows how to grant permission to `READ` all properties on nodes where the property `classification` is included in the list `[UNCLASSIFIED, PUBLIC]`:
