...
If the `native` provider is not configured as an ABAC authorization provider, the `GRANT` or `DENY` administration command still succeeds, but the property rule is not applied when a user's access is evaluated, and so it has no effect, because the `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-security-native-user-tags[abac.native.user_tags()]` function cannot be resolved.
