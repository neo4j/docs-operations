...
The command produces a row per role per auth rule and yields an `authRule` column, in addition to the one returned by `SHOW ROLES AS COMMANDS`, containing the name of the auth rule assigned to the role.
