...
label:new[Introduced in 2026.08] When using the `AS COMMAND` clause, the result will be the commands to create the roles.
...
You can view the commands to create all available roles using the Cypher command `SHOW ROLES AS COMMANDS`, which returns a single column by default.
Optionally, you can also use `SHOW ROLES AS COMMANDS YIELD *` to get the role name for filtering and to see if the role is immutable.
See <<access-control-immutable-roles, Immutable roles>> for more information.
...
To list all roles as commands, use the `SHOW ROLES AS COMMANDS` command:
