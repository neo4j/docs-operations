...
| member
| User name, or null if the role is not assigned to any users.
| STRING
|===
...
The table of results will show information about the role and what user it is assigned to:
...
To see which auth rules are assigned to which roles, `WITH AUTH RULES` can be added to the command.
The command produces a row per role per auth rule and yields the following column in addition to the one output by `SHOW ROLES`:

.`SHOW ROLES WITH AUTH RULES` output
[options="header", width="100%", cols="2a,4,2m"]
|===
| Column
| Description
...
| authRule
| Auth rule name, or null if the role is not assigned to any auth rules.
| STRING
|===

Since this gives a result with one row for each auth rule, it shows up twice if a role is assigned to two auth rules.

.Show roles with auth rule
====
[source, cypher, role=noplay]
----
...
The table of results will show information about the role and what auth rule it is assigned to:
