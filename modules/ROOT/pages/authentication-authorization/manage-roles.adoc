...
[source, syntax, role="noheader"]
----
REVOKE ROLE[S] name[, ...] FROM {AUTH RULE authrule[, ...]
    \| [USER] user[, ...]}
----

| Description
a|
Removes roles from users or auth rules.
...
This section explains how roles can be directly granted to and revoked from users. For information on how roles can be assigned to users based on attribute-based rules, see xref:authentication-authorization/attribute-based-access-control.adoc[].
