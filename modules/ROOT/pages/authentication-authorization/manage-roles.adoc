...
label:new[Introduced in 2026.08] label:cypher[Cypher 25] When using the `AS COMMAND` clause, the result will be the commands to create the roles.

For more information, see xref:authentication-authorization/manage-roles.adoc#access-control-list-roles[Showing roles].
...
label:new[Introduced in 2026.08] label:cypher[Cypher 25] When using the `AS COMMAND` clause, the result will be the commands to assign the users to the roles.
...
label:new[Introduced in 2026.08] label:cypher[Cypher 25] When using the `AS COMMAND` clause, the result will be the commands to assign the auth rules to the roles.
...
Note that this is only returned for `SHOW ROLES WITH USERS AS COMMANDS`.
| STRING
| {check-mark}
...
Note that this is only returned for `SHOW ROLES WITH AUTH RULES AS COMMANDS`.
| STRING
| {check-mark}
...
To list the commands used to assign users to roles use the `SHOW ROLES WITH USERS AS COMMANDS`.
...
The result is already filtered, so adding the `POPULATED` clause will have no effect.
...
To see the commands to assign auth rules to roles, add `WITH AUTH RULES` to the command:
