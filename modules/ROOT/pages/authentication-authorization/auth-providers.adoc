...
** Using RBAC to directly grant roles to the user.
** label:new[Introduced in 2026.06] Assigning tags to the user and using tag-based ABAC to conditionally assign roles to the user.
See xref:authentication-authorization/attribute-based-access-control.adoc[Attribute-based access control (ABAC)] for more information.
...
If there is a match, then the user is linked to the Neo4j user and authorized according to the <<database-management-system, DBMS>> security configuration settings that match the name of the matching auth provider.
