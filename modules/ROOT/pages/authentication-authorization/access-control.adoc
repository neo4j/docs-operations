...
When creating a database, administrators may want to establish which users have the ability to access certain information or not.
...
`(:Symptom)-[:OF]->(:Disease)`::
Symptoms is part of a graph of diseases and their symptoms.
...
* *IT administrators* who need to manage the database, for instance to create, and assign users.
...
With the `receptionist` role, Bob can delete any new patient nodes he has just created, but he is not able to delete patients that have already received diagnoses since those are connected to parts of the graph that Bob cannot see. 
