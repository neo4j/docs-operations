...
`(:Symptom)-[:OF]->(:Disease)`::
Symptoms are part of a graph of diseases and their symptoms.
...
Consider Charlie from the example of users. As a researcher, they will not need write access to the database, so they are assigned the `reader` role.
...
However, this would have the unfortunate side effect of allowing him the ability to create other nodes, like new `Symptom` nodes, even though they would subsequently be unable to find or read those same nodes.
...
Either they need to ask Tina the `itadmin` for help for this task, or you can add more privileges to the `receptionist` role:
