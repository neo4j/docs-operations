...
See xref:authentication-authorization/limitations.adoc#access-control-limitations-non-existing-labels[Authentication and authorization -> Manage Privileges -> Limitations -> Privileges and non-existing labels, relationship types and property names] for more information.
====
