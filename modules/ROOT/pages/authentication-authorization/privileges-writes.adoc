...
For more details, see link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/clauses/create.adoc[the Cypher Manual -> `CREATE`] clause.
...
For more details, see link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/clauses/delete.adoc[the Cypher Manual -> `DELETE`] clause.
...
See href:tutorial/access-control.adoc#detach-delete-restricted-user[] for more info.
====
...
The `SET LABEL` privilege allows you to set labels on a node using the link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/clauses/set.adoc#set-set-a-label-on-a-node[Cypher `SET` clause]:
...
The `REMOVE LABEL` privilege allows you to remove labels from a node by using the link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/clauses/remove.adoc#remove-remove-a-label-from-a-node[Cypher `REMOVE` clause]:
...
The `SET PROPERTY` privilege allows a user to set a property on a node or relationship element in a graph by using the link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/clauses/set.adoc#set-set-a-property[Cypher `SET` clause]:
...
This is intended to enable the use of the link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/clauses/merge.adoc[Cypher `MERGE` command], but it is also applicable to all reads and writes that require these privileges.
...
See xref:tutorial/access-control#detach-delete-restricted-user[] for more info.
====
