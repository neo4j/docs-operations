...
[NOTE]
====
The functions, `date`, `datetime`, `localdatetime`, `localtime`, and `time` are only supported when you use an input argument to specify the instance. This is because auth rules get evaluated at the instant that the transaction begins.
As far as auth rules are concerned, there is no concept of statement time or real time. To get a temporal instance without specifying the temporal instance, use any of the `*.transaction()` functions.
====
