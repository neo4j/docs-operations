...
1. Specify which OIDC provider(s) will be used for attribute-based access control by setting the xref:configuration/configuration-settings.adoc#config_dbms.security.abac.authorization_providers[`dbms.security.abac.authorization_providers`] setting to the appropriate providers.
2. Define the authorization rules that will be used to grant roles based on user attributes by using the Cypher command `CREATE AUTH RULE`, explained below.
3. Specify which roles will be assigned when the authorization rules are fulfilled using the `GRANT ROLE` command, explained below.
...
[[create-auth-rule-examples]]
=== Examples

An auth rule specifying that users with the attribute `department` equal to `sales` should be granted the associated roles can be created as follows:
...
Creating an auth rule specifying users with the attribute `department` equal to `engineering` and `location` equal to `UK` can be done as follows:
...
Creating an auth rule with a condition based on the presence of the allowed countries in a list of citizenship countries in the claims can be done as follows:
...
Creating an auth rule that temporarily grants assigned roles based on the value of a claim and the time being between 1900 and 2000 can be done as follows:
...
Assigning roles to the auth rules from the xref:authentication-authorization/attribute-based-access-control.adoc#create-auth-rule-examples[create auth rule examples] can be done as follows:
...
Revoking the `reader` role from auth rule `temporary_reader` can be done as follows:
...
Optionally, you can also use `SHOW AUTH RULES AS COMMANDS YIELD *` to see which roles has been granted to the auth rule.
...
[NOTE]
====
To be able to list auth rules, the user must have the `SHOW AUTH RULE` privilege,
see xref:authentication-authorization/dbms-administration/dbms-auth-rule-management-privileges.adoc#grant-list-auth-rule-privilege[DBMS SHOW AUTH RULE privilege].
====
...
[NOTE]
====
To be able to modify auth rules, the user must have the `ALTER AUTH RULE` privilege,
...
Modifying an auth rule can be done as follows:
...
This will replace the condition for the auth rule `ruleset_countries` with the new condition that also allows the country `'c6'`.

Modifying the enablement of the auth rule can be done as follows:
...
Which will turn off evaluation off the auth rule `ruleset_countries`.
...
Deleting the auth rule `temporary_reader` can be done as follows:
