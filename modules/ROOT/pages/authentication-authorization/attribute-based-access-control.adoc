...
To create an authorization rule, use the `CREATE AUTH RULE` command with the following syntax:
...
+
The `conditionExpression` can use any valid Cypher expression that evaluates to a Boolean value, including logical operators (`AND`, `OR`, `NOT`, etc), comparison operators (`=`, `<>`, `<`, `>`, `<=`, and `>=`), and the following build-in functions:
+
...
The following example creates an auth rule that temporarily grants the assigned roles to users with the attribute `jobTitle` equals `cover`, between the hours of 19:00 and 20:00:
...
To specify which roles to be granted to the user when the authorization rule is fulfilled, use the `GRANT ROLE` command with the following syntax:

[NOTE]
====
The `GRANT ROLE` command requires the `ASSIGN ROLE` privilege.
See xref:authentication-authorization/dbms-administration/dbms-role-management-privileges.adoc#access-control-dbms-administration-role-assignment[Grant privilege to create roles] for details on how to grant this privilege.
====
...
| condition
| A Cypher expression that is evaluated when determining whether to grant the user the roles.
| STRING
...
To return the commands used to create the auth rules, you can use the `SHOW AUTH RULES AS COMMANDS`.
Optionally, you can also append `YIELD *` to see which roles has been granted to the auth rule.
...
To modify an existing authorization rule, use the following syntax:
...
The following example modifies the condition of the auth rule `ruleset_countries` to also allow the country `'c6'`:
