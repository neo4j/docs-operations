...
Attribute-based access control (ABAC) grants roles based on the evaluation of user attributes. These could be claims contained in a user's authentication token, or tags attached to a native user object.
...
. In the _neo4j.conf_ file, set xref:configuration/configuration-settings.adoc#config_dbms.security.abac.authorization_providers[`dbms.security.abac.authorization_providers`] to be the list of authorization provider(s) you want to use for ABAC. These can include any of the OIDC providers as well as the `native` provider. In order for a provider to be allowed in the list of xref:configuration/configuration-settings.adoc#config_dbms.security.abac.authorization_providers[`dbms.security.abac.authorization_providers`], it must also appear in the list of xref:configuration/configuration-settings.adoc#config_dbms.security.authorization_providers[`dbms.security.authorization_providers`].
+
[NOTE]
====
...
In contrast, a rule that calls the metadata accessor function of a provider that is not in the list will fail. When a provider is not listed in xref:configuration/configuration-settings.adoc#config_dbms.security.abac.authorization_providers[`dbms.security.abac.authorization_providers`], its relevant accessor function (for example, `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-security-oidc-user-attribute[abac.oidc.user_attribute()]` for an OIDC provider, or `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-security-native-user-tags[abac.native.user_tags()]` for the `native` provider) is not available at all, so any rule referencing it intentionally fails rather than silently evaluating to a default, meaning that the rule cannot be fulfilled and the user cannot receive the roles from that rule.
====
...
====
When using native user tags in auth rules, avoid writing rules whose condition is solely based on the _absence_ of a tag from a user, for example `NOT ('restricted' IN abac.native.user_tags())`. See <<example-avoid-absence-of-tags>> for an illustration.
...
Such a user satisfies an "absence of tag" condition and is therefore granted the roles associated with the rule, which can lead to an unintended escalation of privileges.
...
* Additionally (if using `abac.oidc.user_attribute()`) turn on JWT claims logging at debug level in the _neo4j.conf_ file by setting `dbms.security.logs.oidc.jwt_claims_at_debug_level_enabled=true`. This allows you to see the claims that are being evaluated in the auth rules.