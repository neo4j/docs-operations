...
As well as evaluating claims from an OIDC token, auth rules can evaluate the xref:authentication-authorization/manage-users.adoc#_create_a_user_with_tags[tags] attached to a native user using the link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-security-native-user-tags[`abac.native.user_tags()`] function.
