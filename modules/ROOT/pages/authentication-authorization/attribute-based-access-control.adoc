...
+
The `conditionExpression` is a Boolean Cypher expression that evaluates conditions against user attributes. These attributes could be claims contained in the users OIDC token (accessed via the `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-oidc_attributes[abac.oidc.user_attribute()]` function), or tags on the native user object (accessed via the `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-native_tags[abac.native.user_tags()]` function).
+
...
| Security functions
| link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-oidc_attributes[abac.oidc.user_attribute()], link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-native_tags[abac.native.user_tags()]
...
* Avoid mixing time values from authentication token claims (via `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-oidc_attributes[abac.oidc.user_attribute()]`) with server time functions such as `datetime.transaction()` or `time()`.
...
As well as evaluating claims from an OIDC token, auth rules can evaluate the tags attached to a native user using the `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-native_tags[abac.native.user_tags()]` function.
...
Because `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-native_tags[abac.native.user_tags()]` returns a list, you can combine it with the list predicate functions to require several tags at once.
...
For this rule to evaluate `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-native_tags[abac.native.user_tags()]` for the OIDC-authenticated user, that user must be linked to a native user — see <<linking-external-users-to-native, linking external users>>.
...
[NOTE]
====
For an auth rule to evaluate `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-native_tags[abac.native.user_tags()]` for an externally authenticated user (for example, an OIDC or LDAP user), the external auth must be linked to a native user so that the user's tags can be resolved.
...
* using xref:authentication-authorization/auth-providers.adoc#access-control-auth-providers[user auth providers] to explicitly associate the external identity with a native user.
====
...
* When evaluating `link:{neo4j-docs-base-uri}/cypher-manual/current/functions/security/#functions-oidc_attributes[abac.oidc.user_attribute()]`, if the claim does not exist in the authentication token, it will evaluate to `NULL`.
