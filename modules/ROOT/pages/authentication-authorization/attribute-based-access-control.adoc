...
If specifying `OR REPLACE`, if a auth rule with the same name exists, replace it. Any assigned roles associated with the old auth rule will not be associated to the new auth rule.
...
| enabled
| Whether the auth rule is enabled or not. This column will have the value `true` when the auth rule is enabled, and `false` when it is not. When an auth rule is disabled it will not be evaluated and so users cannot receive its roles
| BOOLEAN
