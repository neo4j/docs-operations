...
** Manage link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/[indexes] and link:{neo4j-docs-base-uri}/cypher-manual/current/schema/constraints/[constraints].
...
[[access-control-built-in-roles-admin-recreate-after-drop]]
...
. Using a client such as xref:cypher-shell.adoc[Cypher Shell] or the Neo4j Browser, connect as a user with rights to manage roles and privileges:
+
[source, shell]
----
bin/cypher-shell -d -u <username> -p <password>
----
...
[[access-control-built-in-roles-admin-recreate-after-modification]]
...
. Create a new `admin2` role:
+
[source, cypher]
----
CREATE ROLE admin2;
----
. Run the following commands to recreate the `admin2` role with the original capabilities of the built-in `admin` role:
+
[source, cypher, role=noplay test-skip]
----
...
. Assign the `admin2` role to the users that you want to have the same privileges as the original `admin` role:
+
[source, cypher]
----
...
DROP ROLE admin;
----
. Finally, you can rename the `admin2` role to `admin`:
+
[source, cypher, role=noplay]
----
