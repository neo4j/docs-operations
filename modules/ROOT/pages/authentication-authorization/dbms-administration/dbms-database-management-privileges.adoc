:description: Details the DBMS privileges for database management.
...
The <<database-management-system, DBMS>> privileges for database management can be granted, denied, or revoked like other privileges.
...
GRANT [IMMUTABLE] CREATE COMPOSITE DATABASE
  ON DBMS
  TO role[, ...]
| Enables the specified roles to create new <<composite-database, composite databases>>.
