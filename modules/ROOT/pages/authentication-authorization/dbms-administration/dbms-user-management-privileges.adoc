...
CREATE ROLE userCommandViewer IF NOT EXISTS;
...
GRANT [IMMUTABLE] SHOW USER CREDENTIAL[S]
  ON DBMS
  TO role[, ...]
| label:new[Introduced in 2026.09] label:cypher[Cypher 25] Enables the specified roles to list users as commands (together with the `SHOW USER` privilege).
...
To show all privileges for the role `userAdder` as commands, use the following query:
...
To show all privileges for the role `userNameModifier` as commands, use the following query:
...
To show all privileges for the role `userModifier` as commands, use the following query:
...
To show all privileges for the role `passwordModifier` as commands, use the following query:
...
To show all privileges for the role `statusModifier` as commands, use the following query:
...
To show all privileges for the role `homeDbModifier` as commands, use the following query:
...
To show all privileges for the role `userDropper` as commands, use the following query:
...
You can grant the privilege to show users using the `SHOW USER`.
For example:
...
To show all privileges for the role `userViewer` as commands, use the following query:
...
== Grant privilege to show users as commands

You can grant the privilege to show users as commands using the `SHOW USER CREDENTIALS` privilege.
To fully call the command the user also needs the `SHOW USER` privilege. +
For example:

[source, cypher, role=noplay]
----
GRANT SHOW USER CREDENTIALS ON DBMS TO userCommandViewer;
GRANT SHOW USER ON DBMS TO userCommandViewer;
----

As a result, the `userCommandViewer` role has privileges that allow showing users and showing users as commands.
To show all privileges for the role `userCommandViewer` as commands, use the following query:

[source, cypher, role=noplay]
----
SHOW ROLE userCommandViewer PRIVILEGES AS COMMANDS;
----
...
To show all privileges for the role `userManager` as commands, use the following query:
