...
GRANT [IMMUTABLE] SHOW USER CREDENTIAL[S]
  ON DBMS
  TO role[, ...]
| label:new[Introduced in 2026.08] label:cypher[Cypher 25] Enables the specified roles to list users and list users as commands.
...
You can grant the privilege to show users using the `SHOW USER` or, starting with 2026.08, also the `SHOW USER CREDENTIALS` privilege. +
For example:
