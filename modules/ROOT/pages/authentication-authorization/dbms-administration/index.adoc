...
All <<database-management-system, DBMS>> privileges are relevant system-wide.
...
* Set and show user metadata.
* Create, alter, rename, drop and show auth rules.
...
To show all privileges for the role `dbmsManager` as commands, use the following query:
