...
:page-role: new-2026.03 enterprise-edition aura-db-business-critical aura-db-dedicated
...
CREATE ROLE authRuleModifier IF NOT EXISTS;
CREATE ROLE authRuleDropper IF NOT EXISTS;
CREATE ROLE authRuleViewer IF NOT EXISTS;
----
////

The <<database-management-system, DBMS>> privileges for auth rule management can be granted, denied, or revoked like other privileges.
...
You can grant the privilege to modify auth rules using the `ALTER AUTH RULE` privilege.
For example:
