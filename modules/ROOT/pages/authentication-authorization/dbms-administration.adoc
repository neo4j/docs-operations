...
=== Create a custom administrator role by creating a new role from scratch
...
----
. Explicitly deny the privilege to manage databases and aliases:
+
[source, cypher, role=noplay]
----
...
The DBMS privileges for role management can be granted, denied, and revoked like other privileges.
...
As a result, the `roleShower` role has privileges that only allow showing roles.
To list all privileges for the role `roleShower` as commands, use the following query:
...
The DBMS privileges for user management can be granted, denied, and revoked like other privileges.
...
As a result, the `userShower` role has privileges that only allow showing users.
To list all privileges for the role `userShower` as commands, use the following query:
...
The DBMS privileges for impersonation can be granted, denied, and revoked like other privileges.
...
As a result, the `userImpersonator` role has privileges that allow impersonating all users.
...
The DBMS privileges for database management can be granted, denied and revoked like other privileges.
...
=== Grant privilege to modify access to composite databases

You can grant the privilege to modify access to composite databases using the `ALTER COMPOSITE DATABASE` privilege.
For example:
...
=== Grant privilege to modify the default language of composite databases

You can grant the privilege to modify the default language of composite databases using the `SET COMPOSITE DATABASE DEFAULT LANGUAGE` privilege.
For example:
...
As a result, the `aliasLister` role has privileges that only allow modifying aliases.
To list all privileges for the role `aliasLister` as commands, use the following query:
...
The DBMS privileges for server management can be granted, denied, and revoked like other privileges.
...
As a result, the `serverLister` role has privileges that only allow showing servers.
To list all privileges for the role `serverLister` as commands, use the following query:

[source, cypher, role=noplay]
----
SHOW ROLE serverLister PRIVILEGES AS COMMANDS
----
...
The DBMS privileges for privilege management can be granted, denied and revoked like other privileges.
...
As a result, the `privilegeShower` role has privileges that only allow showing privileges.
To list all privileges for the role `privilegeShower` as commands, use the following query:
...
The DBMS privileges for procedure and user-defined function execution can be granted, denied and revoked like other privileges.
...
As a result, the `procedureExecutor`  role has privileges that only allow executing procedures in the `db.schema` namespace.
...
=== Grant privilege to execute specific procedures

You can grant the privilege to execute specific procedures using `EXECUTE PROCEDURES *` and deny the unwanted procedures.
...
As a result, the `deniedProcedureExecutor` role has privileges that only allow executing all procedures except those starting with `dbms.cluster`.
...
=== Grant privilege to execute boosted procedures
For example:
...
You can grant the privilege to execute admin procedures (annotated with `@Admin`) using the `EXECUTE ADMIN PROCEDURE` privilege. +
...
The function in the example is run using the user's own privileges.
...
=== Grant privilege to execute specific user-defined functions
...
As a result, the `deniedFunctionExecutor` role has privileges that only allow the execution of all procedures except those starting with `apoc.any.prop`.
...
You can elevate the privilege to execute user-defined functions (UDFs) by granting the `EXECUTE BOOSTED USER DEFINED FUNCTION` privilege. +
...
As a result, the `deniedConfigurationViewer` role has privileges that allow querying all settings except those starting with `dbms.security`.
