...
----
CREATE ROLE noAccessUsers IF NOT EXISTS;
CREATE ROLE regularUsers IF NOT EXISTS;
CREATE ROLE regularAuthRules IF NOT EXISTS;
CREATE USER jake SET PASSWORD 'abcd1234' CHANGE NOT REQUIRED;
CREATE USER joe SET PASSWORD 'abcd5678' CHANGE NOT REQUIRED;
CREATE AUTH RULE salesRule IF NOT EXISTS
  SET CONDITION abac.oidc.user_attribute('department') = 'sales';
CREATE AUTH RULE engineeringUKRule IF NOT EXISTS
  SET CONDITION abac.oidc.user_attribute('department') = 'engineering' AND abac.oidc.user_attribute('location') = 'UK';
DENY ACCESS ON DATABASE neo4j TO `noAccessUsers`;
GRANT ACCESS ON DATABASE neo4j TO `regularUsers`, `regularAuthRules`;
GRANT ROLE `regularUsers` TO `jake`;
GRANT ROLE `noAccessUsers` TO `joe`;
GRANT ROLE `regularAuthRules` TO AUTH RULE `salesRule`, `engineeringUKRule`;
GRANT ROLE `reader` TO AUTH RULE `salesRule`;
GRANT ROLE `architect` TO AUTH RULE `engineeringUKRule`;
----
////
...
Role-based access control (_RBAC_) is a secure method for granting users access by defining roles with specific privileges and then mapping users to those roles.
These privileges define what users can and cannot do within the database, such as which data they can read or write, and which administrative actions they can perform.
Therefore, it is very important to apply the principle of least privilege when defining user roles and privileges, so that users are not granted more access than necessary.
Or in other words, users should be granted only the minimum level of access necessary to perform their tasks, and no more.
...
Note that defining privileges for a specific graph or property shard is not supported.
...
[NOTE]
====
Privileges may not be denied to a role that has already been assigned to an auth rule.
For more information about auth rules, see xref:authentication-authorization/attribute-based-access-control.adoc[Attribute-based access control].
====
...
It shows the privileges that are possible to grant or deny on a server, together with the structure of the privilege.
...
.Show auth rule privileges syntax
[cols="<15s,<85"]
|===

| Command
m| label:cypher[Cypher 25] label:new[Introduced in 2026.09] +SHOW AUTH RULE ... PRIVILEGE+

| Syntax
a|
[source, syntax, role="noheader", indent=0]
----
SHOW AUTH RULE[S] name[, ...] PRIVILEGE[S] [AS [REVOKE] COMMAND[S]]
  [YIELD { * \| field[, ...] } [ORDER BY field[, ...]] [SKIP n] [LIMIT n]]
  [WHERE expression]
  [RETURN field[, ...] [ORDER BY field[, ...]] [SKIP n] [LIMIT n]]
----
...
| Description
| List privileges for a specific auth rule.
...
For an easy overview of the existing privileges, it is recommended to use the `AS COMMANDS` version of the `SHOW` command, which returns the privilege as a string of the `GRANT` or `DENY` command.
It also has a few other columns that can be useful for filtering.
...
| access
| label:cypher[Cypher 25] label:new[Introduced in 2026.09] Whether the privilege is granted or denied.
| STRING
|

| action
| label:cypher[Cypher 25] label:new[Introduced in 2026.09] The type of the privilege.
E.g., traverse, read, index, or role_management.
| STRING
|

| roles
| label:cypher[Cypher 25] label:new[Introduced in 2026.09] The roles the privilege is granted to.
| LIST<STRING>
|
...
| users
| label:cypher[Cypher 25] label:new[Introduced in 2026.09] The users the privilege belongs to.
...
| authRules
| label:cypher[Cypher 25] label:new[Introduced in 2026.09] The auth rules the privilege belongs to.

Note that this is only returned for `SHOW AUTH RULE ruleName PRIVILEGES`.
| LIST<STRING>
| {check-mark}
|===
...
| authRule
| label:cypher[Cypher 25] label:new[Introduced in 2026.09] The auth rule the privilege belongs to.
...
| "GRANTED" | "access"                 | "database"       | "neo4j" | "database"        | "regularAuthRules" | FALSE     |
...
| "regularAuthRules" | "GRANTED" | [["neo4j", "database", "database", "access"]]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| "GRANT ACCESS ON DATABASE `neo4j` TO `regularAuthRules`"    |
...
| "REVOKE GRANT ACCESS ON DATABASE `neo4j` FROM `regularAuthRules`"    |
...
Again, it is possible to show the privileges as revoking commands instead of granting or denying.
For more info about revoking privileges, please see xref:authentication-authorization/manage-privileges.adoc#access-control-revoke-privileges[The REVOKE command].
...
| command                                                  | users    |
+---------------------------------------------------------------------+
| "GRANT ACCESS ON DATABASE `neo4j` TO $role"              | ["jake"] |
| "GRANT ACCESS ON HOME DATABASE TO $role"                 | ["jake"] |
| "GRANT EXECUTE PROCEDURE * ON DBMS TO $role"             | ["jake"] |
| "GRANT EXECUTE USER DEFINED FUNCTION * ON DBMS TO $role" | ["jake"] |
| "GRANT LOAD ON ALL DATA TO $role"                        | ["jake"] |
+---------------------------------------------------------------------+
...
[source, cypher, role=noplay]
----
SHOW USER jake, joe PRIVILEGES AS REVOKE COMMANDS
...
| command                                                           | users           |
+-------------------------------------------------------------------------------------+
| "REVOKE GRANT EXECUTE PROCEDURE * ON DBMS FROM $role"             | ["jake", "joe"] |
| "REVOKE GRANT EXECUTE USER DEFINED FUNCTION * ON DBMS FROM $role" | ["jake", "joe"] |
+-------------------------------------------------------------------------------------+
...
[[access-control-list-privileges-auth-rules]]
=== Examples for showing privileges for specific auth rules

Available privileges for specific auth rules can be displayed using `SHOW AUTH RULE name PRIVILEGES`.
...
[source, cypher, role=noplay]
----
SHOW AUTH RULE salesRule PRIVILEGES;
----

Lists all privileges for auth rule `salesRule`.

.Result
[role="queryresult"]
----
+-------------------------------------------------------------------------------------------------------------------------------+
| access    | action            | resource         | graph   | segment           | role               | immutable | authRule    |
+-------------------------------------------------------------------------------------------------------------------------------+
| "GRANTED" | "match"           | "all_properties" | "*"     | "NODE(*)"         | "reader"           | FALSE     | "salesRule" |
| "GRANTED" | "match"           | "all_properties" | "*"     | "RELATIONSHIP(*)" | "reader"           | FALSE     | "salesRule" |
| "GRANTED" | "access"          | "database"       | "*"     | "database"        | "reader"           | FALSE     | "salesRule" |
| "GRANTED" | "show_constraint" | "database"       | "*"     | "database"        | "reader"           | FALSE     | "salesRule" |
| "GRANTED" | "show_index"      | "database"       | "*"     | "database"        | "reader"           | FALSE     | "salesRule" |
| "GRANTED" | "access"          | "database"       | "neo4j" | "database"        | "regularAuthRules" | FALSE     | "salesRule" |
+-------------------------------------------------------------------------------------------------------------------------------+
...
[source, cypher, role=noplay]
----
SHOW AUTH RULES salesRule, engineeringUKRule PRIVILEGES;
----

Lists all privileges for auth rules `salesRule` and `engineeringUKRule`.
...
| access    | action            | resource         | graph   | segment           | role               | immutable | authRule            |
+---------------------------------------------------------------------------------------------------------------------------------------+
| "GRANTED" | "match"           | "all_properties" | "*"     | "NODE(*)"         | "architect"        | FALSE     | "engineeringUKRule" |
| "GRANTED" | "write"           | "graph"          | "*"     | "NODE(*)"         | "architect"        | FALSE     | "engineeringUKRule" |
| "GRANTED" | "match"           | "all_properties" | "*"     | "RELATIONSHIP(*)" | "architect"        | FALSE     | "engineeringUKRule" |
| "GRANTED" | "write"           | "graph"          | "*"     | "RELATIONSHIP(*)" | "architect"        | FALSE     | "engineeringUKRule" |
| "GRANTED" | "access"          | "database"       | "*"     | "database"        | "architect"        | FALSE     | "engineeringUKRule" |
| "GRANTED" | "constraint"      | "database"       | "*"     | "database"        | "architect"        | FALSE     | "engineeringUKRule" |
| "GRANTED" | "index"           | "database"       | "*"     | "database"        | "architect"        | FALSE     | "engineeringUKRule" |
| "GRANTED" | "show_constraint" | "database"       | "*"     | "database"        | "architect"        | FALSE     | "engineeringUKRule" |
| "GRANTED" | "show_index"      | "database"       | "*"     | "database"        | "architect"        | FALSE     | "engineeringUKRule" |
| "GRANTED" | "token"           | "database"       | "*"     | "database"        | "architect"        | FALSE     | "engineeringUKRule" |
| "GRANTED" | "access"          | "database"       | "neo4j" | "database"        | "regularAuthRules" | FALSE     | "engineeringUKRule" |
| "GRANTED" | "match"           | "all_properties" | "*"     | "NODE(*)"         | "reader"           | FALSE     | "salesRule"         |
| "GRANTED" | "match"           | "all_properties" | "*"     | "RELATIONSHIP(*)" | "reader"           | FALSE     | "salesRule"         |
| "GRANTED" | "access"          | "database"       | "*"     | "database"        | "reader"           | FALSE     | "salesRule"         |
| "GRANTED" | "show_constraint" | "database"       | "*"     | "database"        | "reader"           | FALSE     | "salesRule"         |
...
As for the other privilege commands, available privileges for auth rules can also be listed as Cypher commands with the optional `AS COMMAND[S]`.

[NOTE]
====
When showing auth rule privileges as commands, the roles in the Cypher commands are replaced with a parameter.
This can be used to quickly create new roles based on the privileges of specific auth rules.
====

[source, cypher, role=noplay]
----
SHOW AUTH RULE salesRule PRIVILEGES AS COMMANDS;
----
...
| command                                              | authRules     |
+----------------------------------------------------------------------+
| "GRANT ACCESS ON DATABASE * TO $role"                | ["salesRule"] |
| "GRANT ACCESS ON DATABASE `neo4j` TO $role"          | ["salesRule"] |
| "GRANT MATCH {*} ON GRAPH * NODE * TO $role"         | ["salesRule"] |
| "GRANT MATCH {*} ON GRAPH * RELATIONSHIP * TO $role" | ["salesRule"] |
| "GRANT SHOW CONSTRAINT ON DATABASE * TO $role"       | ["salesRule"] |
| "GRANT SHOW INDEX ON DATABASE * TO $role"            | ["salesRule"] |
+----------------------------------------------------------------------+
...
[source, cypher, role=noplay]
----
SHOW AUTH RULE salesRule, engineeringUKRule PRIVILEGES AS REVOKE COMMANDS
YIELD *
WHERE action = 'access';
----
...
----
+--------------------------------------------------------------------------------------------------------------------------------------------------------+
| command                                              | access    | action   | roles                   | immutable | authRules                          |
+--------------------------------------------------------------------------------------------------------------------------------------------------------+
| "REVOKE GRANT ACCESS ON DATABASE * FROM $role"       | "GRANTED" | "access" | ["architect", "reader"] | FALSE     | ["engineeringUKRule", "salesRule"] |
| "REVOKE GRANT ACCESS ON DATABASE `neo4j` FROM $role" | "GRANTED" | "access" | ["regularAuthRules"]    | FALSE     | ["engineeringUKRule", "salesRule"] |
+--------------------------------------------------------------------------------------------------------------------------------------------------------+
