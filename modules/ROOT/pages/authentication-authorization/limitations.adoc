...
As described in link:{neo4j-docs-base-uri}/cypher-manual/5/indexes/search-performance-indexes/overview/[Cypher Manual -> Indexes for search performance], Neo4j {neo4j-version} supports the creation and use of indexes to improve the performance of Cypher queries.
...
However, this rule is not fully obeyed by link:{neo4j-docs-base-uri}/cypher-manual/5/indexes/semantic-indexes/full-text-indexes/[Cypher Manual -> Indexes for full-text search].
