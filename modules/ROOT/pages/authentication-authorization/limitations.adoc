...
Neo4j's role-based access control (RBAC) has some limitations that can lead to unexpected results if not carefully considered when designing the security model and writing Cypher queries.
...
* <<fail-open-deny-behavior, *Fail-open `DENY` behavior:*>> A `DENY` rule fails open when its criteria is not met, so Neo4j does not apply the restriction and it grants access by default if a broader `GRANT` exists. This can lead to unintended data exposure if the `DENY` rule is not carefully crafted.
...
link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/search-performance-indexes/[Search-performance indexes] are used automatically by the Cypher planner when they are relevant to a query.
...
link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/semantic-indexes/full-text-indexes/[Full-text indexes] and link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/semantic-indexes/vector-indexes/[vector indexes] work differently compared to the search-performance indexes, because they are built on Lucene and are not used automatically by the Cypher planner.
...
+
Starting with Neo4j 2026.01, the preferred way of using vector indexes is by using the Cypher {neo4j-docs-base-uri}/cypher-manual/current/clauses/search/[SEARCH] clause.
This avoids a situation where the same Cypher query returns different results because of an underlying index.
...
In previous versions, they can only be on a single property and a single label or relationship type.
...
A key difference is that full-text indexes are backed by _Lucene_, and uses the _Lucene_ syntax for querying.
...
Finally, the query returns two results and only one of them has the `surname` property.
...
Finally, the query returns three results with only one of them having the `surname` property.
======
...
=== An un-met `DENY` failing open with property-based RBAC
...
=== An un-met `DENY` failing open with label-based RBAC
...
----
This way, the user only sees the `salary` property on nodes that have the `IndividualContributor` label, and not on any other nodes.
====
...
To get information about about labels attached to a node by calling the built-in `labels()` function, a user needs to have the privilege to traverse on one or multiple labels attached to that node using `GRANT TRAVERSE` or `GRANT MATCH`.
...
Privileges for non-existent labels, relationship types, and property names take effect only after they are created.
...
This is because the graph elements must be resolved internally to check against privileges when users try to use them later.
...
. Define some privileges to the `custom` role that include a non-existing label `:A`:
+
[source, cypher]
----
...
. Then, connect as a user with the `custom` role, for example, `Alice`, and run the following query to create a node with the non-existing label `:A`:
+
[source, cypher]
----
CREATE (:A)
...
For example, security rules based on property or label combinations can lead to significant performance degradation, as Neo4j must check each node/relationship individually rather than using fast lookup structures like count stores or optimized index scans.
...
* Users with the `unrestricted` role -- the database accesses the count store to retrieves the total number of nodes with the label `:Person`, without needing to check each node against any security rules.
This is a very quick operation.

* User with the `restricted` role -- the database cannot access the count store because it must make sure that only traversable nodes with the desired label `:Person` are counted.
It needs to check each node with the `:Person` label whether it also has the `:Customer` label to deny access to it.
So due to the additional data access required by the security checks, this operation is slower compared to executing the query as an unrestricted user.

[[property-based-access-control-limitations]]
=== Access control based on properties rules
...
* User with `unrestricted` role -- the databases scans all nodes and quickly identifies accessible nodes based solely on the presence of the `:Customer` label.
This is a relatively quick operation.

* User with `restricted` role -- the database scans all nodes, identifies potentially accessible nodes based on the presence of the specified label, and then also accesses the properties of each of those nodes and inspects their values to ensure the property rule criteria are met (i.e., that `secret` is not set to `true` in this case).
So due to the additional data access required by the security checks, this operation is slower compared to executing the query as an unrestricted user.