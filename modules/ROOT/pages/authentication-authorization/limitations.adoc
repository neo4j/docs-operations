...
For example, when you use link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/search-performance-indexes/[search-performance indexes] (non–full-text and non-vector) indexes, queries return the same results they would without any index.
...
link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/semantic-indexes/full-text-indexes/[Full-text indexes] and link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/semantic-indexes/vector-indexes/[vector indexes] work differently.
...
So, if there is any chance a result might violate active security privileges for a query, Neo4j does not return that result from the full-text or vector indexes.
This mean it either returns zero results (if all results from the index is potentially affected) or a partial result (if some rows can be guaranteed to not be affected).

Also, Cypher does not use full-text or vector indexes automatically — you have to explicitly call procedures to use them.
From Neo4j 2026.01, in addition to procedure calls, it is possible to use the link:{neo4j-docs-base-uri}/cypher-manual/current/clauses/search/[SEARCH] clause for vector indexes.
...
The following full-text index examples also apply to the vector indexes.
Vector indexes with multiple labels, multiple relationship types or additional properties were introduced in Neo4j 2026.01, before then they could only be on a single property and a single label or relationship type.
...
Two results are returned from this dataset and only one of them has the `surname` property.
...
=== Example with denied properties on all labels

Consider denying read access on properties for all labels, like the `surname` property from the example in <<denied-labels-index-multiple-properties>>.
...
Using the same queries as in <<denied-labels-index-multiple-properties>>:
...
Hence, three results is returned with only one of them having the `surname` property.
...
Since the `surname` property is denied, it always appears to be `null` and the composite index empty.
...
As in <<denied-labels-index-multiple-properties>>, it is not clear whether the results returned by the index are due to a match on the `name` or `surname` property.
Results from the `surname` property now need to be excluded by the security rules, because they require the user to not be able to see any `surname` properties.
...
Therefore, in order to never return results the user should not be able to see, the index will consider all properties denied instead of just the `surname` property.
The steps taken by the query engine would be:

* Run a _Lucene_ query on the full-text index to produce results containing `ndy` in either property, leading to five results.
* Filter the results to exclude all nodes, filtering out everyone, leading to no result.

In this case, the query will return zero results rather than simply returning the results `Andy`, `Mandy` and `Sandy`, which might have been expected.

[[denied-properties-on-specific-label]]
=== Example with denied properties on a specific label

Instead of denying the `surname` property for all labels like the example in <<denied-properties-on-all-labels>>, consider deny reading it only for nodes with the `:Person` label.
...
And running the same queries again:
...
This query operates exactly as in <<denied-properties-on-all-labels>>, returning the same three results, because nothing in it relates to the denied property.
...
Since the `surname` property is denied only for nodes with label `:Person`, it will only appear to be `null` for those nodes and the composite index will not contain them.
The composite index still contains the remaining `:User` nodes that have both properties.
Therefore, the query returns one result.
...
As in <<denied-properties-on-all-labels>>, it is not clear whether the results returned by the index are due to a match on the `name` or `surname` property.
However, the impact is lessened, as only nodes with label `:Person` need to exclude the results from the `surname` property.
Nodes without the `:Person` label can read the `surname` property and does not need to be excluded.
However, the security model cannot introspect the _Lucene_ query to determine what it will actually do, whether it works only on the allowed `name` property or also on the disallowed `surname` property.
Therefore, in order to never return results the user should not be able to see, the index will consider all properties on nodes with label `:Person` denied instead of just the `surname` property.
Since the `surname` property is only denied for nodes with label `:Person` the index can safety return any nodes that does not have this label.
The steps taken by the query engine would be:

* Run a _Lucene_ query on the full-text index to produce results containing `ndy` in either property, leading to five results.
* Filter the results to exclude nodes that also have the label `:Person`, filtering out `Mandy` and `Joe`, leading to three results.

== Fail-open `DENY` behavior with property-based RBAC
