...
The examples below all features full-text indexes but also applies to the vector indexes.
...
Continuing the fulltext example above, now consider denying read access on properties for all labels, like the `surname` property:
...
Since the privilege affects all nodes regardless of label, all the nodes in the index will either be affected or not.
...
Continuing the full-text example, instead of denying traverse on `:Person` and deny reading the `surname` property for all labels, we deny reading the `surname` property only for nodes with the `:Person` label:
...
This query operates similarly to before, but because we do not have the denied traversal on `:Person` the third check on the label is not run.
And since nothing in the query relates to the denied property it returns all three results from the index.
...
Since the `surname` property is denied for nodes with label `:Person`, it will appear to be `null` for those nodes and the composite index will not contain them.
...
The property problem remains, since it is not certain whether the results provided by the index were returned due to a match on the `name` or the `surname` property, but are lessened.
Results from the `surname` property on nodes with label `:Person` now need to be excluded by the security rules, because they require that the user is unable to see those `surname` properties.
But nodes without the `:Person` label can read the `surname` property and does not need to be excluded.
Same as for denying the `surname` property on all labels, the security model is not able to introspect the _Lucene_ query in order to know what it will actually do, whether it works only on the allowed `name` property, or also on the disallowed `surname` property.
...
This then returns the same three results as the denied traverse on `:Person` case, instead of nothing like the denied property on all labels case.
