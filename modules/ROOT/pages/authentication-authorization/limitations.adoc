...
When authorizing a user, only privileges for existing labels, relationship types, and property names are applied.
...
However, running the same query again will work, and the node will be created.
...
To ensure she does not fail on the initial attempt, running the `db.createLabel()` procedure is good practice for all non-existing labels that get assigned privileges.
When creating the custom role, connect to 'testing' and run `CALL db.createLabel('A')` to ensure Alice creates the node successfully on her first attempt.
