...
Hence, three results are returned with only one of them having the `surname` property.
...
In this case, the query will return zero results rather than the expected results `Andy`, `Mandy`, and `Sandy`.
...
Since the `surname` property is denied only for nodes with the label `:Person`, it will appear to be `null` only for those nodes, and the composite index will not contain them.
...
Since the `surname` property is denied only for nodes with the label `:Person`, the index can safely return any nodes that do not have this label.
The steps taken by the query engine would be:
