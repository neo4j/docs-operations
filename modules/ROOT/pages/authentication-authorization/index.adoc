...
In addition, Neo4j supports mixed-mode authentication and authorization that allows to use multiple authentication providers in your database setup.
To configure mixed-mode authentication and authorization in Neo4j, modify the following configuration settings in the _neo4j.conf_ file:

* xref:configuration/configuration-settings.adoc#config_dbms.security.authentication_providers[`dbms.security.authentication_providers`]
* xref:configuration/configuration-settings.adoc#config_dbms.security.authorization_providers[`dbms.security.authorization_providers`]

These configuration values are comma-separated lists and can include Neo4j native, LDAP, and SSO providers.
...
* xref:authentication-authorization/sso-integration.adoc#auth-sso-configure-sso[Configure Neo4j to use OpenID Connect]
