...
== User States
Users in the system database can exist in two states, `ACTIVE` and `SUSPENDED`. `ACTIVE` is the default state for new users on
the database and when a user is in the `ACTIVE` state they are able to log into the system and perform queries as usual according
to their privileges.

`SUSPENDED` users are not permitted to log into the system using the credentials in the system graph. This does not prevent them
from logging in to the database using external authentication. To prevent this the user should be suspended there using the mechanisms
provided by the identity system.

In the event that the user is using the database at the point they are suspended:

* A user authorizing against only the system graph will be stripped of all roles assigned in the system graph, including the `PUBLIC` role.
This has the effect of removing all privileges.
* If the user is assigned roles by an external authorization provider, these roles will remain assigned to that user, and they will also
retain the `PUBLIC` role. As above, the user should also be suspended there using the mechanism provided by that system.

[NOTE]
====
The `SUSPENDED` state is only available in Neo4j Enterprise Edition. label:enterprise-edition[]
====
