...
Tags and `SHOW USER METADATA` are Enterprise Edition features introduced in Neo4j 2026.06.
...
| Command
m|
label:new[Introduced in 2026.09] label:cypher[Cypher 25] SHOW USERS AS COMMANDS
...
| Description
a|
Lists commands to create all users.

When using the `RETURN` clause, the `YIELD` clause is mandatory and must not be omitted.

For more information, see <<access-control-list-user-as-commands>>.

| Required privilege
a|
[source, privilege, role="noheader"]
----
GRANT SHOW USER CREDENTIALS
----
...
The `SHOW USER METADATA` privilege is additionally required to include the tags in the commands string and to see the values in the `tags` column returned by `SHOW USERS AS COMMANDS YIELD *`.
Otherwise, they are left out of the create command and the `tags` column returns `null`.
For more information, see <<access-control-create-users>> and xref:authentication-authorization/dbms-administration/dbms-user-metadata-management-privileges.adoc#grant-show-user-metadata-privilege[Grant privilege to show user metadata].
...
* `SET TAG[S]` is an Enterprise Edition feature introduced in Neo4j 2026.06.
...
label:new[Introduced in 2026.06] label:enterprise-edition[] The `SET USER METADATA` privilege is only required when attaching tags with the `SET TAG[S]` clause.
For more information, see <<access-control-create-users>> and xref:authentication-authorization/dbms-administration/dbms-user-metadata-management-privileges.adoc#grant-show-user-metadata-privilege[Grant privilege to show user metadata].
|===
...
`SET TAG[S]`, `ADD TAG[S]`, `REMOVE TAG[S]`, and `REMOVE ALL TAG[S]` are Enterprise Edition features introduced in Neo4j 2026.06.
Tags must be supplied as a single quoted `STRING` value, a `LIST` containing `STRING` values, or a parameter resolving to a `STRING` or a `LIST<STRING>`.
...
| Description
a|
label:enterprise-edition[] label:new[Introduced in 2026.06] Modifies the tags of several existing users in a single command.
...
[.tabbed-example]
=====
[role=include-with-create-user-syntax]
======
.Syntax
[source, syntax, role="noheader"]
----
...
----
======
[role=include-with-create-or-replace-user-syntax]
======
...
<4> label:enterprise-edition[] Specifies the user's status.
If not set, the default is `ACTIVE`.

<5> label:enterprise-edition[] Specifies a home database for a user.
...
<6> One or more `SET AUTH` clauses can be used to set xref:authentication-authorization/auth-providers.adoc[auth providers], which define authentication / authorization providers for that user.
...
====
The `SET STATUS {ACTIVE | SUSPENDED}`, `SET HOME DATABASE`, and `SET TAG[S]` parts of the command are only available in Neo4j Enterprise Edition.
...
The `SET TAG[S]` clause is introduced in Neo4j 2026.06.
====
...
Alternatively, you can use the native the xref:authentication-authorization/auth-providers.adoc[auth provider].
...
You can create the user `molly` in an active state, with an encrypted password (taken from the _/data/scripts/databasename/restore_metadata.cypher_ of a restored database backup), and the requirement to not change the password by running:
...
Alternatively, you can use the native the xref:authentication-authorization/auth-providers.adoc[auth provider] to set the password, similar to the previous example.
...
For example, you can create the user `jim` and attach the tags `finance` and `auditor`, which can then be evaluated in xref:authentication-authorization/attribute-based-access-control.adoc[ABAC] auth rules:
...
[NOTE]
====
Tags can be supplied as a single quoted `STRING` value, a `LIST` containing `STRING` values, or a parameter resolving to a `STRING` or a `LIST<STRING>`.
====
...
An example of showing the current user:
...
[NOTE]
====
The `SHOW CURRENT USER` command is supported only for the logged-in user and returns an empty result if authorization is disabled.
====
...
In Cypher 25 from 2026.10, you can combine the `SHOW CURRENT USER` command with regular Cypher syntax.
However, since `SHOW CURRENT USER` is run against the `system` database, all parts of the query must be allowed on the `system` database.
For example, clauses like `MATCH` and `SHOW INDEXES` are not allowed on the `system` database; therefore, they cannot be used with `SHOW CURRENT USER`.
However, `SHOW DATABASES` and system-allowed procedure calls are allowed.
Combining `SHOW CURRENT USER` with other clauses disables the automatic re-routing of the command to the `system` database, and requires explicit connection or routing.
For examples of combining `SHOW CURRENT USER` with other Cypher clauses, see <<combine-show-current-user-show-databases, Show information about the current users home database>> and <<combine-show-current-user-show-transactions, Show information about the current users running transactions>>.

[role=label--cypher-25 label--new-2026.10]
[[combine-show-current-user-show-databases]]
=== Show information about the current users home database

The `SHOW CURRENT USER` command can be combined with `SHOW DATABASES`.
To ensure the command is run against the `system` database, prepend it with the `USE` clause.
Combining `SHOW CURRENT USER` with other Cypher clauses disables the automatic re-routing.
...
SHOW DATABASES
YIELD name AS database, serverID AS server, currentStatus, aliases
WHERE home = database
RETURN user, database, server, currentStatus, aliases
----

This query is different from the `SHOW HOME DATABASE` command because if the user does not have a home database, you will get an empty result, instead of the result for the DBMS default database.

[role=label--cypher-25 label--new-2026.10]
[[combine-show-current-user-show-transactions]]
=== Show information about the current users running transactions

The `SHOW CURRENT USER` command can also be combined with a limited set of show commands, for example `SHOW TRANSACTIONS`.
To ensure the command is run against the `system` database, prepend it with the `USE` clause.
Combining `SHOW CURRENT USER` with other Cypher clauses disables the automatic re-routing.
...
YIELD user
SHOW TRANSACTIONS
YIELD database, transactionId, username, currentQuery, status
WHERE user = username
RETURN transactionId, currentQuery, database, status
----
...
| user    | roles               | passwordChangeRequired | suspended | home        |
+----------------------------------------------------------------------------------+
| "alice" | ["PUBLIC"]          | NULL                   | TRUE      | "anotherdb" |
| "jake"  | ["PUBLIC"]          | TRUE                   | FALSE     | NULL        |
| "jessy" | ["PUBLIC"]          | FALSE                  | FALSE     | NULL        |
| "jim"   | ["PUBLIC"]          | TRUE                   | FALSE     | NULL        |
| "joe"   | ["PUBLIC"]          | TRUE                   | TRUE      | "anotherdb" |
| "molly" | ["PUBLIC"]          | FALSE                  | FALSE     | NULL        |
| "neo4j" | ["admin", "PUBLIC"] | FALSE                  | FALSE     | NULL        |
+----------------------------------------------------------------------------------+
...
+-----------------------------------------------------------------+
| "jake" | FALSE     | TRUE                   | ["PUBLIC"] | NULL |
+-----------------------------------------------------------------+

1 row
ready to start consuming query after 165 ms, results consumed after another 2 ms
----
...
1 row
ready to start consuming query after 113 ms, results consumed after another 4 ms
----
...
----
GRANT SHOW USER METADATA ON DBMS TO PUBLIC;
----
////
...
Assuming `jim` has been assigned the tags `finance` and `auditor` (tags are managed using the <<access-control-alter-users, `ALTER USER`>> command):
...
[role=label--new-2026.09 label--cypher-25]
== Showing users as commands

To list the commands to create users, use `SHOW USERS AS COMMANDS`.
The command produces a row per user and yields the following columns:
...
The set of roles a user receives in practice may differ from those in this column.
It depends on the DMBS configuration and the user's xref:authentication-authorization/auth-providers.adoc[auth providers].
For example, users may use external (e.g., LDAP or OIDC) auth, and `native` may not be listed in the xref:configuration/configuration-settings.adoc#config_dbms.security.authorization_providers[`dbms.security.authorization_providers`] configuration setting.
...
| tags
| The xref:authentication-authorization/attribute-based-access-control.adoc[ABAC] tags attached to the user.
...
Only columns marked as default are returned by default, the other columns require a `YIELD *` or `YIELD` of that column to be returned.
...
| command                                                                                                                                                                                                                                                                                                     |
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| "CREATE USER neo4j SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,1d18af8e64cb4f77c64fe60f3e0e22a5e54eb9b222e33f8bf72900c3956d1873,dad046cc0751fd3fd4b74b8aaa18857ddf11d0b050a57e44f39c38675d96d41b' SET PASSWORD CHANGE NOT REQUIRED }"                                                            |
| "CREATE USER jake SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,8fe7ea2f353a54bcdb3782d9f2f2cef2574c75e5201a5b0e21c94255e465d8b1,0181edc0ed7070821f7e829f9427f59e369a9a72df629a195442cd01c96fdc5a' }"                                                                                              |
| "CREATE USER joe SET STATUS SUSPENDED SET HOME DATABASE anotherdb SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,97f3f09d71dbb5ecd881c58410b187550749defe857f802421a7efe3488ccc9f,edbfa769686e7bea77ef9c06b58892286961d51601fd61b73705b2393039218a' }"                                              |
| "CREATE USER alice SET STATUS SUSPENDED SET HOME DATABASE anotherdb SET AUTH PROVIDER 'oidc1' { SET ID 'alicesUniqueMySsoId' }"                                                                                                                                                                             |
| "CREATE USER molly SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,6d57a5e0b3317055454e455f96c98c750c77fb371f3f0634a1b8ff2a55c5b825,190ae47c661e0668a0c8be8a21ff78a4a34cdf918cae3c407e907b73932bd16c' SET PASSWORD CHANGE NOT REQUIRED }"                                                            |
| "CREATE USER jessy SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,6d57a5e0b3317055454e455f96c98c750c77fb371f3f0634a1b8ff2a55c5b825,190ae47c661e0668a0c8be8a21ff78a4a34cdf918cae3c407e907b73932bd16c' SET PASSWORD CHANGE NOT REQUIRED } SET AUTH PROVIDER 'oidc1' { SET ID 'jessysUniqueMySsoId' }" |
| "CREATE USER jim SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,cce68c2ec70323aa46e4625f3461bce263602a1e50979c7bedeed1b00bcd6f37,c24b217101de8fd23fac3cd4c98b78b70d6d8590f2815998fc0b7ca3c8d8fec1' } SET TAGS [finance,auditor]"                                                                    |
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
When using the `AS COMMANDS` clause together with `SHOW USERS WITH AUTH`, the command produces a row per user per auth provider, and yields the following two columns in addition to those output by `SHOW USERS AS COMMANDS`:
...
[NOTE]
====
The `WITH AUTH` clause only adds columns, the auth information will always be a part of the command even when not using `WITH AUTH`.
====
...
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| "CREATE USER neo4j SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,1d18af8e64cb4f77c64fe60f3e0e22a5e54eb9b222e33f8bf72900c3956d1873,dad046cc0751fd3fd4b74b8aaa18857ddf11d0b050a57e44f39c38675d96d41b' SET PASSWORD CHANGE NOT REQUIRED }"                                                            | "native" | {changeRequired: FALSE, password: "***"} |
| "CREATE USER jake SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,8fe7ea2f353a54bcdb3782d9f2f2cef2574c75e5201a5b0e21c94255e465d8b1,0181edc0ed7070821f7e829f9427f59e369a9a72df629a195442cd01c96fdc5a' }"                                                                                              | "native" | {changeRequired: TRUE, password: "***"}  |
| "CREATE USER joe SET STATUS SUSPENDED SET HOME DATABASE anotherdb SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,97f3f09d71dbb5ecd881c58410b187550749defe857f802421a7efe3488ccc9f,edbfa769686e7bea77ef9c06b58892286961d51601fd61b73705b2393039218a' }"                                              | "native" | {changeRequired: TRUE, password: "***"}  |
| "CREATE USER alice SET STATUS SUSPENDED SET HOME DATABASE anotherdb SET AUTH PROVIDER 'oidc1' { SET ID 'alicesUniqueMySsoId' }"                                                                                                                                                                             | "oidc1"  | {id: "alicesUniqueMySsoId"}              |
| "CREATE USER molly SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,6d57a5e0b3317055454e455f96c98c750c77fb371f3f0634a1b8ff2a55c5b825,190ae47c661e0668a0c8be8a21ff78a4a34cdf918cae3c407e907b73932bd16c' SET PASSWORD CHANGE NOT REQUIRED }"                                                            | "native" | {changeRequired: FALSE, password: "***"} |
| "CREATE USER jessy SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,6d57a5e0b3317055454e455f96c98c750c77fb371f3f0634a1b8ff2a55c5b825,190ae47c661e0668a0c8be8a21ff78a4a34cdf918cae3c407e907b73932bd16c' SET PASSWORD CHANGE NOT REQUIRED } SET AUTH PROVIDER 'oidc1' { SET ID 'jessysUniqueMySsoId' }" | "native" | {changeRequired: FALSE, password: "***"} |
| "CREATE USER jessy SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,6d57a5e0b3317055454e455f96c98c750c77fb371f3f0634a1b8ff2a55c5b825,190ae47c661e0668a0c8be8a21ff78a4a34cdf918cae3c407e907b73932bd16c' SET PASSWORD CHANGE NOT REQUIRED } SET AUTH PROVIDER 'oidc1' { SET ID 'jessysUniqueMySsoId' }" | "oidc1"  | {id: "jessysUniqueMySsoId"}              |
| "CREATE USER jim SET AUTH PROVIDER 'native' { SET ENCRYPTED PASSWORD '1,cce68c2ec70323aa46e4625f3461bce263602a1e50979c7bedeed1b00bcd6f37,c24b217101de8fd23fac3cd4c98b78b70d6d8590f2815998fc0b7ca3c8d8fec1' } SET TAGS [finance,auditor]"                                                                    | "native" | {changeRequired: TRUE, password: "***"}  |
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
7 rows
ready to start consuming query after 5 ms, results consumed after another 1 ms
----
...
<2> label:enterprise-edition[] Removes the home database for the user.
As a result, the DBMS default database will be used as the home database for that user.
<3> label:enterprise-edition[] Removes one, several, or all existing xref:authentication-authorization/auth-providers.adoc[auth provider(s)] from a user.
...
The `SET PASSWORD` prefix of the `CHANGE [NOT] REQUIRED` clause is only optional if it directly follows the `SET PASSWORD 'password'` clause and is not part of a `SET AUTH` clause.
<8> label:enterprise-edition[] Specifies the user's status.
<9> label:enterprise-edition[] Specifies a home database for a user.
A home database is resolved if it is pointing to a database or a database alias.
If no home database is set, the DBMS default database is used as the home database for that user.
...
[NOTE, role=label--enterprise-edition]
====
The `SET STATUS {ACTIVE | SUSPENDED}`, `SET HOME DATABASE`, `REMOVE HOME DATABASE`, `REMOVE AUTH`, and the tag clauses: `SET TAG[S]`, `ADD TAG[S]`, `REMOVE TAG[S]`, and `REMOVE ALL TAG[S]` parts of the command are only available in Neo4j Enterprise Edition.
The `SET AUTH` clause for external providers is only available in Neo4j Enterprise Edition.
However, `SET AUTH 'native'` can be used in Neo4j Community Edition.
The tags clauses are introduced in Neo4j 2026.06.
====
...
+-----------------------------------------------------------------------------------------------------------+
| "alice" | ["PUBLIC"]          | NULL                   | TRUE      | "anotherdb" | []                     |
| "bob"   | ["PUBLIC"]          | NULL                   | TRUE      | NULL        | ["finance", "auditor"] |
| "jessy" | ["PUBLIC"]          | FALSE                  | FALSE     | NULL        | []                     |
| "jim"   | ["PUBLIC"]          | TRUE                   | FALSE     | NULL        | ["finance", "auditor"] |
| "joe"   | ["PUBLIC"]          | FALSE                  | FALSE     | "anotherdb" | []                     |
| "molly" | ["PUBLIC"]          | FALSE                  | FALSE     | NULL        | []                     |
| "neo4j" | ["admin", "PUBLIC"] | FALSE                  | FALSE     | NULL        | []                     |
+-----------------------------------------------------------------------------------------------------------+

7 rows
ready to start consuming query after 84 ms, results consumed after another 1 ms
----
...
| user    | roles               | passwordChangeRequired | suspended | home        | tags                              |
+----------------------------------------------------------------------------------------------------------------------+
| "alice" | ["PUBLIC"]          | NULL                   | TRUE      | "anotherdb" | ["on-call"]                       |
| "bob"   | ["PUBLIC"]          | NULL                   | TRUE      | NULL        | ["finance", "auditor", "on-call"] |
| "jessy" | ["PUBLIC"]          | FALSE                  | FALSE     | NULL        | []                                |
| "jim"   | ["PUBLIC"]          | TRUE                   | FALSE     | NULL        | ["finance", "auditor"]            |
| "joe"   | ["PUBLIC"]          | FALSE                  | FALSE     | "anotherdb" | []                                |
| "molly" | ["PUBLIC"]          | FALSE                  | FALSE     | NULL        | []                                |
...
| user    | roles               | passwordChangeRequired | suspended | home        | tags                   |
+-----------------------------------------------------------------------------------------------------------+
| "alice" | ["PUBLIC"]          | NULL                   | TRUE      | "anotherdb" | ["finance", "auditor"] |
| "bob"   | ["PUBLIC"]          | NULL                   | TRUE      | NULL        | ["finance", "auditor"] |
| "jessy" | ["PUBLIC"]          | FALSE                  | FALSE     | NULL        | []                     |
| "jim"   | ["PUBLIC"]          | TRUE                   | FALSE     | NULL        | ["finance", "auditor"] |
| "joe"   | ["PUBLIC"]          | FALSE                  | FALSE     | "anotherdb" | []                     |
