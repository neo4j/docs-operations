...
Or is using a third-party ID provider:
...
//This one doesn't work on 2026.06 version:
// neo4j@system> CREATE USER jim
//               SET PASSWORD 'abcd1234'
//               SET TAGS 'finance', 'auditor';
// 42I06: syntax error or access rule violation - invalid input. Invalid input ',', expected: 'SET' or <EOF>. (line 3, column 19 (offset: 58))
// "SET TAGS 'finance', 'auditor'"
//                    ^
...
| user    | roles               | passwordChangeRequired | suspended | home        |
+----------------------------------------------------------------------------------+
| "alice" | ["PUBLIC"]          | NULL                   | FALSE     | "anotherdb" |
...
6 rows
ready to start consuming query after 4 ms, results consumed after another 3 ms
----
...
+----------------------------------------------------------------------------------------------------------------------------------------+
| "alice" | ["PUBLIC"]          | NULL                   | FALSE     | "anotherdb" | "oidc1"  | {id: "alicesUniqueMySsoId"}              |
...
6 rows
ready to start consuming query after 8 ms, results consumed after another 2 ms
----
...
//This one doesn't work on 2026.06 version for the same reason: the comma between the two tags.
[source, cypher,role=noplay]
----
ALTER USER bob
SET TAGS 'finance', 'auditor';
...
SHOW USERS YIELD *;
----
//To update the result after we fix the issue with the comma.
.Result
[role="queryresult"]
----
+-----------------------------------------------------------------------------------------+
...
+-----------------------------------------------------------------------------------------+
| "alice" | ["PUBLIC"]          | NULL                   | FALSE     | "anotherdb" | []   |
| "bob"   | ["PUBLIC"]          | NULL                   | TRUE      | NULL        | []   |
| "jessy" | ["PUBLIC"]          | FALSE                  | FALSE     | NULL        | []   |
| "joe"   | ["PUBLIC"]          | TRUE                   | FALSE     | "anotherdb" | []   |
| "molly" | ["PUBLIC"]          | FALSE                  | FALSE     | NULL        | []   |
| "neo4j" | ["admin", "PUBLIC"] | FALSE                  | FALSE     | NULL        | []   |
+-----------------------------------------------------------------------------------------+

6 rows
ready to start consuming query after 9 ms, results consumed after another 2 ms
----
