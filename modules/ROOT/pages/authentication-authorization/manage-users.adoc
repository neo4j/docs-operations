...
You can create a new user using the `CREATE USER` command.
You can optionally append `IF NOT EXISTS` to the command to ensure that no exception is thrown and nothing happens should the user already exist.
...
Alternatively, you can use the native the xref:authentication-authorization/auth-providers.adoc[auth provider] to set the password, similar to the previous example, for example for the user `jessy`:
...
== Creating or replacing users

You can replace an existing user with the `CREATE OR REPLACE USER` command.
You can specify the user's password, whether they must change it at the next login, their status, home database, and auth provider settings.
...
You can create the user `jake` or replace it if it already exists by using the command:
...
1 row
ready to start consuming query after 103 ms, results consumed after another 1 ms
----
