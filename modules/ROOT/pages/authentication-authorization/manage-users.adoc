...
The `SET USER METADATA` privilege is only required when setting, adding, or removing tags.
For more information, see xref:authentication-authorization/dbms-administration/dbms-user-metadata-management-privileges.adoc#grant-set-user-metadata-privilege[Grant privilege to set user metadata].
...
This column is only returned by `SHOW CURRENT USER YIELD *`, or when the `tags` column is yielded explicitly.
...
| tags
| The xref:authentication-authorization/attribute-based-access-control.adoc[ABAC] tags attached to the user.
This column is only returned by `SHOW USERS YIELD *`, or when the `tags` column is yielded explicitly.
...
<7> One or more tags can be attached to the user with the `SET TAG[S]` clause.
...
<4> Removes one, several, or all existing tags from the user.
...
<5> Adds one or more tags to the user, keeping any existing tags.
The value is either a single string, or a comma-separated list of strings.
...
"}"
----
<11> Sets the user's tags, replacing any tags the user already has.
...
For example, you can replace `bob`'s tags with exactly `finance` and `auditor`:
...
The `tags` column is not part of the default output, so use `SHOW USERS YIELD *` (the only additional column it returns is `tags`) to see the tags set above:
