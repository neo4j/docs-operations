...
[role=label--new-5.24]
[role=label--enterprise-edition]
.Modify the user `bob` to remove his native (username / password) auth and add an external OIDC Auth Provider:
======
[source,cypher,role=noplay]
----
ALTER USER bob
...
[role=label--new-5.24]
[role=label--enterprise-edition]
.Modify the user `bob` to remove any/all of his existing Auth Providers and add two external OIDC Auth Providers:
======
[source,cypher,role=noplay]
----
ALTER USER bob
...
.Remove the home database from the user `bob` and set their status to suspended:
======
[source,cypher,role=noplay]
----
ALTER USER bob
