...
To get the commands to create users, use `SHOW USERS AS COMMANDS`. The command produces a row per user and yields the following columns:
...
=== Show user with auth as commands
...
[NOTE]
====
This only adds columns, the auth part of the command will always be returned even if not using `WITH AUTH`.
====
