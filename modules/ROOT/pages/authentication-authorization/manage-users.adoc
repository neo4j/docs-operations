...
However, the xref:authentication-authorization/dbms-administration/dbms-user-metadata-management-privileges.adoc#grant-show-user-metadata-privilege[`SHOW USER METADATA`] privilege is required to see the values in the `tags` column returned by `SHOW CURRENT USER YIELD *`; without it, the column returns `null`.
...
For more information, see xref:authentication-authorization/manage-users.adoc#access-control-list-users[Showing users].
...
`SET TAGS` and `SHOW USER METADATA` are Enterprise Edition features introduced in Neo4j 2026.06.
For more information, see xref:authentication-authorization/dbms-administration/dbms-user-metadata-management-privileges.adoc#grant-show-user-metadata-privilege[Grant privilege to show user metadata] .
...
For more information, see xref:authentication-authorization/manage-users.adoc#access-control-alter-users-bulk[Modifying multiple users].
...
Assuming `jake` has been assigned the tags `finance` and `auditor` (tags are managed using the xref:authentication-authorization/manage-users.adoc#access-control-alter-users[`ALTER USER`] command):
...
Remember that any `REMOVE` clause(s) must come before any `ADD` clause(s), which must come before any `SET` clause(s).
======
...
To instead remove every tag from a user, use `REMOVE ALL TAGS`, for example `ALTER USER bob REMOVE ALL TAGS`.
======
