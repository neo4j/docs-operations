...
The Enterprise Edition includes everything the Community Edition offers, plus additional enterprise requirements such as backups, clustering, and failover capabilities.
Infinigraph is a special Enterprise Edition that also includes unlimited horizontal scaling with automatic sharding, as well as Graph Data Science with a license key.
...
Infinigraph::
Infinigraph is a specialized Enterprise Edition of Neo4j designed for high-performance graph processing and analytics at scale.
It includes all features from Enterprise edition, as well as unlimited horizontal scaling with automatic sharding and graph analytics capabilities.
...
[[edition-details]]
.Key features per edition
[cols="6,2a,2a,2a",frame="topbot",options="header"]
|===
| Feature
| CE
| EE
...
| xref:monitoring/query-management.adoc[Showing and terminating running queries]
| {check-mark}
| {check-mark}
| {check-mark}
...
a| APOC 450+ link:https://neo4j.com/docs/apoc/current/[Core Procedures and Functions]
| {check-mark}
| {check-mark}
| {check-mark}
...
| Support for link:https://neo4j.com/docs/graph-data-science/current/installation/installation-enterprise-edition/[Neo4j Graph Data Science Enterprise Edition] footnote:license[Infinigraph includes a license key for Neo4j Graph Data Science.]
|
| {check-mark}
| {check-mark}

| Support for link:https://neo4j.com/docs/bloom-user-guide/current/[Neo4j Bloom]
|
| {check-mark}
| {check-mark}

| Gen AI integration via link:https://neo4j.com/docs/genai/plugin/current/[Neo4j GenAI plugin]
| {check-mark}
| {check-mark}
| {check-mark}

4+^s| Indexes and constraints
...
| link:{neo4j-docs-base-uri}/cypher-manual/current/schema/constraints/create-constraints/#create-property-uniqueness-constraints[Property uniqueness constraints]
| {check-mark}
| {check-mark}
| {check-mark}

| link:{neo4j-docs-base-uri}/cypher-manual/current/schema/constraints/create-constraints/#create-property-existence-constraints[Property existence constraints]
|
| {check-mark}
| {check-mark}

| link:{neo4j-docs-base-uri}/cypher-manual/current/schema/constraints/create-constraints/#create-property-type-constraints[Property type constraints]
|
| {check-mark}
| {check-mark}

|link:{neo4j-docs-base-uri}/cypher-manual/current/schema/constraints/create-constraints/#create-key-constraints[Node and relationship key constraints]
|
| {check-mark}
| {check-mark}
...
| xref:authentication-authorization/property-based-access-control.adoc[Property-based access control]
|
| {check-mark}
|
...
| xref:import.adoc#import-tool-full[Offline full import]
| {check-mark}
| {check-mark}
| {check-mark}

| xref:import.adoc#import-tool-incremental[Incremental import]
|
| {check-mark}
| {check-mark}
...
| xref:scalability/sharded-property-databases/overview.adoc[Sharded property databases] label:new[New in 2025.12]
|
|
| {check-mark}

4+^s| xref:monitoring/index.adoc[Monitoring and management]
