...
.. To find the correct SHA hash, go to Neo4j Download Center and click on `SHA-256` that is located below your downloaded file.
...
. Place the extracted files in a permanent home on your server, for example, _D:\neo4j\_.
The top-level directory is referred to as _NEO4J_HOME_.
...
For example, updating the value of `server.memory.heap.initial_size` in the default xref:configuration/file-locations.adoc[_neo4j.conf_] file or changing the `NEO4J_CONF` environment variable will not automatically apply the changes.
