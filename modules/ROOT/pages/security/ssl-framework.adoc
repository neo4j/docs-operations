...
Post-quantum/hybrid key exchange for other interfaces (such as the HTTP server) which rely on native Java
cryptographic libraries is currently unsupported pending
backporting of post-quantum/hybrid key exchange features to the LTS versions of the JVM.
