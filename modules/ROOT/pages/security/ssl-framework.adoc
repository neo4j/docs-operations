...
The `netty-tcnative-boringssl-static` version must match the `netty-tcnative` version.
In the Neo4j Enterprise Edition, you can find the bundled `boringssl` library under the same `/lib` directory .
...
[TIP]
====
If the same certificates are used across all servers of the <<cluster>>, ensure that the DNS names of all cluster members are included in the certificates when generating them.
...
Once SSL is enabled over Bolt, you can connect to the <<database-management-system, Neo4j DBMS>> using `neo4j+s` or `bolt+s`:
...
Note that CBC (cipher block chaining)-based ciphers (RFC 8447), used in TLS v1.2 network encryption, are not supported from 2025.01 onwards.
See xref:breaking-changes.adoc#ssl-framework-breaking-changes[Breaking changes in SSL framework] for more information.
...
Neo4j supports post-quantum hybrid key exchange over Bolt when using the xref:security/ssl-framework.adoc#ssl-providers[OpenSSL provider].
