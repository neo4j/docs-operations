...
Neo4j supports Post-quantum cryptographic encryption over Bolt when using the xref:security/ssl-framework.adoc#ssl-providers[OpenSSL provider].

Post-quantum for other interfaces which rely on native Java cryptography is current unsupported pending
backporting of post-quantum cryptographic features to the LTS versions of the JVM.
