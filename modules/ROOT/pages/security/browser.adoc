...
[IMPORTANT]
====
Caching credentials in browser local storage can *no longer* be disabled altogether.
`browser.retain_connection_credentials` is no longer supported in the server configuration.
====
