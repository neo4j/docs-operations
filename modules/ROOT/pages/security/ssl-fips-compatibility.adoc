...
Specifically Netty's _Forked Tomcat Native_ library called link:https://github.com/netty/netty-tcnative[netty-tcnative].
...
Refer to link:https://netty.io/wiki/forked-tomcat-native.html[Forked Tomcat Native] for more information.
...
To install link:https://apr.apache.org[Apache Portable Runtime Library], use the operating system's package manager.
...
Check the xref:configuration/file-locations.adoc[file locations] documentation for the correct location.
+
...
These can be downloaded from link:https://assets.neo4j.com/netty-tcnative/2.0.65.Final/simplified/netty-tcnative-2.0.65.Final-all.tar[]
+
...
SSL configuration is described in detail in xref:security/ssl-framework.adoc[SSL Framework configuration].
...
. Set `xref:configuration/configuration-settings.adoc#config_dbms.connector.bolt.tls_level[dbms.connector.bolt.tls_level]=REQUIRED`
