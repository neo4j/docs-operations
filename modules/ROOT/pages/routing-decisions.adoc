...
Query routing is the process of deciding which Cypher executor (database) and at which physical location the query should be executed.
Every query that arrives at a Neo4j server, over the bolt protocol from a driver, undergoes this process here described.
...
**** and xref:clustering/setup/routing.adoc#clustering-routing[Server-side routing] is xref:reference/configuration-settings.adoc#config_dbms.routing.enabled[enabled], then location is _Remote_ (using the xref:reference/configuration-settings.adoc#config_server.routing.advertised_address[routing advertised address] of that server).
**** if not, then fail.
