...
| credentials
| label:new[Introduced in 2025.10] The type of credentials used to connect to the remote database, either `STORED NATIVE CREDENTIALS` or `OIDC CREDENTIAL FORWARDING`, or  null if the target database is local.
| STRING
| {check-mark}
...
* label:new[Introduced in 2025.10] `OIDC CREDENTIAL FORWARDING`, forwarding the bearer authentication token from the logged-in user on the local DBMS.
...
You can verify that the remote database alias has been created correctly by using the `SHOW ALIASES FOR DATABASE` command:
.Query
[source, cypher]
----
