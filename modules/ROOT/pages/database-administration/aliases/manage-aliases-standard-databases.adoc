...
| credentials
| The type of credentials used to connect to the remote database, either `STORED NATIVE CREDENTIALS` or `OIDC CREDENTIAL FORWARDING`, or  null if the target database is local. label:default-output[] label:new[Introduced in 2025.10]
| STRING
