...
| user
| Native user connecting to the remote database, or `null` if target database is local or if OIDC credential forwarding is used to authenticate against the remote database. label:default-output[]
| STRING
