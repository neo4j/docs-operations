...
The following examples describe how to set up access to a remote database via a remote database alias in two different ways: using stored native credentials and using OIDC credential forwarding.
...
* label:new[Introduced in 2025.x] `OIDC CREDENTIAL FORWARDING`, forwarding the bearer authentication token from the logged-in user on the local *DBMS A*.
...
* Actions on the remote *DBMS B* are all attributed to the user configured for the remote database alias.
