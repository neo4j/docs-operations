...
The following set-up is required to recreate the below examples:
...
.Create alias `cineasts`.`latest` on a composite database
[source, cypher]
----
...
.Create alias `cineasts`.`upcoming` for a remote database on a Composite database
[source, cypher]
----
