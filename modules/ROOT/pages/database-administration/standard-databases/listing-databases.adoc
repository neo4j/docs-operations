...
| type
| The type of the database: `system`, `standard`, `graph shard`, `property shard` or `composite`. label:default-output[]
| STRING
...
| role
| The current role of the database (`primary`, `secondary`, `property shard replica`, `unknown`). label:default-output[]
...
The value for composite databases, virtual sharded databases and property shards is `NULL` because it does not apply to them.footnote:compositeDb[This change applies to versions 2025.04 and later and 5.26.5 and later.]
| INTEGER
...
The value for composite databases, virtual sharded databases and property shards is `NULL` because it does not apply to them.footnote:compositeDb[]
| INTEGER
...
Applicable only for property shards.
label:new-2025.12[Introduced in Neo4j 2025.12]
| INTEGER
...
The value for composite databases, virtual sharded databases and property shards is `NULL` because it does not apply to them.
| INTEGER
...
The value for composite databases and virtual sharded databases is `NULL` because it does not apply to them.footnote:compositeDb[]
| STRING
...
The value for composite databases and sharded databases is `NULL` because it does not apply to them.footnote:compositeDb[]
| INTEGER
...
|constituents
|The names of any graph shards the database may have. Applicable only for composite databases. label:default-output[]
| LIST<STRING>

|graphShards
|The names of any property shards the database may have. Applicable only for standard and virtual sharded databases.
label:new-2025.12[Introduced in Neo4j 2025.12]
| LIST<STRING>

|propertyShards
|The names of any property shards the database may have. Applicable only for virtual sharded databases.
label:new-2025.12[Introduced in Neo4j 2025.12]
| LIST<STRING>

|defaultLanguage
|The default Cypher version for this database.
Queries that are not prefixed with a Cypher version will be executed with the Cypher version of the database. label:new[Introduced in Neo4j 2025.06]
|STRING
