...
The `SHOW DATABASES` command lists all databases in the DBMS, including their states and other relevant information.
...
In Cypher 25 from 2026.05, you can combine the `SHOW DATABASES` command with regular Cypher syntax.
...
However, system-allowed procedure calls and, from 2026.10, `SHOW CURRENT USER` are allowed.
Combining `SHOW DATABASES` with other clauses disables the automatic re-routing of the command to the `system` database, and requires explicit connection or routing.

== `SHOW DATABASES` output

Depending on what you want to see, you can show different sets of databases:
