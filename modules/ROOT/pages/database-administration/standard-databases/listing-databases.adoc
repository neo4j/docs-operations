...
Not applicable to <<composite-database, composite databases>> (the value is `NULL`).
| STRING
| {check-mark}
...
By specifying the `name` column and sorting the results by distinct name, only the number of distinct databases are counted, **not** the number of allocations of databases in a <<cluster, clustered environment>>.
