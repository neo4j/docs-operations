...
The `neo4j-admin database upload` command uploads a local Neo4j database xref:backup-restore/offline-backup.adoc[`.dump`] or xref:backup-restore/online-backup.adoc[`.backup`] file into a Neo4j Aura instance.
Keep in mind that a `.backup` file is preferred.
...
Otherwise, the upload fails with SSL errors.

You can upload databases in either xref:database-internals/store-formats.adoc#aligned-format[`aligned`] (the default store format in Community Edition) or xref:database-internals/store-formats.adoc#block-format[`block`] (the default store format in Enterprise Edition) format.

== Limitations

* The `neo4j-admin database upload` command _does not_ support xref:backup-restore/planning.adoc#splitting-backup-archive[split backup/dump archives] introduced in Neo4j 2026.09.
* You cannot upload a sharded property database (Infinigraph), introduced in 2025.12, to an Aura instance.
Before uploading, convert a sharded property database into a standard database by following the steps in xref:/scalability/sharded-property-databases/resharding-databases.adoc#_consolidating_a_sharded_property_database_into_a_standard_database[Scalability -> Property sharding -> Resharding and consolidating databases].
* You can run the `neo4j-admin database upload` command only if public traffic is enabled.
...
neo4j-admin database upload [-h] [--expand-commands] [--verbose]
                            ...
                            [--additional-config=<file>] --from-path=<path> [--to=<destination>] [--to-dbid=<databaseid>]
...
|--to-dbid=<databaseid>
|label:new[Introduced in 2026.07] Database ID of the target database. Required when uploading to a Multi-DB instance, using an instance-based URI (e.g. `*.instances.neo4j.io`).
|
