...
Starting with Neo4j 2025.06, `SET DEFAULT LANGUAGE CYPHER {5|25}` is available on both self-managed deployments and Aura.
====
...
[NOTE]
====
Modifying access mode is only available to standard databases and not <<composite-database, composite databases>>.
====
...
[role=label--new-2025.06 label--aura]
...
In a <<cluster, clustered environment>>, you can use the `ALTER DATABASE` command to change the number of servers hosting a database.
