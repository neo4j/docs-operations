...
A command using a `WAIT` sub-clause automatically <<commit, commits>> the current transaction when it executes successfully, as the command needs to run immediately for it to be possible to `WAIT` for it to complete.
