...
[NOTE]
====
Both standard databases and <<composite-database, composite databases>> can be deleted using this command.
====
...
They will simply no longer resolve their targets as if they were created targeting a nonexistent database.

Using `CASCADE ALIASES` requires the `DROP ALIAS` privilege.
For more information about the privilege, see xref:authentication-authorization/dbms-administration/dbms-alias-management-privileges.adoc[ALIAS MANAGEMENT privileges].
