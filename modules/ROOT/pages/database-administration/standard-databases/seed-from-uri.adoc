...
The `server://` scheme allows you to use a directory or a specific artifact within this directory as the external source for creating (or recreating) a database.

In other words, you can seed a database from a specific backup artifact located on the server's file system.
...
Production servers should not be used as long-term backup storage.
...
See <<example-seedSourceDatabase, Example 3>> for details.

// This `ServerSeedProvider` can also be used with the xref:database-administration/standard-databases/recreate-database.adoc[`dbms.recreateDatabase` procedure].


[[example-specific-artifact]]
Example 1: Single URI with a specific artifact::
...
----
+
If `foo.backup` is not found an error is thrown.
+
...
[[example-directory]]
Example 2: Single URI with a directory::
...
[[example-seedSourceDatabase]]
Example 3: Single URI with a directory and `seedSourceDatabase`::
...
----
CREATE DATABASE foo
OPTIONS {seedURI: 'server://server-id/', seedSourceDatabase: 'bar'};
----
+
Fails if no valid backup chain can be found for `bar`.
