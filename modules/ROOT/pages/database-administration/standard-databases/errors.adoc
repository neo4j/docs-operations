...
However, databases may only be in one of a select number of states (see xref::database-administration/standard-databases/manage-databases.adoc#database-states[Database states] for more information):
...
[NOTE]
====
Additionally, `dropped` is also a valid database state, but it is only transiently observable, as database records are removed from `SHOW DATABASE` results once the `DROP` operation is complete.
====
