...
If there is only one allocation in `quarantined` mode, then it is usually safe to choose `replaceStateKeepStore` or `replaceStateReplaceStore`.
If there are two allocations in `quarantined` mode (meaning that the database is `quarantined` on two servers), then you should not use `replaceStateKeepStore` or `replaceStateReplaceStore` because there would be no majority to let them in.
In this case, it is recommended to recreate the quarantined database from a backup.
See xref:clustering/multi-region-deployment/disaster-recovery.adoc#make-servers-available[Disaster recovery guide -> Make servers available, step 5].
