...
However, if the store is seeded with `seedURI`, `existingDataSeedServer`, or `existingDataSeedInstance` (removed in Cypher 25), or if the command is being used to mount pre-existing store files already present on the disk, they will use their current store format without any alterations.
...
| `replicaConfig` label:new[Introduced in 2026.07]
a| Type: `MAP` +
Possible keys of the map include:
...
Defines the upstream information for the replica database to pull from.

For details, see xref:clustering/replicating-databases-across-clusters.adoc[]
