...
* Defines an identical seed from an external source which will be used to seed all servers for each shard of a sharded database:
** The key is the name of the shard.
** The value is the seedUri of the given shard.

* label:new[Introduced in 2026.04] Defines a list of Neo4j server endpoints that will be used to source an identical seed which will be used to seed all servers for each shard of a sharded database.
For more information, see xref:scalability/sharded-property-databases/creating-sharded-databases.adoc[Create sharded property databases].
