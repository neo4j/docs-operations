...
[NOTE]
====
In Cypher 25, Neo4j 2025.06, the options `seedCredentials` and `existingDataSeedInstance` have been removed, while the `existingData` option is deprecated and has no effect.
====
======
=====
