...
For more information, see xref:database-administration/syntax.adoc[].
...
More details about seeding options can be found xref::clustering/databases.adoc#cluster-seed[here].
====
...
The_ `WAIT` _sub-clause was added as an option to the_ `ALTER DATABASE` _command in Neo4j 5.7._
...
A command using a `WAIT` sub-clause will automatically commit the current transaction when it executes successfully, as the command needs to run immediately for it to be possible to `WAIT` for it to complete.
