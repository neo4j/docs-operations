= Built-in procedures
:description: This page provides a complete reference to the built-in Neo4j procedures.
:table-caption!:
:table-stripes: hover
:table-frame: topbot
...
For changed, deprecated, or removed procedures in the 2025-2026 series, refer to xref:changes-2025-2026.adoc[Changes in Neo4j 2025-2026 series], xref:deprecations.adoc[Current deprecations], and xref:breaking-changes.adoc[Breaking changes in Neo4j 2025.01].
...
For more information, see xref:authentication-authorization/dbms-administration/dbms-execute-privileges.adoc[the `EXECUTE` privileges].
====
...
| *Mode* 3+| READ
| *Replaced by* 3+| xref:procedures/built-in-procedures.adoc#procedure_db_cdc_current[`db.cdc.current()`]
|===
...
| `id` | `STRING` | The change identifier.
| *Mode* 3+| READ
| *Replaced by* 3+| xref:procedures/built-in-procedures.adoc#procedure_db_cdc_earliest[`db.cdc.earliest()`]
|===
...
| *Mode* 3+| READ
| *Replaced by* 3+| xref:procedures/built-in-procedures.adoc#procedure_db_cdc_query[`db.cdc.query()`]
|===
...
| *Description* 3+a| Returns the current change identifier and <<commit>> time that can be used to stream changes from.
.3+| *Return arguments* | *Name* | *Type* | *Description*
| `id` | `STRING` | The change identifier.
| `txCommitTime` label:new[Introduced in 2026.06] label:cypher[Cypher 25] | `ZONED DATETIME` | The commit time of the last committed transaction.
| *Mode* 3+| READ
|===
...
The Neo4j <<cluster>> is a distributed database system that allows for horizontal scaling and high availability.
...
[NOTE]
====
The procedure `dbms.cluster.cordonServer()` requires the `SERVER MANAGEMENT` privilege.
Running the procedure with an `Admin` privilege is deprecated as of Neo4j 5.23.
====
...
[NOTE]
====
The procedure `dbms.cluster.setAutomaticallyEnableFreeServers()` requires the `SERVER MANAGEMENT` privilege.
Running the procedure with an `Admin` privilege is deprecated as of Neo4j 5.23.
====
...
[NOTE]
====
The procedure `dbms.cluster.uncordonServer()` requires the `SERVER MANAGEMENT` privilege.
Running the procedure with an `Admin` privilege is deprecated as of Neo4j 5.23.
====

[role=label--enterprise-edition label--new-2026.06 label--cypher-25]
[[procedure_dbms_promoteReplicaDatabase]]
=== dbms.promoteReplicaDatabase()

.Details
|===
| *Syntax* 3+m| dbms.promoteReplicaDatabase(database :: STRING, options :: MAP)
| *Description* 3+a| Convert a replica database to a standard database while keeping the data currently has. Optionally modify its topology.
.3+| *Input arguments* | *Name* | *Type* | *Description*
| `database` | `STRING` | database STRING
| `options` | `MAP` | Topology options for the promoted database
| *Mode* 3+| WRITE
|===
...
Starting with 2026.08, the `dbms.recreateDatabase()` procedure can be used to recreate a sharded property database.
See xref:scalability/sharded-property-databases/recreating-sharded-databases.adoc[Recreate a sharded property database].
====
...
.Details
|===
| *Syntax* 3+m| dbms.setDefaultAllocationNumbers(primaries, secondaries, propertyShardReplicas)
| *Description* 3+a| With this method you can set the default number of primaries, secondaries and property shard replicas. footnote:[The description is updated in 2025.12 and includes property shard replicas.]
.4+| *Input arguments* | *Name* | *Type* | *Description*
| `primaries` | `INTEGER` | The default number of primaries.
| `secondaries` | `INTEGER` | The default number of secondaries.
| `propertyShardReplicas` label:new[Introduced in 2025.12] label:cypher-25[] | `INTEGER` | The default number of property shard replicas.
| *Mode* 3+| WRITE
|===
...
.Details
|===
| *Syntax* 3+m| dbms.showTopologyGraphConfig() :: (allocator, defaultPrimariesCount, defaultSecondariesCount, defaultDatabase, autoEnableFreeServers, propertyShardReplicas)
...
[role=label--new-2026.08 label--enterprise-edition label--cypher-25]
...
| *Description* 3+a| Change the default database to the provided value. The old default database must be stopped.
.2+| *Input arguments* | *Name* | *Type* | *Description*
| `databaseName` | `STRING` | The name of the database.
.2+| *Return arguments* | *Name* | *Type* | *Description*
...
[NOTE]
====
Starting with Neo4j 2025.04, the default database can also be set to a local or remote database alias, or to a database that does not yet exist.
====
...
For more information, see link:{neo4j-docs-base-uri}/aura/fleet-management/overview/[Fleet management].


[role=label--admin-only label--new-2026.03]
...
.Details
|===
| *Syntax* 3+m| fleetManagement.inspectToken([ token ]) :: (projectId, tokenExpires, errorMessage, connectionUrl)
...
| `connectionUrl` | `STRING` | label:new[Introduced in 2026.09] The URL that Fleet Manager connects to, if set.
| *Mode* 3+| DBMS
|===
...
.Details
|===
| *Syntax* 3+m| fleetManagement.registerToken(token, connectionUrl) :: (result)
| *Description* 3+a| Add a token for authenticating to Fleet Manager.
.3+| *Input arguments* | *Name* | *Type* | *Description*
| `token` | `STRING` | A token obtained from a fleet manager endpoint.
| `connectionUrl` | `STRING` | label:new[Introduced in 2026.09] (optional) The URL of the service to connect to. Will connect to Neo4j Aura, if not provided.
.2+| *Return arguments* | *Name* | *Type* | *Description*
| `result` | `STRING` | A message indicating the result of the operation.
| *Mode* 3+| DBMS
|===
...
[IMPORTANT]
====
The `db.schema.visualization()` procedure is designed to query metadata stored in the database store rather than the graph itself.
Therefore, its output may include additional relationships that do not actually exist in the graph, due to the information available in the count store.
====
...
* link:{neo4j-docs-base-uri}/genai/plugin/current/[Neo4j GenAI plugin documentation]
...
[role=label--deprecated-2026.04 label--deprecated-cypher-25]
...
| *Mode* 3+| READ
| *Replaced by* 3+| Cypher clause `SEARCH`
|===

[NOTE]
====
The Cypher clause `SEARCH` is available from Neo4j 2026.01 onwards, and is the preferred way of querying vector indexes.
For more information, see the link:{neo4j-docs-base-uri}/cypher-manual/current/clauses/search/[Cypher Manual -> SEARCH].
====
