...
It also lists current xref:procedures/built-in-procedures.adoc#deprecated-procedures[deprecated procedures] and the xref:procedures/built-in-procedures.adoc#removed-procedures[procedures removed in Neo4j 5], along with their replacements.
...
* Neo4j's link:{neo4j-docs-base-uri}/apoc/{page-version}/[APOC Core library] and link:{neo4j-docs-base-uri}/genai/plugin/5/[GenAI plugin] are installed by default on link:{neo4j-docs-base-uri}/aura/[Aura] instances, but have to be installed separately on on-prem servers.
...
Further details on how to use the `dbms.cluster.recreateDatabase()` procedure are provided in the section xref:database-administration/standard-databases/recreate-database.adoc[Database administration -> Recreate a database].
====
...
* link:{neo4j-docs-base-uri}/genai/plugin/5/[GenAI plugin]
* link:{neo4j-docs-base-uri}/genai/plugin/5/reference/functions-procedures/[GenAI plugin -> Functions]
...
[role=label--new-5.13 label--beta-until-5.26]
...
[role=label--new-5.18 label--beta-until-5.26]
...
For more information, see the link:{neo4j-docs-base-uri}/cypher-manual/5/indexes/semantic-indexes/vector-indexes/#create-vector-index[Cypher Manual -> Create a vector index].
====
...
For more information, see the link:{neo4j-docs-base-uri}/cypher-manual/5/genai-integrations/#multiple-embeddings[Cypher Manual -> Generating a batch of embeddings].
...
* link:{neo4j-docs-base-uri}/cypher-manual/5/indexes/search-performance-indexes/overview/[Cypher Manual -> Search performance indexes]
...
| xref:procedures/built-in-procedures.adoc#procedure_cdc_current[`cdc.current()`]
| label:no[]
| label:yes[]
...
| xref:procedures/built-in-procedures.adoc#procedure_cdc_earliest[`cdc.earliest()`]
| label:no[]
| label:yes[]
| label:new[Introduced in 5.13] label:beta[] label:deprecated[Deprecated in 5.17]
Replaced by: xref:procedures/built-in-procedures.adoc#procedure_db_cdc_earliest[`db.cdc.earliest()`]

| xref:procedures/built-in-procedures.adoc#procedure_cdc_query[`cdc.query()`]
| label:no[]
| label:yes[]
...
| xref:procedures/built-in-procedures.adoc#procedure_db_create_setVectorProperty[`db.create.setVectorProperty()`]
| label:yes[]
| label:yes[]
...
| xref:procedures/built-in-procedures.adoc#procedure_dbms_cluster_readReplicaToggle[`dbms.cluster.readReplicaToggle()`]
| label:no[]
| label:yes[]
...
| xref:procedures/built-in-procedures.adoc#procedure_dbms_setDatabaseAllocator[`dbms.setDatabaseAllocator()`]
| label:no[]
| label:yes[]
| label:admin-only[] label:deprecated[Deprecated in 5.23]

// New in 4.1
| xref:procedures/built-in-procedures.adoc#procedure_dbms_upgrade[`dbms.upgrade()`]
| label:yes[]
| label:yes[]
| label:admin-only[] label:deprecated[Deprecated in 5.9]

// New in 4.1
| xref:procedures/built-in-procedures.adoc#procedure_dbms_upgradeStatus[`dbms.upgradeStatus()`]
| label:yes[]
| label:yes[]
| label:admin-only[] label:deprecated[Deprecated in 5.9]
