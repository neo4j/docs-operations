...
For more information about the `CALL` clause in this context, refer to the link:{neo4j-docs-base-uri}/cypher-manual/5/subqueries/call-subquery/[Cypher Manual -> `CALL` subquery].
====
...
For more information, see the link:{neo4j-docs-base-uri}/cypher-manual/5/syntax/parameters/[Cypher Manual -> Parameters].
====
...
In the following example, `propertyKey` is aliased as `prop` and then used later in the query to link:{neo4j-docs-base-uri}/cypher-manual/5/functions/aggregating/#functions-count[count] the occurrence of each property in the graph.
...
Calling a `VOID` procedure in the middle of a larger query simply passes on each input record (i.e., it acts like link:{neo4j-docs-base-uri}/cypher-manual/5/clauses/with/[`WITH *`] in terms of the record stream).
...
Similar to link:{neo4j-docs-base-uri}/cypher-manual/5/clauses/optional-match/[`OPTIONAL MATCH`], any empty rows produced by `OPTIONAL CALL` return `null`.

=== Regular procedure call

This following query uses the link:{neo4j-docs-base-uri}/apoc/5/overview/apoc.neighbors/apoc.neighbors.tohop[`apoc.neighbors.tohop()`] procedure (part of Neo4j's link:{neo4j-docs-base-uri}/apoc/current/[APOC Core library]), which returns all nodes connected by the given relationship type within the specified distance (1 hop, in this case) and direction.
