...
== Procedure calls against system database

While not all procedures are allowed to run against the system database, there are those that are allowed.
When run against the system database, the queries are more restricted as most Cypher clauses are not allowed on the system database.
Standalone procedure calls are allowed, as well as procedure calls with `YIELD` and `RETURN` clauses.
Additionally, from Neo4j 2026.06, they can have the `WHERE` clause.

=== Procedure call against system
...
=== Procedure call with `WHERE` against system
