...
Drivers and client applications access and use the Fabric execution context by naming it as the selected database for a session. For more information, see link:{neo4j-docs-base-uri}/driver-manual/{page-version}/cypher-workflow#driver-databases[Driver Manual -> Databases and execution context].
