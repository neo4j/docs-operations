...
= Importing data
:description: This tutorial provides detailed examples to illustrate the capabilities of importing data from CSV files with the command `neo4j-admin database import full`.

The neo4j-admin import command is used for loading large amounts of data from CSV files and supports full and incremental import into a running or stopped Neo4j DBMS.

This tutorial provides detailed examples to illustrate the capabilities of importing data from CSV files with the command xref:import.adoc[`neo4j-admin database import full`].
...
The database must not exist before the `neo4j-admin database import full` command has been executed, and the database should be created afterward.
The command will exit with an error message if the database already exists.

If the database already exists, you need to run the `neo4j-admin database import incremental` command.
The database must be offline if you want to perform the incremental import within one command.

If you cannot afford a full downtime of your database, you can run the import in three stages.
For details, see xref:import.adoc#incremental-import-stages[Incremental import in stages]. 
====
...
The call to `neo4j-admin database import full` would look like this:
...
The call to `neo4j-admin database import full` would look as follows:
...
Importing the data using regular expressions, the call to `neo4j-admin database import full` can be simplified to:
