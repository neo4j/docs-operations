...
The xref:tools/neo4j-admin-import.adoc[`neo4j-admin import`] is a command for loading large amounts of data from CSV files into an **unused** database.
...
It is possible to create the given database either before or after the `neo4j-admin import` command have been executed.
If the database already exists the given database needs to be in a state where no data have been introduced before.
====
...
* UNIX styled paths.
* The xref:tools/neo4j-admin.adoc[`neo4j-admin import`] command.
...
.Handy tips:
====
* The details of CSV file header format can be found at xref:tools/neo4j-admin-import.adoc#import-tool-header-format[CSV header format].
...
You can customize the configuration options that the import tool uses (see xref:tools/neo4j-admin-import.adoc#import-tool-options[Options]) if your data does not fit the default format.
...
The property type are specified in the CSV header row, see xref:tools/neo4j-admin-import.adoc#import-tool-header-format[CSV header format].
...
See xref:tools/neo4j-admin-import.adoc#import-tool-id-spaces[Using ID spaces].
...
For more information, see the `xref:tools/neo4j-admin-import.adoc#import-tool-option-skip-bad-relationships[--skip-bad-relationships]` option.
...
For more information, see the `xref:tools/neo4j-admin-import.adoc#import-tool-option-skip-duplicate-nodes[--skip-duplicate-nodes]` option.
