...
[[docker-cc]]
= Deploy a Neo4j cluster via Docker locally
...
For production deployment across multiple servers, please see
xref:clustering/index.adoc[here].
For deployment using docker across multipler servers, please see xref:docker/clustering.adoc[here]
...
# The behavior of the initial discovery is determined by the parameters `dbms.cluster.discovery.type` and `dbms.cluster.discovery.endpoints`.
