...
----
http://localhost:7474/browser/oauth/callback
----
====
...
.. Add the *Sign-in redirect URIs*, for example, _\http://localhost:7474/browser/oauth/callback_.
...
dbms.security.oidc.okta.well_known_discovery_uri=https://dev-54101110.okta.com/oauth2/default/.well-known/openid-configuration
...
.. In the *Value* field, type `(appuser != null) ? appuser.login : app.clientId`.
.. Click *Create*.
...
. Under *Redirect URI*, select `Single-page application (SPA)` and enter the following static redirect URI: _\http://localhost:7474/browser/oauth/callback_.
This redirect URI is used to complete the OAuth 2.0 / OpenID Connect authorization code flow.
+
[NOTE]
====
The redirect URI must be registered exactly as configured in the identity provider. Application-specific context (such as selected identity provider or flow state) should be passed using the OAuth state parameter rather than query parameters in the redirect URI.
====
. Click *Register*.
