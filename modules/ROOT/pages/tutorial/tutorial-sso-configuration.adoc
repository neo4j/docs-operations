...
----
https://{server}:7474/browser/?idp_id={provider}&auth_flow_step=redirect_uri
----
====
...
Because Neo4j Browser is a single-page application, this is the recommended configuration.
. Click *Next*.
. Configure the client with the appropriate redirect URI.
.. Add a name for the app integration.
.. Add the *Sign-in redirect URIs*, for example, _\https://{server}:7474/browser/?idp_id={provider}&auth_flow_step=redirect_uri_.
This URI will accept returned token responses after successful authentication.
. Add the *Sign-out redirect URIs*, for example, _\https://{server}:7474/browser/?idp_id={provider}&auth_flow_step=redirect_uri_.
...
dbms.security.oidc.okta.well_known_discovery_uri=https://example.okta.com/oauth2/default/.well-known/openid-configuration
...
`implicit` flow has been removed from OAuth 2.1 because it exposes the access token in the browser history and the redirect URL, in addition to other security vulnerabilities.
...
`implicit` flow has been removed from OAuth 2.1 because it exposes the ID token in the browser history and the redirect URL, in addition to other security vulnerabilities.
Starting with Neo4j 2026.06, its support is deprecated and will be removed in a future release.
====
...
. Under *Select the supported account types*, select `Accounts in this organizational directory only (Default Directory only - Single tenant)` or the option appropriate for your deployment.
. Under *Redirect URI*, select `Single-page application (SPA)` and enter the following static redirect URI: _\https://{server}:7474/browser/?idp_id={provider}&auth_flow_step=redirect_uri_.
This redirect URI is supported only by single-tenant applications and is used to redirect the user back to the Neo4j Browser after successful authentication.
+
[NOTE]
====
If using multi-tenant applications, it is recommended to use a centralized callback endpoint, such as _\https://login.neo4j.com/login/callback_, that can handle the redirect from multiple tenants and then redirect the user to the appropriate Neo4j Browser instance.
The redirect URI must be registered exactly as configured in the identity provider.
Application-specific context (such as selected identity provider or flow state) should be passed using the OAuth `state` parameter rather than query parameters in the redirect URI.
====
. Click *Register*.
...
# Configure the iss claim in the access token with the Directory (tenant) ID
# Make sure you add the trailing slash (`/`) at the end of the URL or this operation might fail.
dbms.security.oidc.azure.issuer=https://login.microsoftonline.com/54e85725-ed2a-49a4-a19e-11c8d29f9a0f/v2.0
...
----
. From the left-hand side menu, navigate to *Microsoft Entra ID -> Groups*.
...
Some frequently asked questions and answers about SSO configuration.
