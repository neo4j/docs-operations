...
[[why-to-use-pkce]]
=== Why to use PKCE as auth flow?

Assuming the client (Neo4j Browser or Bloom) can be accessed through the public internet, always use `pkce` auth flow.
...
It has been removed from OAuth 2.1 because it exposes the access token in the browser history and in the URL redirect, in addition to other security vulnerabilities.
Implicit flow is deprecated starting with Neo4j 2026.06 and will be removed in a future release.
====
