...
.. Set the `dbms.security.require_local_user` configuration setting to `true` in the _neo4j.conf_ file. This will switch to Auth Providers mode whereby users can only authenticate and authorize if they have a corresponding Auth Provider in the database.
