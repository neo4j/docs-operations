...
. The Neo4j server contacts the identity provider (Okta, Entra ID, Google, etc.) and fetches the JSON Web Keys (JWKs) from the provider.
...
# Configure the audience claim in the access token with the Application (client) ID
dbms.security.oidc.azure.audience=4376dc8b-b5af-424f-9ada-c1c1b2d416b9
# Configure the OIDC well-known discovery endpoint with the Directory (tenant) ID
...
[[tutorial-sso-entra-groups-mapping]]
...
See https://learn.microsoft.com/en-us/entra/identity/hybrid/connect/how-to-connect-fed-group-claims[Microsoft documentation] for details on when to use which and how it can help with the 200 roles limit described in <<tutorial-sso-entra-groups-mapping>>.
