...
.. Add the *Sign-in redirect URIs*, for example, _\http://localhost:7474/browser/preview/?idp_id=okta&auth_flow_step=redirect_uri_.
This URI will accept returned token responses after successful authentication.
. Add the *Sign-out redirect URIs*, for example, _\http://localhost:7474/browser/preview/?idp_id=okta&auth_flow_step=redirect_uri_.
...
. Under *Redirect URI*, select `Single-page application (SPA)` and enter the following static redirect URI: _\http://localhost:7474/browser/?idp_id=azure&auth_flow_step=redirect_uri_.
