...
.Details
|===
| *Syntax* 3+| `dbms.cluster.recreateDatabase(database :: STRING, options = {} :: MAP)`
| *Description* 3+| Recreates a database while keeping all RBAC settings.
...
.Details
|===
| *Syntax* 3+| `dbms.cluster.statusCheck(databases, timeoutMilliseconds) :: (database, serverId, serverName, address, replicationSuccessful, memberStatus, recognisedLeader, recognisedLeaderTerm, requester, error)`
| *Description* 3+| Performs a rafted status check.
