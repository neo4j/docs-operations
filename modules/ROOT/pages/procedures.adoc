...
For an accurate representation of the database schema, use link:{neo4j-docs-base-uri}/apoc/current/overview/apoc.meta/apoc.meta.graph/[`apoc.meta.graph`], which examines the full graph and returns a meta-graph.
Note that the `apoc.meta.graph` procedure is more time- and resource-consuming than `db.schema.visualization()`.
====
