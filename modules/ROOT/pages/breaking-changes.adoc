:description: Page lists breaking changes in Neo4j 2025.01 series. Here you can find changes made to procedures, configurations settings, metrics, and `neo4j-admin` commands.

//Check Mark
:check-mark: icon:check[]

[[breaking-changes-2025.01]]
= Breaking changes in Neo4j 2025.01

The section covers breaking changes in Neo4j 2025.01, the first release after Neo4j 5.26 LTS.
Breaking changes are only made in the release immediately following a long-term support (LTS) release.
For guidelines on how to migrate from previous Neo4j versions to the latest version, Neo4j {neo4j-version}, see the link:https://neo4j.com/docs/upgrade-migration-guide/current/version-2025[Upgrade and Migration Guide].
...
`connect-randomly-to-server-group` and `connect-randomly-within-server-group` are removed and replaced by `connect-randomly-to-server-tags` and `connect-randomly-within-server-tags` respectively.
...
| link:{neo4j-docs-base-uri}/operations-manual/5/configuration/configuration-settings/#config_server.max_databases[server.max_databases] label:enterprise[Enterprise Edition]
|Replaced by xref:configuration/configuration-settings.adoc#config_dbms.max_databases[`dbms.max_databases`].
...
In Neo4j 2025.01, the following operating systems are no longer supported:
...
[[ssl-framework-breaking-changes]]
== SSL framework
...
Starting with Neo4j 2025.01, the `URLConnectionSeedProvider` does not support `file`.
This is true for both Cypher 5 and Cypher 25. +
It is replaced by `FileSeedProvider`.
See xref:database-administration/standard-databases/seed-from-uri.adoc#seed-provider-reference[Seed provider reference] for more details.
