...
The Azure Marketplace listing uses a Azure Resource Manager (ARM) template maintained by Neo4j. 
...
The Neo4j link:https://azuremarketplace.microsoft.com/en-us/marketplace/apps/neo4j.neo4j-ee?tab=Overview[Azure marketplace listing^] can be configured to deploy either Neo4j Enterprise Edition v5 or v4.4.
...
Azure Resource Manager (ARM) is a declarative Infrastructure as Code (IaC) language that is based on JSON and instructs AWS to deploy a set of cloud resources.
...
| Admin password
| An password for the `neo4j` user. The password must be between 12 and 72 characters long, and contain characters from at least 3 of the following groups: uppercase characters, lowercase characters, numbers, and special characters.
|===
...
| Node Count
| Specify the number of desired Virtual Machine instances to be used to form a Neo4j cluster (a minimum of 3 instances is required to form a cluster). 

| Disk Size
| Size (in GB) of the secondary volume on each Azure VM instance. 

| Graph Database Version
| Select "5" for the latest minor version of Neo4j v5, or select "4.4" for the latest minor version of Neo4j v4.4
...
After the deployment finishes successfully, the ARM template provides the following outputs, which can be found in the *Outputs* section of the CloudFormation page on the AWS console.
...
If you add more VM instances to your VMSS, they will be tagged with the same minor version, ensuring that all Neo4j cluster servers are installed with the same version, regardless of when the VM instances were created.
