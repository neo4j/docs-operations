...
The Neo4j image is available with either `debian:trixie-slim`, `debian:bullseye-slim`, `redhat/ubi10-minimal:latest`
or `redhat/ubi9-minimal:latest` as the base image. The default is `debian:trixie-slim`.
