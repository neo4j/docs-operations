...
* `+NEO4J_dbms_cluster_discovery_endpoints+`: the network addresses of an initial set of Core cluster members.
...
docker run --name=core1 --detach --network=neo4j-cluster \
    --publish=7474:7474 --publish=7473:7473 --publish=7687:7687 \
    --hostname=core1 \
    --env NEO4J_dbms_mode=CORE \
    --env NEO4J_dbms_cluster_discovery_endpoints=core1:5000,core2:5000,core3:5000 \
    --env NEO4J_ACCEPT_LICENSE_AGREEMENT=yes \
    --env NEO4j_server_bolt_advertised_address=localhost:7687 \
    --env NEO4j_server_http_advertised_address=localhost:7474 \
    --env NEO4J_AUTH=neo4j/foo123 \
    neo4j:{neo4j-version-exact}-enterprise

docker run --name=core2 --detach --network=neo4j-cluster \
...
docker run --name=core3 --detach --network=neo4j-cluster \
...
[source, shell, subs="attributes"]
----
docker run --name=read-replica1 --detach --network=neo4j-cluster \
...
In order to create a  highly-availabile cluster of containers, cluster nodes can be deployed on different physical machines.
...
docker run --name=neo4j-core --detach \
         ...
         --env NEO4J_dbms_mode=CORE \
         --env NEO4J_dbms_cluster_discovery_endpoints=core1-public-address:5000,core2-public-address:5000,core3-public-address:5000 \
