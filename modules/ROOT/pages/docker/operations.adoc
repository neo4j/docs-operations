...
The xref:tools/neo4j-admin-memrec.adoc[neo4j-admin memrec] command with the argument `--docker` outputs environmental variables that can be passed to a Neo4j docker container.
...
|[enterprise-edition]#&#32;# Bloom footnote:bloom[The Bloom plugin requires a license and this needs to be provided as a shared volume. Please see link:{neo4j-docs-base-uri}/bloom-user-guide/current/bloom-installation/#bloom-docker[Bloom User Guide -> Installing Bloom in a Docker container].]
|bloom
