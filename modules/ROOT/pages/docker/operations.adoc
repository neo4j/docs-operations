...
| APOC Core
| `apoc-core`
| link:{neo4j-docs-base-uri}/apoc[APOC]
...
.Install the APOC plugin (`apoc`) and the Neo Semantics plugin (`n10s`)
====
...
[NOTE]
====
The `--user="$(id -u):$(id -g)"` argument is required to ensure that the plugins are installed with the correct permissions.
====
