...
The xref:neo4j-admin-neo4j-tools.adoc[`neo4j-admin` CLI] can be run locally within a container using the following command:
...
For more information about the `neo4j-admin` commands, see xref:neo4j-admin-neo4j-tools.adoc[].

[[docker-neo4j-import]]
== Use Neo4j Import

The xref:import/index.adoc[Neo4j Import tool] can be run locally within a container using the following commands:
...
For more information about the commands' syntax and options, see xref:import/full-import.adoc[Full import] and xref:import/incremental-import.adoc[Incremental import].
...
* Verify that the CSV files that you want to load into Neo4j are formatted as per xref:import/header-format.adoc#import-tool-csv-header-format[CSV header format].
