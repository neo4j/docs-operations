...
services:
  neo4j:
    ...
    volumes:  # <1>
        ...
        - /$HOME/neo4j/plugins:/plugins # <2>
    environment:
        - NEO4J_AUTH=neo4j/your_password # <3>
    ports:
      - "7474:7474"
      - "7687:7687"
    restart: always
...
<2> If you face broken plugin lookups, keep the plugins volume mapping and add the matching configuration path under your environment variables: `- NEO4J_server_directories_plugins=/plugins`.
The configuration setting does not replace `/plugins` volume mount and only controls where Neo4j looks for plugins.
<3> Set the `neo4j` username and password.
