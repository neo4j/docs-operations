...
If you need to adjust the Neo4j logging configuration when running Neo4j in Docker, follow the steps.

. Set the `neo4j` user as the owner of _$HOME/neo4j/conf_ to allow _write_ access from the Neo4j Docker container:
+
[source, shell, subs="attributes"]
----
sudo chown neo4j:neo4j $HOME/neo4j/conf
