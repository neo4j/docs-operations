...
[NOTE]
====
For production use, it is recommended to use one of the latter two methods, replacing the entire configuration.
This provides stability in the face of changes or additions to the default configuration in the image.
====
...
The Neo4j logging configuration is located in the _conf_ directory and consists of two files:
...
Note that _query.log_ and _security.log_ are part of the Enterprise Edition.

To adjust the Neo4j logging configuration when running Neo4j in Docker, you can use environment variables or mount a _conf/_ volume.

Mounting a _conf_ volume::
+
.  Dump the default configuration by running the command:
+
[source, shell, subs="attributes"]
----
docker run --rm \
...
----
+
By including --user="$(id -u):$(id -g)", you ensure the files created on your host are owned by you.
This generates the default _neo4j.conf_ and logging configuration files into the mounted _/conf_ directory so you can modify them.

. Edit the _neo4j.conf_ file to ensure that you listen on all network interfaces:
+
[shell]
----
# Required when mounting /conf in Docker
server.default_listen_address=0.0.0.0
----

. Adjust the _user-logs.xml_ and _server-logs.xml_ files by following examples on the xref:monitoring/logging.adoc#_default_logging_configuration[Configure Neo4j logging].
For example, you can modify xref:monitoring/logging.adoc#general-logging-loggers[`<Loggers>`] or xref:monitoring/logging.adoc#general-logging-appenders[`<Appenders>`].
...
. Finally, verify logging works by running the command after startup:
+
