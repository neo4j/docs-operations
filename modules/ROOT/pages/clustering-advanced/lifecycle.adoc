...
The discovery protocol continues to run throughout the lifetime of the Causal Cluster and is used to maintain the current state of available servers and to help clients route queries to an appropriate server via the client-side xref:introduction.adoc#intro-drivers[drivers].
...
//Only the Akka-based implementation supports <<clustering-intra-cluster-encryption, intra-cluster encryption>> of the discovery protocol.
