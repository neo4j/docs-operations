...
* JVM heap, configured for the admin process through the usual configuration sources described in xref:neo4j-admin-neo4j-tools.adoc#_configuration[`neo4j-admin` and `neo4j` tools -> Configuration].
...
The creation of indexes and constraints during the import process is supported only by the `block` format (Enterprise Edition) starting with Neo4j 2025.02.
...
For other formats or versions prior to 2025.02, you have to add indexes and constraints manually after the import process.
See link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/[Cypher Manual -> Indexes] for more information.
...
This feature is not supported by `block` format.
