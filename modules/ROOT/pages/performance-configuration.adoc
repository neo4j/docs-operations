...
Indexes and constraints are best created through link:{neo4j-docs-base-uri}/cypher-manual/{page-version}/administrationes-for-search-performance[Cypher], but when these indexes or constraints need to be more specifically configured than what is possible through Cypher, then you can use the procedures described in the example below.
...
A full description on how to create and use full-text indexes is provided in the link:/docs/cypher-manual/4.0/administration/indexes-for-full-text-search/[Cypher Manual -> Indexes to support full-text search].
